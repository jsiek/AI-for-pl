...
mutual
  normalize-seq : ∀ {A B C}
    → (s : A ⇨ⁿ B)
    → (t : B ⇨ⁿ C)
    → Σ[ u ∈ A ⇨ⁿ C ] ((⌊ s ⌋ˢ ⨟ ⌊ t ⌋ˢ) —↠ ⌊ u ⌋ˢ)

  _i⨟s_ : ∀ {A B C}
    → (i : A ⇨ⁱ B)
    → (t : B ⇨ⁿ C)
    → (Σ[ j ∈ A ⇨ⁱ C ] ((⌊ i ⌋ⁱ ⨟ ⌊ t ⌋ˢ) —↠ ⌊ j ⌋ⁱ))
      ⊎ (A ≡ C × ((⌊ i ⌋ⁱ ⨟ ⌊ t ⌋ˢ) —↠ idᶜ A))

  _g⨟i_ : ∀ {A B C}
    → (g : A ⇨ᵍ B)
    → (i : B ⇨ⁱ C)
    → Σ[ j ∈ A ⇨ⁱ C ] ((⌊ g ⌋ᵍ ⨟ ⌊ i ⌋ⁱ) —↠ ⌊ j ⌋ⁱ)

  _g⨟g_ : ∀ {A B C}
    → (g : A ⇨ᵍ B)
    → (h : B ⇨ᵍ C)
    → Σ[ k ∈ A ⇨ᵍ C ] ((⌊ g ⌋ᵍ ⨟ ⌊ h ⌋ᵍ) —↠ ⌊ k ⌋ᵍ)

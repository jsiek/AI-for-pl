...
structural-lift-left : ∀ {Δᴸ Δᴿ Δᴿ′ Δ Δ′}
  ...
  → (v : VarImp)
  → StructuralWorldExtendᴿ χs (CTI2.liftWorldLeft W)
...
structural-lift-left-frozen : ∀ {k Δᴸ Δᴿ Δᴿ′ Δ Δ′}
  ...
  → FrozenStructuralTraceᴿ (Nat.suc k)
      (structural-lift-left plan v)
structural-lift-left-frozen frozen-trace-[] {v = v} = frozen-trace-[]
structural-lift-left-frozen (frozen-trace-keep frozen) {v = v} =
  frozen-trace-keep (structural-lift-left-frozen frozen {v = v})
structural-lift-left-frozen
    (frozen-trace-bind frozen-ins frozen) {v = v} =
  frozen-trace-bind (frozen-embedding-keep frozen-ins)
    (structural-lift-left-frozen frozen {v = v})

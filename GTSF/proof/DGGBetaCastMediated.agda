...
-- File Charter:
--   * Mediated-store DGG helpers for target beta-through-function-cast
--     steps.
--   * Keeps the wrap-narrowing / source-cast recursion out of the main
--     DynamicGradualGuaranteeMediated module.
--   * Ports the usable shape of `DGGBetaCastSeparated` and the
--     `sim-beta-cast` argument/result wrapping pattern to the mediated
--     relation.

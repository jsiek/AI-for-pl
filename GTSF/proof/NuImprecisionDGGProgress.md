...
| [`NuImprecisionMatchedLiftInvariantsDef.agda`](NuImprecisionMatchedLiftInvariantsDef.agda) | completed reusable statement; GPT 5.5 proof pending | Preserves the exact-world invariant triple across a matched `LiftStoreⁱ`: coherence and source-name exclusivity at the shifted context, plus left `StoreWf` at the shifted store. This is the next frozen Ginger leaf and a direct dependency of matched-unseal closing. |

module strong-rep-nu.notes.CancelRShiftWall where
...
--     `strong-rep-nu.Reduction` carries the repaired rule and
--     `strong-rep-nu.proof.MoveScope.preserve-CancelR` proves the
...
--
-- WHAT THE RULE SAID, BEFORE (strong-rep-nu.Reduction, until
-- 2026-09-19):
--
...
open import strong-rep-nu.TypeCheck using (tc; tr; int!; conv!; sq!; wf!)
open import strong-rep-nu.proof.MoveScope using (preserve-CancelR)
...
-- `shiftRep` was `strong-rep-nu.Ctx`'s lift of a representation past
...
-- `CancelRCase°` is `strong-rep-nu.proof.Preserve.CancelRCase` AS IT
...
-- `strong-rep-nu.proof.MoveScope.preserve-CancelR` (§6).
CancelRCase° : Set

module LR-narrow.Context.TagMatchBackward where

-- File Charter:
--   * Proves backward coherence of corresponding interpreter tag checks.
--   * Equal right tags force equal left tags through paired-seal injectivity.
...
open import LR-narrow.Context.PairedBindingInjective
...
tag-match-backward {current = current}
  ...
  cong (λ α → variable-tag (seal-name α))
    (paired-binding-injective {w = current} binding other-binding)
tag-match-backward base-tags-equal base-tags-equal refl = refl
tag-match-backward function-tags-equal function-tags-equal refl = refl

...
| [`NuImprecisionPairedLambdaTargetClosingFrameClosingHandlersProof.agda`](NuImprecisionPairedLambdaTargetClosingFrameClosingHandlersProof.agda) | completed top-level higher-order assembly; canonical `Lemma` absent | Constructs all thirteen handler fields from thirteen exact semantic dependencies, composing matched-`Λ` and source-gen structural conversion reductions plus paired-widening and quotient up-id constructor dispatchers; this is the fit-checking skeleton that prevents independently proved leaves from drifting away from the interpreter |

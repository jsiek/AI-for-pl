...
module proof.DGG.Catchup.ContextualLeftSourceCastCatchupDef where

-- File Charter:
--   * States source-headed cast catch-up beneath the canonical full CTI
--     zipper as a separate semantic induction.
--   * Keeps the exact source cast checkpoint and whole related root visible
--     while returning the evolved root-to-focus path inline.
...
ContextualLeftSourceCastCatchupAt : ℕ → Set₁

...
- DualRep≈: FALSE as stated, REPAIRED AND FULLY PROVEN
  (strong-rep-store/DualRepProof.agda): BlkRepWf (the index relation
...
- DualInt≈: FALSE (two corners, xrvld and double-refusal — both the
  ...
  above; strongest-true version delivered (strong-rep-store/DualIntProof.agda):
...
FACE half of a split boundary — scope skeleton (strong-rep-store/, rep-free) +
...
boundary scope entry and the type-context entry it creates), ali → unlock,
...
RESULT: strong-rep-store.Preservation has top-level `preservation : Preservation`
...
retired from identifiers, comments and notes of strong-rep-store/ (v1 memos,
...
mw-b's rep independence from the same boundary scope's unlocks), then:
...
(tail's unlocks).  RULED: refactor into a record `boundary binds changes`
...
boundary scope's unlocks (not just those listed after it) — strictly more
...
`crossΛ W A = ⇑ᴹ W ⟪ boundary [] (lock 0 ∷ []) , mkId (⇑ᵗ A) ⟫`; Beta is
...
lock-free Θ₂ (`TyBetaBoundary`, or a `⋉` of it), and for a lock-free Θ the
dual is all locks, which the conversion context skips.

THE RULING.  The conversion context is the UNION of the names live
anywhere along the boundary scope — that is what "skips `lock`s" means.  Read
...
INSTALLED in strong-rep-store.Boundary §3.  The whole development still checks, and
...
THE TOOLING THIS FORCED (strong-rep-store.TypeCheck).  Because `_⋉_` and `rewind`
...
is looking at a different one.  `Θ↔ = boundary [] (unlock 1 0 ∷ lock 0 0 ∷ [])`
...
map with `Unique` names by `same-target-unique` (strong-rep-store.Ctx) — already
...
and `SameConv` in `strong-rep-store.Conversion` §2b, which is `_⊢_~_` one universe
...
`notes/PeelPremise.agda` to `strong-rep-store.Boundary` §3a, where it is stated from
...
THE RULING, INSTALLED in `strong-rep-store.Boundary` §3a:
...
`strong-rep-store.proof.Preserve` keeps `CancelRCase` as the parameter of
`Impl` it always was, and `strong-rep-store.Preservation` and `strong-rep-store.TypeSafety`
...
THE INTERFACE SHRANK. `strong-rep-store.Preservation.Stage1` now takes `crossΛ`,
`addLock0`, `repWeaken` and `cancel`; `peel` and `idpush` are discharged
and gone. `strong-rep-store.TypeSafety.Stage1` and `proof/TypeSafety.agda` follow.
...
* `proof/MaskFacts.agda` — its whole subject is masking: `mask`/`unmask`
  ...
  `conv-lock` itself (`strong-rep-store.Boundary` §3), which skips a lock outright,
...
* `proof/PreserveObstruct.agda` — four concrete redexes, each written in
  ...
  into `strong-rep-store.Examples`), which exercise them from plain source rather
...
* `proof/Canonicity.agda`, with ONE NEW SECTION. The family and its mint,
  ...
  which validated the invariant on `strong-rep-store.Examples`, is dropped while that
...
* `proof/ShiftAudit.agda`. The criterion survives verbatim; what changes is
  ...
  live facts about the live wrapper clause, and `strong-rep-store.Reduction`'s own
  comment cites them.
...
`strong-rep-store.proof.PreserveObstruct` twice (§§ around its IdPush and TyPeelR
...
comments in `strong-rep-store.Reduction` still name the four deleted modules; they
...
* old §11c `G` — the only closed source that reaches `TyPeelR` over a
  ...
  the live frames (`instantiate`, `dualBoundary`, `addLock0 ∘ renᴮ²`, `_⋉_`,
...
* old §12b, and the witnesses old §13a/§13b imported — they came from
  `strong-rep-store.proof.PreserveObstruct`, deleted in the module sweep.
* old §15 (TIGHTNESS, RULE BY RULE) — its seven frame identities were
  ...
  (`strong-rep-store.Boundary` §3a), and `proof/ShiftAudit.agda` is what consumes
  them.
...
`showRun` renders a whole `strong-rep-store.Eval` trace with the name of the rule
...
THE RULING: STILL NOTHING IS CHANGED.  `strong-rep-store.Reduction` is untouched,
`strong-rep-store.proof.Preserve` keeps `CancelRCase` as the open parameter it was,
...
approved and installed.  `strong-rep-store.Reduction` is no longer untouched.
...
`strong-rep-store.proof.MoveScope.preserve-CancelR`, beside `preserve-IdPush` and by
...
CONSEQUENTLY THE INTERFACE SHRANK AGAIN.  `strong-rep-store.Preservation.Stage1`
takes `crossΛ`, `addLock0` and `repWeaken` — `cancel` is gone, as `peel`
and `idpush` went before it.  `strong-rep-store.TypeSafety.Stage1` and
...
`strong-rep-store.proof.RepWeaken.rep-weaken-⊢`, and `PeelCase` is therefore
UNCONDITIONAL: `strong-rep-store.Preservation.Stage1` now takes only `crossΛ` and
...
`strong-rep-store.proof.PeelDual` §3 — the premise is `bw-binds mwΘ` of the very
...
The cut is then entirely in the closure lemmas, `strong-rep-store.Boundary` §3d:
...
per-relation lemma, in the style §3a already used:
  ...
  * the conversion's TYPING by `conv-ren` (strong-rep-store.Conversion §2d).  A
...
`TermSubst.agda` to `strong-rep-store.Ctx` §8, and `renᶠᴿ`/`renᴮᴿ` from
`TermSubst.agda` to `strong-rep-store.Boundary` §2/§3, because the renaming
...
`strong-rep-store.Ctx` is already opened there.  `strong-rep-store.Ctx` §8 also gains the
...
The proof is `strong-rep-store.proof.RepWeaken.cross-Λ-⊢`, beside
...
THE INTERFACE SHRANK.  `strong-rep-store.Preservation.Stage1` now takes ONLY
...
`CancelR` and `IdPush`.  `strong-rep-store.TypeSafety.Stage1` and
`strong-rep-store.proof.TypeSafety.Stage1` now take `merged-reading` plus
...
WHAT THE PUBLIC SURFACE SAYS NOW.  `strong-rep-store.Preservation.Stage1` and
`strong-rep-store.TypeSafety.Stage1` keep their `addLock0` parameter, but that
...
`proof/Progress.addLock0-reading`, from three pieces:
  ...
  * `conv-weaken` and `conv-snoc-lock` (`Boundary.agda` §3, new): a
  ...
  * `addLock0-conversion-ren` (`Boundary.agda` §3d, new): those two after
...
experiment reported.  `strong-rep-store.Progress.Stage1` therefore still takes
...
lemma that already existed or by one small new one:
  ...
  * `bw-interior` by `addLock0-interior-ren` (`Boundary.agda` §3d, NEW —
...
CONSEQUENTLY `strong-rep-store.Preservation.Stage1` IS GONE.  `preservation` and
`preservation*` are stated outright, unconditionally.  `strong-rep-store.TypeSafety`
exports them unconditionally too, and its `Stage1` — like
`strong-rep-store.Progress.Stage1` and `strong-rep-store.proof.TypeSafety.Stage1` — now takes
...
The resulting module map, bottom up:
  ...
  * `proof/Types.agda` (NEW) — `substᵗ-cong`, `extsᵗ-renᵗ`,
    `substᵗ-renᵗ`.  Imports `strong-rep-store.Types` and the standard library, and
  ...
  * `Boundary.agda` — `Change`, the change judgements, `Boundary`, the
    two induced readings, `dualBoundary`/`rewind`/`_⋉_`/`addLock0`/
...
at all.  Downstream, twelve files gained `open import strong-rep-store.proof.Ctx`;
...
`strong-rep-store.Boundary.merged-conversion-exists`; no premise was added.

THE MILESTONE. `strong-rep-store.proof.Progress.Impl` now consumes the proved
`merged-reading`, and `strong-rep-store.Progress.progress` is stated outright.
`Stage1` is deleted from `strong-rep-store.Progress`, `strong-rep-store.proof.TypeSafety` and
`strong-rep-store.TypeSafety`; `type-safety` is stated outright. Preservation,
...
`strong-rep-store.All` already reached `strong-rep-store.Examples`, so the checked set loses
...
| old | new |
|---|---|
| module `strong-rep-store.CtxMorph`, file `CtxMorph.agda` | `strong-rep-store.Boundary`, `Boundary.agda` |

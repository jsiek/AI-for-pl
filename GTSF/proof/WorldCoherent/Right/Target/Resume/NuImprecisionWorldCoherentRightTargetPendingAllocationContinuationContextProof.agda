...
-- File Charter:
--   * Composes a raw source-silent paired-lambda pending-allocation prefix
--     with the exact recursive world-coherent right-value catch-up.
--   * Preserves indexed transport, type coherence, source-bullet transport,
--     target-context action, and right-only store lineage without requiring
--     the raw pending target to be a value.
...
private
  source-silent-right-only-store-lineage :
    ...
    Σ[ combined-lineage ∈
      WeakOneStepStoreLineage
        (source-silent-resultᵀ
          first source-empty source-same second) ]
      RightOnlyStoreImpPrefix
...
world-coherent-right-target-pending-allocation-continuation-context-proofᵀ
    {Φ = Φ}
    first-indexed refl refl target-exact
    ...
    with source-silent-right-only-store-lineage
      (weakIndexedResult first-indexed)
      refl refl
      (weakIndexedResult
        (rightCatchupIndexedResult
...
world-coherent-right-target-pending-allocation-continuation-context-proofᵀ
  ...
  world-coherent-right-value-indexed-catchup
      (right-value-indexed-catchup
        ...
        (rightCatchupSourceValue second-catchup)
        (rightCatchupSourceNoBullet second-catchup)
        (rightCatchupTargetValue second-catchup)
        (rightCatchupTargetNoBullet second-catchup))
      combined-lineage combined-bullet
      (worldRightCatchupCoherence continued)
      (worldRightCatchupSourceNameExclusive continued)
      combined-unique
      (worldRightCatchupTargetStoreWf continued) ,
    combined-context ,
    combined-prefix
  where
  first = weakIndexedResult first-indexed

  second-catchup = worldRightCatchupResult continued
  second-indexed = rightCatchupIndexedResult second-catchup
  second = weakIndexedResult second-indexed
  second-lineage = worldRightCatchupStoreLineage continued
  second-source-empty =
  ...
  combined =
    source-silent-resultᵀ first refl refl second

  combined-unique =
    worldRightCatchupAssumptionMembershipUnique continued
  ...
  combined-source-empty =
    source-silent-preserves-changes-exactᵀ
      first refl refl second second-source-empty

  combined-source-same =
    source-silent-preserves-result-exactᵀ
      first refl refl second second-source-same

  combined-bullet =
    source-silent-source-bullet-transport
      first refl refl second second-source-empty
      first-bullet
      (worldRightCatchupSourceBulletTransport continued)

...
-- File Charter:
--   * Defines the contextual pre-`β-inst` target-instantiation leaf for a
--     direct paired `Λ⊑Λᵀ` relation and a source-only final index.
--   * Retains the matched body relation instead of manufacturing an
--     independent right-opened relation after target allocation.

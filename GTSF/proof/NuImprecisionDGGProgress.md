...
| [`NuImprecisionRightSilentQuotientWideningPairTransportDef.agda`](NuImprecisionRightSilentQuotientWideningPairTransportDef.agda) | completed flat statement; proof not started | Symmetric quotient-widening transport for the active `up⊑upᵀ` case. It consumes and returns `QuotientWideningPair` directly with transformed target coercion and endpoint types; no QTIP or right-silent result carrier is introduced. |

...
data BlameFree : Term → Set where
  ...
  bf-• : ∀ {M α} → BlameFree M → BlameFree (M • α)
  bf-ν : ∀ {A M} → BlameFree M → BlameFree (ν:= A ∙ M)
  bf-$ : ∀ {κ} → BlameFree ($ κ)
...
lookupAny-⟰ᵗ :
  ...
  Σ[ B ∈ Ty ] Σ ∋ˢ α ⦂ B
lookupAny-⟰ᵗ {Σ = []} ()
lookupAny-⟰ᵗ {Σ = (β , B) ∷ Σ} (Z∋ˢ α≡β A≡B) =
  B , Z∋ˢ α≡β refl
lookupAny-⟰ᵗ {Σ = (β , B) ∷ Σ} (S∋ˢ h) with lookupAny-⟰ᵗ {Σ = Σ} h
... | C , hC = C , S∋ˢ hC
...
∉dom-⟰ᵗ :
  ...
  α ∉domˢ Σ →
  α ∉domˢ (⟰ᵗ Σ)
∉dom-⟰ᵗ α∉Σ h with lookupAny-⟰ᵗ h
... | B , hB = α∉Σ hB
...
unique-⟰ᵗ :
  ...
  Uniqueˢ (⟰ᵗ Σ)
unique-⟰ᵗ uniq[] = uniq[]
unique-⟰ᵗ (uniq∷_ α∉Σ uΣ) = uniq∷_ (∉dom-⟰ᵗ α∉Σ) (unique-⟰ᵗ uΣ)
...
type-unique-blamefree uΣ (bf-• bfM) (⊢• {A = A} {α = α} M:∀A α∈ h) (⊢• {A = B} {α = α} M:∀B α∈′ h′) =
  cong (λ T → T [ ｀ α ]ᵗ) bodyEq
  where
    allEq : `∀ A ≡ `∀ B
    allEq = type-unique-blamefree uΣ bfM M:∀A M:∀B

    bodyEq : A ≡ B
    bodyEq = cong forallBodyTy allEq
type-unique-blamefree uΣ (bf-ν {A = A} bfM) (⊢ν wfA M:⇑B) (⊢ν wfA′ M:⇑B′) =
  ⇑ˢ-injective
    (type-unique-blamefree (unique-ν A uΣ) bfM M:⇑B M:⇑B′)
type-unique-blamefree uΣ bf-$ (⊢$ κ) (⊢$ κ′) = refl
type-unique-blamefree uΣ (bf-⊕ bfL bfM) (⊢⊕ L:ℕ op M:ℕ) (⊢⊕ L:ℕ′ op′ M:ℕ′) = refl
type-unique-blamefree uΣ (bf-up bfM) (⊢up {A = A} {B = B} M:A hp) (⊢up {A = A′} {B = B′} M:A′ hp′) =
  proj₂ (wt⊑-unique uΣ hp hp′)
type-unique-blamefree uΣ (bf-down bfM) (⊢down {A = A} {B = B} M:A hp) (⊢down {A = A′} {B = B′} M:A′ hp′) =
  proj₂ (wt⊒-unique uΣ hp hp′)
...
groundOkDec :
  ...
  (Ξ : List Bool) →
  Dec (⊢ g ok Ξ)
groundOkDec (｀ α) Ξ = permMemberDec α Ξ
groundOkDec (‵ ι) Ξ = yes tt
groundOkDec ★⇒★ Ξ = yes tt

ground-ok-cong :
  ∀ {G : Ty} {Ξ : List Bool} {g g′ : Ground G} →
  ⊢ g ok Ξ →
  ⊢ g′ ok Ξ
...
mutual
  up-cast-check′ :
    ...
    (Φ Ξ : List Bool) →
    (p : Up) →
    Dec (Σ[ A ∈ Ty ] Σ[ B ∈ Ty ] Σ ∣ Φ ∣ Ξ ⊢ p ⦂ A ⊑ B)
  up-cast-check′ Σ uΣ Φ Ξ (tag G) with groundTyDec G
  ... | no ¬g =
      no
        (λ
          { (A , (B , wt-tag g gokΞ)) → ¬g g
          })
  ... | yes g with groundOkDec g Ξ
  ...   | no ¬gok =
        no
          (λ
            { (A , (B , wt-tag g′ gokΞ)) →
                ¬gok (ground-ok-cong {g = g′} {g′ = g} gokΞ)
            })
  ...   | yes gokΞ = yes (G , (★ , wt-tag g gokΞ))

  up-cast-check′ Σ uΣ Φ Ξ (unseal α) with permMemberDec α Φ | lookupStoreAnyDec Σ α
  ... | no α∉Φ | _ =
      no
        (λ
  ...
  up-cast-check′ Σ uΣ Φ Ξ (p ↦ q)
      with down-cast-check′ Σ uΣ Φ Ξ p | up-cast-check′ Σ uΣ Φ Ξ q
  ... | no ¬p | _ =
      no
        (λ
  ...
  up-cast-check′ Σ uΣ Φ Ξ (∀ᵖ p) with up-cast-check′ (⟰ᵗ Σ) (unique-⟰ᵗ uΣ) Φ Ξ p
  ... | no ¬p =
      no
        (λ
          { (A , (B , wt-∀ hp)) → ¬p (_ , (_ , hp))
  ...
  up-cast-check′ Σ uΣ Φ Ξ (ν p)
      with up-cast-check′ ((zero , ★) ∷ ⟰ˢ Σ) (unique-ν ★ uΣ) (true ∷ Φ) (false ∷ Ξ) p
  ... | no ¬p =
      no
        (λ
  ...
  up-cast-check′ Σ uΣ Φ Ξ (id A) = yes (A , (A , wt-id (wfTySome A)))

  up-cast-check′ Σ uΣ Φ Ξ (p ； q)
      with up-cast-check′ Σ uΣ Φ Ξ p | up-cast-check′ Σ uΣ Φ Ξ q
  ... | no ¬p | _ =
      no
        (λ
          { (A , (B , wt-； hp hq)) → ¬p (_ , (_ , hp))
  ...
  down-cast-check′ :
    ...
    (p : Down) →
    Dec (Σ[ A ∈ Ty ] Σ[ B ∈ Ty ] Σ ∣ Φ ∣ Ξ ⊢ p ⦂ A ⊒ B)
  down-cast-check′ Σ uΣ Φ Ξ (untag G ℓ) with groundTyDec G
  ... | no ¬g =
      no
        (λ
          { (A , (B , wt-untag g gokΞ ℓ′)) → ¬g g
          })
  ... | yes g with groundOkDec g Ξ
  ...   | no ¬gok =
        no
          (λ
            { (A , (B , wt-untag g′ gokΞ ℓ′)) →
                ¬gok (ground-ok-cong {g = g′} {g′ = g} gokΞ)
            })
  ...   | yes gokΞ = yes (★ , (G , wt-untag g gokΞ ℓ))

  down-cast-check′ Σ uΣ Φ Ξ (seal α) with permMemberDec α Φ | lookupStoreAnyDec Σ α
  ... | no α∉Φ | _ =
      no
        (λ
  ...
  down-cast-check′ Σ uΣ Φ Ξ (p ↦ q)
      with up-cast-check′ Σ uΣ Φ Ξ p | down-cast-check′ Σ uΣ Φ Ξ q
  ... | no ¬p | _ =
      no
        (λ
          { (A , (B , wt-↦ hp hq)) → ¬p (_ , (_ , hp))
  ...
  down-cast-check′ Σ uΣ Φ Ξ (∀ᵖ p) with down-cast-check′ (⟰ᵗ Σ) (unique-⟰ᵗ uΣ) Φ Ξ p
  ... | no ¬p =
      no
        (λ
          { (A , (B , wt-∀ hp)) → ¬p (_ , (_ , hp))
  ...
  down-cast-check′ Σ uΣ Φ Ξ (ν p)
      with down-cast-check′ ((zero , ⇑ˢ ★) ∷ ⟰ˢ Σ) (unique-ν ★ uΣ) (false ∷ Φ) (true ∷ Ξ) p
  ... | no ¬p =
      no
        (λ
          { (A , (B , wt-ν hp)) → ¬p (_ , (_ , hp))
  ...
  down-cast-check′ Σ uΣ Φ Ξ (id A) = yes (A , (A , wt-id (wfTySome A)))

  down-cast-check′ Σ uΣ Φ Ξ (p ； q)
      with down-cast-check′ Σ uΣ Φ Ξ p | down-cast-check′ Σ uΣ Φ Ξ q
  ... | no ¬p | _ =
      no
        (λ
          { (A , (B , wt-； hp hq)) → ¬p (_ , (_ , hp))
...
up-cast-check :
  ...
  Uniqueˢ Σ →
  (p : Up) →
  Dec (Σ[ A ∈ Ty ] Σ[ B ∈ Ty ] Σ ∣ every Ψ ∣ every Ψ ⊢ p ⦂ A ⊑ B)
up-cast-check Σ Ψ uΣ p =
  up-cast-check′ Σ uΣ (every Ψ) (every Ψ) p
...
down-cast-check Σ Ψ uΣ p =
  down-cast-check′ Σ uΣ (every Ψ) (every Ψ) p
...
type-check Δ Ψ Σ Γ wfΓ wfΣ (M • α) with type-check Δ Ψ Σ Γ wfΓ wfΣ M
... | no ¬M =
  no
    (λ
      { ((B , ⊢• M:∀ α∈ h) , bf-• bfM) → ¬M ((`∀ _ , M:∀) , bfM)
      })
... | yes ((＇ X , M:X) , bfM) =
  no
    (λ
...
... | yes ((｀ β , M:β) , bfM) =
  no
    (λ
      { ((B , ⊢• M:∀ α∈ h) , bf-• bfM′) →
          let eq : ｀ β ≡ `∀ _
              eq = type-unique-blamefree (storeWf-unique wfΣ) bfM M:β M:∀
          in nonForall-≠∀ (nf-seal β) eq
      })
... | yes ((‵ ι , M:ι) , bfM) =
  no
    (λ
...
... | yes ((`∀ A , M:∀A) , bfM) with α <? Ψ | lookupStoreAnyDec Σ α
...   | yes α<Ψ | yes (C , hα) =
      yes ((A [ ｀ α ]ᵗ , ⊢• M:∀A (every-member α α<Ψ) hα) , bf-• bfM)
...   | no α≮Ψ | _ =
      no
        (λ
          { ((B , ⊢• M:∀ α∈ h) , bf-• bfM′) →
...
type-check Δ Ψ Σ Γ wfΓ wfΣ (ν:= A ∙ M) with wfTyDec Δ Ψ A
... | no ¬wfA =
  no
    (λ
      { ((B , ⊢ν wfA M:B) , bf-ν bfM) → ¬wfA wfA
      })
... | yes wfA
    with type-check
           Δ
           ...
           (⤊ˢ Γ)
           (ctxWf-⤊ˢ wfΓ)
           (storeWf-ν-ext wfA wfΣ)
           M
... | no ¬M =
  no
    (λ
      { ((B , ⊢ν wfA′ M:B) , bf-ν bfM) → ¬M ((⇑ˢ B , M:B) , bfM)
      })
... | yes ((C , M:C) , bfM) with unshiftSealTyDec C
...   | yes (B , C≡⇑B) =
      yes
        ( ( B
          , ⊢ν
              wfA
              (subst
                (λ T → Δ ∣ suc Ψ ∣ ((zero , ⇑ˢ A) ∷ ⟰ˢ Σ) ∣ (⤊ˢ Γ) ⊢ M ⦂ T)
                C≡⇑B
                M:C)
          )
        , bf-ν bfM
...
...   | no ¬unshift =
      no
        (λ
          { ((B , ⊢ν wfA′ M:⇑B) , bf-ν bfM′) →
              let eq : C ≡ ⇑ˢ B
                  eq =
                    type-unique-blamefree
                      (storeWf-unique (storeWf-ν-ext wfA wfΣ))
                      bfM
                      M:C
                      M:⇑B
              in
...
... | no ¬M =
  no
    (λ
      { ((B , ⊢up {A = A′} M:A′ hp) , bf-up bfM) → ¬M ((A′ , M:A′) , bfM)
      })
... | yes ((A , M:A) , bfM) with up-cast-check Σ Ψ (storeWf-unique wfΣ) p
...   | no ¬p =
      no
        (λ
          { ((B , ⊢up {A = A′} {B = B′} M:A′ hp) , bf-up bfM′) →
              ¬p (A′ , (B′ , hp))
          })
...   | yes (A′ , (B , hp)) with A′ ≟Ty A
...     | yes refl = yes ((B , ⊢up M:A hp) , bf-up bfM)
...     | no A′≢A =
        no
          (λ
            { ((C , ⊢up {A = A″} {B = C′} M:A″ hp′) , bf-up bfM′) →
                let eqCastA : A′ ≡ A″
...
... | no ¬M =
  no
    (λ
      { ((B , ⊢down {A = A′} M:A′ hp) , bf-down bfM) → ¬M ((A′ , M:A′) , bfM)
      })
... | yes ((A , M:A) , bfM) with down-cast-check Σ Ψ (storeWf-unique wfΣ) p
...   | no ¬p =
      no
        (λ
          { ((B , ⊢down {A = A′} {B = B′} M:A′ hp) , bf-down bfM′) →
              ¬p (A′ , (B′ , hp))
          })
...   | yes (A′ , (B , hp)) with A′ ≟Ty A
...     | yes refl = yes ((B , ⊢down M:A hp) , bf-down bfM)
...     | no A′≢A =
        no
          (λ
            { ((C , ⊢down {A = A″} {B = C′} M:A″ hp′) , bf-down bfM′) →
                let eqCastA : A′ ≡ A″
                    eqCastA = proj₁ (wt⊒-unique (storeWf-unique wfΣ) hp hp′)
                in
                let eqTermA : A″ ≡ A

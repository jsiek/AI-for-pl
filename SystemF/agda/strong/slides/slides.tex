...
\newcommand{\owners}[1]{\mathsf{binds}(#1)}
...
\newcommand{\dualscope}[1]{\mathsf{dualScope}(#1)}
\newcommand{\scopepart}[1]{\mathsf{scopeOf}(#1)}
...
%===============================================================================
\frame{
  \frametitle{Type-variable lookup}
...
\begin{center}
The only visible type variables are those introduced by the type context.
\end{center}
}
%===============================================================================
\frame{
  \frametitle{Frames acting on type contexts}
...
\[
\begin{aligned}
\owners{\emptyset} &= \emptyset
& \owners{\bind{X}{A},\Delta} &= A,\owners{\Delta} \\
\owners{\pm X,\Delta} &= \owners{\Delta}
\end{aligned}
\]
\[
\begin{aligned}
\intc{\Delta}{\Psi} &= \pushbinds{\owners{\Delta}}{\Delta(\Psi)} \\
\fce{\Delta}{\Psi} &= \pushbinds{\owners{\Delta}}{\unlocked{\Delta}(\Psi)}
\end{aligned}
\]
}
}
...
\hide{\bind{X}{A},\Delta} &= \lockX{X},\hide{\Delta} \\
\hide{\pm X,\Delta} &= \hide{\Delta} \\
\dualscope{\emptyset} &= \emptyset \\
\dualscope{\bind{X}{A},\Delta} &= \dualscope{\Delta} \\
\dualscope{\unlockX{X},\Delta} &= \dualscope{\Delta},\lockX{X} \\
\dualscope{\lockX{X},\Delta} &= \dualscope{\Delta},\unlockX{X} \\
\dual{\Delta} &= \hide{\Delta},\dualscope{\Delta}
\end{aligned}
\]
\end{column}
...
\[
\begin{aligned}
\scopepart{\emptyset} &= \emptyset \\
\scopepart{\bind{X}{A},\Delta} &= \scopepart{\Delta} \\
\scopepart{\pm X,\Delta} &= \pm X,\scopepart{\Delta} \\
\rewind{\Delta} &= \dualscope{\Delta},\Delta \\
\Delta_1\scopemove\Delta_2
  &= \Delta_1,\scopepart{\Delta_2}
\end{aligned}
\]
\end{column}
\end{columns}
...
\centering
\fbox{$\Psi\vdash\Delta$}

{\scriptsize
\begin{gather*}
\inference{}{\Psi\vdash\emptyset}
\\[2ex]
\inference{\unlocked{\Delta}(\Psi)\vdash A\qquad\Psi\vdash\Delta}
          {\Psi\vdash\bind{X}{A},\Delta}
\\[2ex]
\inference{\intc{\Delta}{\Psi}\ni X\qquad\Psi\vdash\Delta}
          {\Psi\vdash\lockX{X},\Delta}
\\[2ex]
\inference{\intc{\Delta}{\Psi}\ni\lockX{X}\qquad\Psi\vdash\Delta}
          {\Psi\vdash\unlockX{X},\Delta}
\end{gather*}
}
\end{column}
\end{columns}

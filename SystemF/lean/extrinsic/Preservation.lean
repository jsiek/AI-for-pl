...
def typing_renameTT :
  ...
  | _, Δ', _, _, _, ρ, hρ, .t_tapp (Γ := Γ) (M := M) (A := A) (B := B) hM hB => by
      ...
      have hTap :
          Δ' ⊢ (List.map (renameT ρ) Γ) ⊢ (Term.tapp (renameTT ρ M) (renameT ρ B))
      ...
      have hTap' :
        ...
        exact Eq.ndrec (motive := fun T =>
            Δ' ⊢ (List.map (renameT ρ) Γ) ⊢ (Term.tapp (renameTT ρ M) (renameT ρ B)) ⦂ T)
...
def typing_substTT :
  ...
  | _, Δ', _, _, _, σ, hσ, .t_tapp (Γ := Γ) (M := M) (A := A) (B := B) hM hB => by
      ...
      have hTap :
          Δ' ⊢ (List.map (substT σ) Γ) ⊢ (Term.tapp (substTT σ M) (substT σ B))
      ...
      have hTap' :
        ...
        exact Eq.ndrec (motive := fun T =>
            Δ' ⊢ (List.map (substT σ) Γ) ⊢ (Term.tapp (substTT σ M) (substT σ B)) ⦂ T)

...
| theorem (`strong-rep-nu.TypeSafety`) | statement |
|---|---|
...
* **`Eval` for v2 — DONE.**  New public `strong-rep-nu/Eval.agda`; no
...
* **Stranded v1 Agda deleted.**  Every `.agda` under `notes/old/` (the
  ...
  files) imported v1 modules — `strong-rep-nu.Context`, `strong-rep-nu.Boundary`,
  `strong-rep-nu.BReduction`, `strong-rep-nu.Weakening`, `strong-rep-nu.Unfold` — deleted at
  ...
  v1 tree, commit `c5db9f59`, `SystemF/agda/strong-rep-nu/notes/old/`), where
...
* **Deletion of four record files — RULED, DELETED.**  `proof/WallReach`,
  ...
  *delete* (2026-09-06, closed-world repo); the record lives in
  ...
  with them; later section numbers are unchanged.  `strong-rep-nu/PLAN.md` moved
...
* **`Eval` — DONE**, review round 1 above.  v1's step/trace evaluator
  with `step-sound` (`strong-rep-nu/Eval` + `EvalDec`, `8530bb7f`) went the way
  of the rest of v1; the v2 successor is `strong-rep-nu/Eval.agda`, where the

...
rename-cong {rho} {rho'} h (case_[zero⇒_|suc⇒_] l m n) =
  ...
  where
    ...
    h-ext zeroℕ = refl
    h-ext (sucℕ i) = cong sucℕ (h i)
rename-cong {rho} {rho'} h (μ A ⇒ n) = cong (μ A ⇒_) (rename-cong h-ext n)
  where
    h-ext : (i : ℕ) -> ext rho i ≡ ext rho' i
    h-ext zeroℕ = refl
    h-ext (sucℕ i) = cong sucℕ (h i)
...
subst-cong {sigma} {tau} h (case_[zero⇒_|suc⇒_] l m n) =
  ...
  where
    ...
    h-ext zeroℕ = refl
    h-ext (sucℕ i) = cong (rename sucℕ) (h i)
subst-cong {sigma} {tau} h (μ A ⇒ n) = cong (μ A ⇒_) (subst-cong h-ext n)
  where
    h-ext : (i : ℕ) -> exts sigma i ≡ exts tau i
    h-ext zeroℕ = refl
    h-ext (sucℕ i) = cong (rename sucℕ) (h i)
...
rename_rename_commute rho1 rho2 (μ A ⇒ n) =
  trans
    (cong (μ A ⇒_) (rename_rename_commute (ext rho1) (ext rho2) n))
    (cong (μ A ⇒_) (rename-cong (ext_comp rho1 rho2) n))
...
rename_subst_commute rho tau (μ A ⇒ n) =
  trans
    (cong (μ A ⇒_) (rename_subst_commute (ext rho) (exts tau) n))
    (cong (μ A ⇒_) (subst-cong (exts_ext_comp rho tau) n))
...
rename_subst rho tau (μ A ⇒ n) =
  trans
    (cong (μ A ⇒_) (rename_subst (ext rho) (exts tau) n))
    (cong (μ A ⇒_) (subst-cong (ext_exts_comp rho tau) n))
...
sub_sub sigma tau (μ A ⇒ n) =
  trans
    (cong (μ A ⇒_) (sub_sub (exts sigma) (exts tau) n))
    (cong (μ A ⇒_) (subst-cong (exts_seq sigma tau) n))
...
subst_id (case_[zero⇒_|suc⇒_] l m n) =
  ...
  where
    ...
    exts-` (sucℕ i) = refl
subst_id (μ A ⇒ n) =
  trans (cong (μ A ⇒_) (subst-cong exts-` n)) (cong (μ A ⇒_) (subst_id n))
  where
    exts-` : (i : ℕ) -> exts `_ i ≡ `_ i
    exts-` zeroℕ = refl
    exts-` (sucℕ i) = refl

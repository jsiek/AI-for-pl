...
--   * WHAT IS DELIBERATELY ONE LAYER DOWN.  `extN` is strong-rep-store.Ctx §8
-- and
--     the representation-only `renᶠᴿ`/`renᴮᴿ` are strong-rep-store.Boundary
--     §2/§3, beside the syntax they act on, because the
--     representation-renaming metatheory of strong-rep-store.Boundary §3d is
--     stated over them and cannot import this module.  Reduction is
--     strong-rep-store.Reduction; the typing transport for `renᴹᴿ` and `crossΛᴹ`
--     is strong-rep-store.proof.RepWeaken (`rep-weaken-⊢`, `cross-Λ-⊢`), not
--     here — §3's `⊢renⁿ` is the TERM-variable half only.
--   * THREE LAWS A READER MUST KNOW.  (1) Boundaries are TERM-CLOSED
--     (strong-rep-store.Terms `env`), so `renⁿ` and `substᵐ` do NOT descend into
...
-- `renᶠᴿ`/`renᴮᴿ` live one layer down — `extN` in strong-rep-store.Ctx §8 and
-- the
-- two renamings in strong-rep-store.Boundary §2/§3, beside the syntax they act
-- on —
-- because the representation-renaming metatheory of strong-rep-store.Boundary
-- §3d

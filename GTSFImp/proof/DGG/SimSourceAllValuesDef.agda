...
module proof.DGG.SimSourceAllValuesDef where

-- File Charter:
--   * States the value-level operation needed by forward simulation of a
--     source-only type application.
--   * Given a related source universal value and target value, turns one
--     source root step into the target trace and canonical world evolution.
...
SimSourceAllValuesᵀ : Set
SimSourceAllValuesᵀ = ∀ {Δᴸ Δᴿ Δᴸ′ : TyCtx}
  ...
  → Σ[ Δᴿ′ ∈ TyCtx ]
    ...
    Σ[ γ′ ∈
      ...
      ⟨ Δᴿ′ , Σᴿ′ , [] ⟩ ]
    Σ[ s ∈ applyTy χᴸ (C [ A ]ᵗ) ⊑ᵀ⟨ γ′ ⟩ applyTys χsᴿ B ]
      (V′ —↠[ χsᴿ ] N′)
      × MultiWorldEvolution
          {W = γ} {W′ = γ′} (χᴸ ∷ˢ []ˢ) χsᴿ
      × (γ′ ⊢² N ⊑ N′ ∶ s)

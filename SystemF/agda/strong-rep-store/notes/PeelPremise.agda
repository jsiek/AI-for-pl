module strong-rep-store.notes.PeelPremise where
...
-- moved into the core:
--
--   * strong-rep-store.Boundary §3b proves (Q): the conversion contexts on the
-- two
--     sides of a crossing name the same representation variables;
--   * strong-rep-store.Boundary §3c constructs the dual's conversion context;
--   * strong-rep-store.Conversion §2c re-spells a well-typed conversion and
-- exports
...
open import strong-rep-store.Types using (`ℕ; `𝔹)

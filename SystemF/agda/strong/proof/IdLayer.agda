...
--     two names (cancel-name): typing forces X ≡ numBinds Θ₁ + Y in both
--     cases, so neither rule moves an index or invents a slot, and
...
-- In any typed id-layer under an `unseal`, the inner `id (` X)`'s
-- variable IS the pushed conversion's name.  IdPush therefore moves no
-- index.
...
-- THE SAME FACT FOR CANCEL.  The mini-core's Cancel wrote one name on
-- both conversions, which presumes numBinds Θ₁ ≡ 0.  strong.Reduction's
-- CancelR carries two names; this lemma is why no premise has to relate
-- them.
...
-- The mask jam is a phantom, twice over.  (1) A conceal is INVISIBLE to
...
-- names — `value-var-visible` (strong.Terms) says a value's variable
-- type is visible on the value's bind type context, because `env`'s last
-- conjunct checks it there.  So "Θ₁ contains `lock Y` while the
-- conversion cites Y" is untypeable.

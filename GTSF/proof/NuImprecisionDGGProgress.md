...
| [`NuImprecisionRightSilentPairedCastTransportDef.agda`](NuImprecisionRightSilentPairedCastTransportDef.agda) | completed flat statement; GPT-5.5 proof in progress | Symmetric paired-cast transport for a completed right-value catch-up. The two source-silence facts are direct premises rather than another invariant record, and the conclusion is the transformed `PairedCast` evidence itself. |

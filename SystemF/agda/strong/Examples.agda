...
--     APPLIED (`F₀`, 25 steps to 42).  This is the run on which the
...
-- 25 STEPS TO 42.  History of this run: 36 steps before frame-exact
-- Beta (PR #199); 49 with it (every value Beta pushes under a Λ carries
-- a `↓Y` wrapper the later passes push through); 25 with the TyPeelR
-- split (PR #202), whose Λ clause performs the instantiation itself and
-- mints ONE binder where TyPeelR ⨟ TyBeta minted two.
_ : traceLen (eval 60 ⊢F₀) ≡ 25
_ = refl
...
-- History: 130 / 389 without the tests; 50 / 101 with them; 248 / 518
-- once frame-exact Beta added its `↓Y` layers; 64 / 130 after the
-- TyPeelR split removed one boundary per instantiation.  The residue
-- (merges of lists unlocking DIFFERENT slots) is what remains — see
-- notes/DECISIONS.md, "Change-list growth".
_ : peak maxChanges (traceTerms (eval 60 ⊢F₀)) ≡ 64
_ = refl

_ : peak sumChanges (traceTerms (eval 60 ⊢F₀)) ≡ 130
_ = refl

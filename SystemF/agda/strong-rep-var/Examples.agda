module strong-rep-var.Examples where
...
-- (strong-rep-var.Eval) calls `check⊢` on every contractum at the type the run
...
--     `strong-rep-var.Preservation.preservation` (2026-09-20) and
--     `strong-rep-var.Progress.progress` (2026-09-21) are UNCONDITIONAL, but
-- the
...
--     `strong-rep-var.proof.PreserveObstruct`, which was deleted in the module
...
open import strong-rep-var.TypeCheck using (tc; infer)
...
-- would be a soundness bug in `strong-rep-var.TypeCheck`; the run of §2b is
-- the

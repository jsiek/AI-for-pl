...
`strong-rep-store.Preservation.Stage1` is therefore GONE: `preservation` and
`preservation*` are stated outright. `strong-rep-store.TypeSafety` exports them
outright too. As of 2026-09-21 `strong-rep-store.Progress.Stage1`,
`strong-rep-store.TypeSafety.Stage1` and `proof/TypeSafety.Stage1` are GONE as well:
...
What compiles, from `SystemF/agda/strong-rep-store/`:
...
```
scripts/render_term.sh 'showRun 0 11 Q₀-⊢' 'open import strong-rep-store.Examples'
scripts/render_term.sh 'showTCtx Δ₆'       'open import strong-rep-store.Examples'
```
...
7. **DONE (2026-09-19).** `agda --safe --no-allow-unsolved-metas -v0
   All.agda` passes from `SystemF/agda/strong-rep-store/`, and so does `make check`

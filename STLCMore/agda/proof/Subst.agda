module proof.Subst where

-- File Charter:
--   * Private substitution algebra and commuting lemmas.
--   * Consumed by private proofs, especially termination.
...
rename-cong {rho} {rho'} h (m as A) = cong (_as A) (rename-cong h m)
rename-cong {rho} {rho'} h (let' m `in n) =
  cong₂ let'_`in_ (rename-cong h m) (rename-cong h-ext n)
  where
    h-ext : (i : ℕ) -> ext rho i ≡ ext rho' i
    h-ext zeroℕ = refl
    h-ext (sucℕ i) = cong sucℕ (h i)
rename-cong {rho} {rho'} h `zero = refl
rename-cong {rho} {rho'} h `unit = refl
...
rename-cong {rho} {rho'} h (case_[zero⇒_|suc⇒_] l m n) =
  ...
  where
    ...
    h-ext zeroℕ = refl
    h-ext (sucℕ i) = cong sucℕ (h i)
rename-cong {rho} {rho'} h (pair m , n) =
  cong₂ pair_,_ (rename-cong h m) (rename-cong h n)
rename-cong {rho} {rho'} h (fst m) = cong fst_ (rename-cong h m)
rename-cong {rho} {rho'} h (snd m) = cong snd_ (rename-cong h m)
rename-cong {rho} {rho'} h (inl m `to A) = cong (inl_`to A) (rename-cong h m)
rename-cong {rho} {rho'} h (inr m `to A) = cong (inr_`to A) (rename-cong h m)
rename-cong {rho} {rho'} h (case⊎_[inl⇒_|inr⇒_] l m n) =
  cong3 case⊎_[inl⇒_|inr⇒_]
    (rename-cong h l)
    (rename-cong h-ext m)
    (rename-cong h-ext n)
  where
...
subst-cong {sigma} {tau} h (m as A) = cong (_as A) (subst-cong h m)
subst-cong {sigma} {tau} h (let' m `in n) =
  cong₂ let'_`in_ (subst-cong h m) (subst-cong h-ext n)
  where
    h-ext : (i : ℕ) -> exts sigma i ≡ exts tau i
    h-ext zeroℕ = refl
    h-ext (sucℕ i) = cong (rename sucℕ) (h i)
subst-cong {sigma} {tau} h `zero = refl
subst-cong {sigma} {tau} h `unit = refl
...
subst-cong {sigma} {tau} h (case_[zero⇒_|suc⇒_] l m n) =
  ...
  where
    ...
    h-ext zeroℕ = refl
    h-ext (sucℕ i) = cong (rename sucℕ) (h i)
subst-cong {sigma} {tau} h (pair m , n) =
  cong₂ pair_,_ (subst-cong h m) (subst-cong h n)
subst-cong {sigma} {tau} h (fst m) = cong fst_ (subst-cong h m)
subst-cong {sigma} {tau} h (snd m) = cong snd_ (subst-cong h m)
subst-cong {sigma} {tau} h (inl m `to A) = cong (inl_`to A) (subst-cong h m)
subst-cong {sigma} {tau} h (inr m `to A) = cong (inr_`to A) (subst-cong h m)
subst-cong {sigma} {tau} h (case⊎_[inl⇒_|inr⇒_] l m n) =
  cong3 case⊎_[inl⇒_|inr⇒_]
    (subst-cong h l)
    (subst-cong h-ext m)
    (subst-cong h-ext n)
...
rename_rename_commute rho1 rho2 (m as A) =
  cong (_as A) (rename_rename_commute rho1 rho2 m)
rename_rename_commute rho1 rho2 (let' m `in n) =
  cong₂ let'_`in_
...
rename_rename_commute rho1 rho2 `unit = refl
...
rename_rename_commute rho1 rho2 (case_[zero⇒_|suc⇒_] l m n) =
  cong3 case_[zero⇒_|suc⇒_]
    ...
    (trans
      (rename_rename_commute (ext rho1) (ext rho2) n)
      (rename-cong (ext_comp rho1 rho2) n))
rename_rename_commute rho1 rho2 (pair m , n) =
  cong₂ pair_,_
    (rename_rename_commute rho1 rho2 m)
    (rename_rename_commute rho1 rho2 n)
rename_rename_commute rho1 rho2 (fst m) =
  cong fst_ (rename_rename_commute rho1 rho2 m)
rename_rename_commute rho1 rho2 (snd m) =
  cong snd_ (rename_rename_commute rho1 rho2 m)
rename_rename_commute rho1 rho2 (inl m `to A) =
  cong (inl_`to A) (rename_rename_commute rho1 rho2 m)
rename_rename_commute rho1 rho2 (inr m `to A) =
  cong (inr_`to A) (rename_rename_commute rho1 rho2 m)
rename_rename_commute rho1 rho2 (case⊎_[inl⇒_|inr⇒_] l m n) =
  cong3 case⊎_[inl⇒_|inr⇒_]
    (rename_rename_commute rho1 rho2 l)
    (trans
      (rename_rename_commute (ext rho1) (ext rho2) m)
      (rename-cong (ext_comp rho1 rho2) m))
    (trans
      (rename_rename_commute (ext rho1) (ext rho2) n)
      (rename-cong (ext_comp rho1 rho2) n))
...
rename_subst_commute rho tau (m as A) =
  cong (_as A) (rename_subst_commute rho tau m)
rename_subst_commute rho tau (let' m `in n) =
  cong₂ let'_`in_
...
rename_subst_commute rho tau `unit = refl
...
rename_subst_commute rho tau (case_[zero⇒_|suc⇒_] l m n) =
  cong3 case_[zero⇒_|suc⇒_]
    ...
    (trans
      (rename_subst_commute (ext rho) (exts tau) n)
      (subst-cong (exts_ext_comp rho tau) n))
rename_subst_commute rho tau (pair m , n) =
  cong₂ pair_,_
    (rename_subst_commute rho tau m)
    (rename_subst_commute rho tau n)
rename_subst_commute rho tau (fst m) =
  cong fst_ (rename_subst_commute rho tau m)
rename_subst_commute rho tau (snd m) =
  cong snd_ (rename_subst_commute rho tau m)
rename_subst_commute rho tau (inl m `to A) =
  cong (inl_`to A) (rename_subst_commute rho tau m)
rename_subst_commute rho tau (inr m `to A) =
  cong (inr_`to A) (rename_subst_commute rho tau m)
rename_subst_commute rho tau (case⊎_[inl⇒_|inr⇒_] l m n) =
  cong3 case⊎_[inl⇒_|inr⇒_]
    (rename_subst_commute rho tau l)
    (trans
      (rename_subst_commute (ext rho) (exts tau) m)
      (subst-cong (exts_ext_comp rho tau) m))
    (trans
      (rename_subst_commute (ext rho) (exts tau) n)
      (subst-cong (exts_ext_comp rho tau) n))
...
rename_subst rho tau (m as A) =
  cong (_as A) (rename_subst rho tau m)
rename_subst rho tau (let' m `in n) =
  cong₂ let'_`in_
...
rename_subst rho tau `unit = refl
...
rename_subst rho tau (case_[zero⇒_|suc⇒_] l m n) =
  cong3 case_[zero⇒_|suc⇒_]
    ...
    (trans
      (rename_subst (ext rho) (exts tau) n)
      (subst-cong (ext_exts_comp rho tau) n))
rename_subst rho tau (pair m , n) =
  cong₂ pair_,_ (rename_subst rho tau m) (rename_subst rho tau n)
rename_subst rho tau (fst m) = cong fst_ (rename_subst rho tau m)
rename_subst rho tau (snd m) = cong snd_ (rename_subst rho tau m)
rename_subst rho tau (inl m `to A) = cong (inl_`to A) (rename_subst rho tau m)
rename_subst rho tau (inr m `to A) = cong (inr_`to A) (rename_subst rho tau m)
rename_subst rho tau (case⊎_[inl⇒_|inr⇒_] l m n) =
  cong3 case⊎_[inl⇒_|inr⇒_]
    (rename_subst rho tau l)
    (trans
      (rename_subst (ext rho) (exts tau) m)
      (subst-cong (ext_exts_comp rho tau) m))
    (trans
      (rename_subst (ext rho) (exts tau) n)
      (subst-cong (ext_exts_comp rho tau) n))
...
sub_sub sigma tau (m as A) = cong (_as A) (sub_sub sigma tau m)
sub_sub sigma tau (let' m `in n) =
  cong₂ let'_`in_
    (sub_sub sigma tau m)
    (trans
...
sub_sub sigma tau (case_[zero⇒_|suc⇒_] l m n) =
  cong3 case_[zero⇒_|suc⇒_]
    ...
    (trans
      (sub_sub (exts sigma) (exts tau) n)
      (subst-cong (exts_seq sigma tau) n))
sub_sub sigma tau (pair m , n) =
  cong₂ pair_,_ (sub_sub sigma tau m) (sub_sub sigma tau n)
sub_sub sigma tau (fst m) = cong fst_ (sub_sub sigma tau m)
sub_sub sigma tau (snd m) = cong snd_ (sub_sub sigma tau m)
sub_sub sigma tau (inl m `to A) = cong (inl_`to A) (sub_sub sigma tau m)
sub_sub sigma tau (inr m `to A) = cong (inr_`to A) (sub_sub sigma tau m)
sub_sub sigma tau (case⊎_[inl⇒_|inr⇒_] l m n) =
  cong3 case⊎_[inl⇒_|inr⇒_]
    (sub_sub sigma tau l)
    (trans
      (sub_sub (exts sigma) (exts tau) m)
      (subst-cong (exts_seq sigma tau) m))
    (trans
      (sub_sub (exts sigma) (exts tau) n)
      (subst-cong (exts_seq sigma tau) n))
...
subst_id (m as A) = cong (_as A) (subst_id m)
subst_id (let' m `in n) =
  cong₂ let'_`in_
    (subst_id m)
...
subst_id (case_[zero⇒_|suc⇒_] l m n) =
  ...
  where
    ...
    exts-` zeroℕ = refl
    exts-` (sucℕ i) = refl
subst_id (pair m , n) = cong₂ pair_,_ (subst_id m) (subst_id n)
subst_id (fst m) = cong fst_ (subst_id m)
subst_id (snd m) = cong snd_ (subst_id m)
subst_id (inl m `to A) = cong (inl_`to A) (subst_id m)
subst_id (inr m `to A) = cong (inr_`to A) (subst_id m)
subst_id (case⊎_[inl⇒_|inr⇒_] l m n) =
  cong3 case⊎_[inl⇒_|inr⇒_]
    (subst_id l)
    (trans (subst-cong exts-` m) (subst_id m))
    (trans (subst-cong exts-` n) (subst_id n))
  where
    exts-` : (i : ℕ) -> exts `_ i ≡ `_ i
    exts-` zeroℕ = refl

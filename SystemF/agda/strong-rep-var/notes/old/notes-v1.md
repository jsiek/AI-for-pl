...
subject reduction is false for it.  The live design is `strong-rep-var/Design.md`.
The v1 Agda probes that accompanied these notes live on `main` (commit
c5db9f59, `SystemF/agda/strong-rep-var/notes/old/`); they import v1 modules
(`strong-rep-var.Context`, `strong-rep-var.Boundary`, `strong-rep-var.BReduction`, …) and were
...
# The interior context     Γ ⇈ Θ
  ...
  **What is deliberately NOT here** (flagged in strong-rep-var/Boundary.agda).  The probes had a
  ...
  **A ≐ˢᵏ A′ — SKELETON EQUALITY**: the two types have the same CONSTRUCTOR TREE, with
  ...
  BOTH premises are LOAD-BEARING, and each catches what the other misses.
    ...
    * Claims-nothing: dropping it admits an adversary that types `7 : ℕ` at an abstract Z, by
      ...
      Machine-checked in strong-rep-var/Boundary.agda (¬starOnly-adv, ¬⊢adv, adv-rep-skel) and in
    ...
    * Skeleton equality: dropping it admits a CLOSED rep, since "A names only rep-less
      ...
      strong-rep-var/Boundary.agda (starOnly-ground, ¬skel-ground, ¬⊢gnd, ¬⊢Tg, ¬⊢Tbad) and in
...
# Correspondence with the Agda   (SystemF/agda/strong-rep-var/)
...
# De Bruijn formalization and the tightened conceal marker  (what we learned)
  ...
  We mechanized the old calculus in Agda under SystemF/agda/strong-rep-var/ using de Bruijn indices:

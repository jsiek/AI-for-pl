...
- In a `WorldInvariants`-valid world, read the invariants directly from the
  grid: right-only cells show `★`; no cell has source entry `★` at mark
  `X⊑★` with a right occupant; and same-pivot scope edges into one world share
  their origin.
- Fixture/legacy worlds outside the `WorldInvariants` companion may violate
  these invariants — the grid makes such violations visible, which is its
  purpose.

...
cast-left {V} {c = c ⨟ ((★ ⇒ ★) !)} V≤M′ vV (⊢⨟ c⦂ (⊢! x)) (⊑drop! c≤id)
    with ⊑ᶜ→⊑ ⊢idᶜ c⦂ c≤id
... | aa , ⊑-⇒ bb bb₁ =
  cast cast V [ c ] [ (★ ⇒ ★) ! ]
  , (cast V [ c ⨟ ((★ ⇒ ★) !) ] —→ᶜ⟨ β-seq vV ⟩
...
cast-left-val {V} {c = c} V≤V′ vV vV′ ⊢idᶜ (⊑idᶜ A⊑A′) =
  V , (cast V [ c ] —→ᶜ⟨ β-id vV ⟩ V ∎ᶜ) , V≤V′ , vV
cast-left-val {V} {c = c} V≤V′ vV vV′ ⊢idᶜ (⊑idL nseq c⦂ A⊑B A⊑C) =
  V , (cast V [ c ] —→ᶜ⟨ β-id vV ⟩ V ∎ᶜ) , V≤V′ , vV
cast-left-val {V} {c = c} V≤V′ vV vV′ ⊢idᶜ (⊑idR nseq ⊢idᶜ A⊑A′ B⊑A′) =
  V , (cast V [ c ] —→ᶜ⟨ β-id vV ⟩ V ∎ᶜ) , V≤V′ , vV
cast-left-val {V} {V′} {c = c} V≤V′ vV vV′ (⊢! g) (⊑idR nseq (⊢! g′) A⊑A′ B⊑A′) =
  inj V [ _ ]! , (cast V [ c ] —→ᶜ⟨ β-inj ⟩ inj V [ _ ]! ∎ᶜ)
  , ⊑injL V≤V′ vV g vV′
  , V-! vV
cast-left-val {V} {V′} {A′ = A′} {c = c} V≤V′ vV vV′ (⊢? G-ℕ) (⊑idR nseq (⊢? g′) A⊑A′ B⊑A′)
...
cast-left-val {V} {V′} {A′ = A′} {c = c} V≤V′ vV vV′ (⊢? G-⇒) (⊑idR nseq (⊢? g′) A⊑A′ B⊑A′)
  with canonical-★-inj vV (⊑ᶜᵀ-left-typed V≤V′)
... | H , W , vW , refl
  with B⊑A′
... | ⊑-⇒ A₁⊑A₂ B₁⊑B₂
...
... | refl =
  W , (cast (inj W [ _ ]!) [ c ] —→ᶜ⟨ β-proj-inj-ok vW ⟩ W ∎ᶜ) , W⊑V′ , vW
cast-left-val {V} {V′} {c = c} V≤V′ vV vV′ (⊢↦ cwt₁ dwt₁) (⊑idR↦ c≤id d≤id) =
  cast V [ c ] , (cast V [ c ] ∎ᶜ)
  , ⊑castL V≤V′ (⊢↦ cwt₁ dwt₁) (⊑idR↦ c≤id d≤id)
  , V-cast↦ vV
cast-left-val {V} {V′} {c = c ⨟ d} V≤V′ vV vV′ (⊢⨟ c⦂ d⦂) (⊑idR⨟ c≤id d≤id)
  with cast-left-val V≤V′ vV vV′ c⦂ c≤id
... | V₁ , V-c—↠V₁ , V₁⊑V′ , vV₁
  with cast-left-val V₁⊑V′ vV₁ vV′ d⦂ d≤id
... | V₂ , V₁-d—↠V₂ , V₂⊑V′ , vV₂ =
  V₂
  , (cast V [ c ⨟ d ]
       —→ᶜ⟨ β-seq vV ⟩
  ...
  , V₂⊑V′
  , vV₂
cast-left-val {V} {V′} {A′ = A′} {c = (★ ⇒ ★) `? ⨟ c} V≤V′ vV vV′ (⊢⨟ (⊢? G-⇒) c⦂) (⊑drop? c≤id)
  with canonical-★-inj vV (⊑ᶜᵀ-left-typed V≤V′)
... | H , W , vW , refl
  with ⊑ᶜ→⊑ ⊢idᶜ c⦂ c≤id
... | A⊑A′ , _
...
... | g″ , W⊑V′
  with ground-upper-unique g″ G-⇒ (⊑ᶜᵀ-type-precision W⊑V′) (⊑-⇒ A₁⊑A₂ B₁⊑B₂)
... | refl
  with cast-left-val W⊑V′ vW vV′ c⦂ c≤id
... | V₂ , W-c—↠V₂ , V₂⊑V′ , vV₂ =
  V₂
  , (cast V [ (★ ⇒ ★) `? ⨟ c ]
       —→ᶜ⟨ β-seq vV ⟩
  ...
  , V₂⊑V′
  , vV₂
cast-left-val {V} {V′} {c = c ⨟ (★ ⇒ ★) !} V≤V′ vV vV′ (⊢⨟ c⦂ (⊢! G-⇒)) (⊑drop! c≤id)
  with cast-left-val V≤V′ vV vV′ c⦂ c≤id
... | V₁ , V-c—↠V₁ , V₁⊑V′ , vV₁ =
  inj V₁ [ ★ ⇒ ★ ]!
  , (cast V [ c ⨟ (★ ⇒ ★) ! ]
...
catchup vV′ (⊑castL M⊑M′ cwt c⊑id)
    with catchup vV′ M⊑M′
... | V , vV , M—↠V , V⊑V′
    with cast-left-val V⊑V′ vV vV′ cwt c⊑id
... | V₂ , castV—↠V₂ , V₂⊑V′ , vV₂ =
      V₂ , vV₂
      , (cast _ [ _ ]
           —↠ᶜ⟨ ξ* (cast□[ _ ]) M—↠V ⟩
...
cast-⊑⨟ {V} V≤V′ vV vV′ (⊑drop? {c′ = d} d≤c′⨟d′) (⊢⨟ (⊢? x) d⦂₁) c′⦂ d′⦂
    with ⊑ᶜ→⊑ (⊢⨟ c′⦂ d′⦂) d⦂₁ d≤c′⨟d′
... | aa , bb
    with cast-left-val V≤V′ vV vV′ (⊢? x) (⊑idR atom-? (⊢? x) ⊑-★ aa)
... | V₂ , →V₂ , V₂≤ , vV₂
...
... | N₂ , →N₂ , N₂≤ =
     let red = cast V [ ((★ ⇒ ★) `?) ⨟ d ] —→ᶜ⟨ β-seq vV ⟩
               ...
               N₂ ∎ᶜ in
     N₂ , red , N₂≤
cast-⊑⨟ {V} V≤V′ vV vV′ (⊑drop!{c′ ⨟ d′}{d} d≤c′⨟d′) (⊢⨟ d⦂ (⊢! x)) c′⦂ d′⦂ 
    with ⊑ᶜ→⊑ (⊢⨟ c′⦂ d′⦂) d⦂ d≤c′⨟d′
... | aa , ⊑-⇒ bb bb₁
    with cast-⊑⨟ V≤V′ vV vV′ d≤c′⨟d′ d⦂ c′⦂ d′⦂
... | N₂ , →N₂ , N₂≤ =
    let red = cast V [ d ⨟ (★ ⇒ ★) ! ] —→ᶜ⟨ β-seq vV ⟩
...
... | ⊑-⇒ A₁⊑A′ B₁⊑B′ | ⊑-⇒ A⊑A′ B⊑B′
    with c⊑id
... | ⊑idR↦ c₁⊑id d₁⊑id
    with cast-left-val W⊑W′ vW vW′ cwt₁ c₁⊑id
... | W₁ , W-c₁—↠W₁ , W₁⊑W′ , vW₁

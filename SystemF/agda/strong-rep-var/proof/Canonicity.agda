module strong-rep-var.proof.Canonicity where
...
-- went with the index (Jeremy's ruling, strong-rep-var.Conversion): a mixed
-- tree
...
-- spelling `s′` with a `SameConv` relating the two (strong-rep-var.Reduction).
...
-- dropped while `strong-rep-var.Examples` is unported on this branch; the
...
-- untouched (strong-rep-var.TermSubst).  So no conversion is ever renamed by
-- term
...
-- (strong-rep-var.proof.Preserve, `preserve-TyPeelR-Λ`; the tree was
-- untypeable

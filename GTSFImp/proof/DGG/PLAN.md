...
| M6 | Value catch-up driver: ties the M4↔M5 MUTUAL knot (M5 continuations call back into ExtraCastRight²) by well-founded recursion on the target cast-column length; both Proofs stay higher-order over each other's Defs | M | **workers landed**: castSize support, provenance-carrying columns, tail transport, the fuel-aware `extra-cast-right-at` dispatcher, and `value-catchup-right-prov-at` column recursion are proven. The non-well-founded `next-knot` field is removed; M5-to-`InstCatchupRightAt` wiring and the final `Acc _<_` knot remain | — |

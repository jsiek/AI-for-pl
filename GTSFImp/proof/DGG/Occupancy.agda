...
occupied? : ∀ {Γᴸ Γᴿ : Ctx}
  ...
  → Dec (Σ[ Y ∈ TyVar (Δᵉ Γᴿ) ]
      toRenameᵗ (ηᴿᶜ γ) Y ≡ Z)
occupied? γ Z = fin-image? (toRenameᵗ (ηᴿᶜ γ)) Z
...
occupied-at-source? γ X = occupied? γ (toRenameᵗ (ηᴸᶜ γ) X)


no-target-at-source? : ∀ {Γᴸ Γᴿ : Ctx}
  → (γ : Γᴸ ⊑ᶜ Γᴿ)
  → (X : TyVar (Δᵉ Γᴸ))
  → Dec (∀ Y → toRenameᵗ (ηᴿᶜ γ) Y ≢ toRenameᵗ (ηᴸᶜ γ) X)
...
bindLeft-fresh-no-target : ∀ {Γᴸ Γᴿ : Ctx} {γ : Γᴸ ⊑ᶜ Γᴿ}
    (A : Ty (Δᵉ Γᴸ))
  → ∀ Y → toRenameᵗ (ηᴿᶜ (bindLeftᶜ γ A)) Y
      ≢ toRenameᵗ (ηᴸᶜ (bindLeftᶜ γ A)) Fin.zero
...
liftBoth-fresh-occupied : ∀ {Γᴸ Γᴿ : Ctx} {γ : Γᴸ ⊑ᶜ Γᴿ}
  ...
  → Σ[ Y ∈ TyVar (Nat.suc (Δᵉ Γᴿ)) ]
      toRenameᵗ (ηᴿᶜ (liftBothᶜ v γ)) Y ≡ Fin.zero
...
bindRight-fresh-occupied : ∀ {Γᴸ Γᴿ : Ctx} {γ : Γᴸ ⊑ᶜ Γᴿ}
  ...
  → Σ[ Y ∈ TyVar (Nat.suc (Δᵉ Γᴿ)) ]
      toRenameᵗ (ηᴿᶜ (bindRightᶜ γ B fresh)) Y ≡ Fin.zero
...
bindBoth-fresh-occupied : ∀ {Γᴸ Γᴿ : Ctx} {γ : Γᴸ ⊑ᶜ Γᴿ}
    ...
    (represented : A ⊑ᵀ⟨ γ ⟩ B)
  → Σ[ Y ∈ TyVar (Nat.suc (Δᵉ Γᴿ)) ]
      toRenameᵗ (ηᴿᶜ (bindBothᶜ γ represented)) Y ≡ Fin.zero

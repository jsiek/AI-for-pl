...
open import Compile using (compile; compile-preserves; compile-preserves-precision)
...
cast-!-not-value : ∀ {V G} → Valueᶜ (cast V [ G ! ]) → ⊥
cast-!-not-value ()
...
sim-back
  (⊑cast {M = V} {M′ = M′} {c = G !} {c′ = c′} V⊑M′ c≤c′ (⊢! g) c′⦂)
  (β-inj vV)
    with value-right-catchup vV V⊑M′
... | W′ , M′—↠W′ , inj₂ refl =
  blame
  , (cast M′ [ c′ ]
     ...
     ∎ᶜ)
  , ⊑blameR
      (⊢! (⊑ᶜᵀ-left-typed V⊑M′) g vV)
      (⊑ᶜᵀ-type-precision (⊑cast V⊑M′ c≤c′ (⊢! g) c′⦂))
... | W′ , M′—↠W′ , inj₁ (vW′ , V⊑W′)
    with cast-value-catchup vW′ (⊑ᶜᵀ-right-typed V⊑W′) c′⦂
... | N′ , castW′-c′—↠N′ , (inj₂ refl , N′⦂) =
  blame
  , (cast M′ [ c′ ]
     ...
     ∎ᶜ)
  , ⊑blameR
      (⊢! (⊑ᶜᵀ-left-typed V⊑W′) g vV)
      (⊑ᶜᵀ-type-precision (⊑cast V⊑W′ c≤c′ (⊢! g) c′⦂))
... | N′ , castW′-c′—↠N′ , (inj₁ vN′ , N′⦂)
    with sim* (⊑cast V⊑W′ c≤c′ (⊢! g) c′⦂) castW′-c′—↠N′
... | N₁ , castV!—↠N₁ , N₁⊑N′
    with catchup vN′ N₁⊑N′
... | V₂ , vV₂ , N₁—↠V₂ , V₂⊑N′
    with castV!—↠N₁ ++ᶜ N₁—↠V₂
... | cast V [ G ! ] ∎ᶜ = ⊥-elim (cast-!-not-value vV₂)
... | cast V [ G ! ] —→ᶜ⟨ castV!→N₂ ⟩ N₂—↠V₂
    rewrite cast-!-step-inv vV castV!→N₂
    with value-—↠ᶜ-refl (V-! vV) N₂—↠V₂
... | refl =
  N′
  , (cast M′ [ c′ ]
       —↠ᶜ⟨ ξ* (cast□[ c′ ]) M′—↠W′ ⟩
...
sim-back (⊑castL {M = inj V [ ℕ ]!} {M′ = M′} {c = ℕ `?} injV⊑M′ (⊢? G-ℕ) c≤id)
...
sim-back
  (⊑castL {M = inj V [ ★ ⇒ ★ ]!} {M′ = M′} {c = (★ ⇒ ★) `?} injV⊑M′ (⊢? G-⇒) c≤id)
...
... | cast (inj V [ G ]!) [ G `? ] ∎ᶜ = ⊥-elim (cast-proj-not-value vV₂)
... | cast (inj V [ G ]!) [ G `? ] —→ᶜ⟨ castInj→N₂ ⟩ N₂—↠V₂
...
sim-back (⊑castL {M = inj V [ G ]!} {M′ = M′} {c = ℕ `?} injV⊑M′ (⊢? G-ℕ) c≤id)
...
sim-back
  (⊑castL {M = inj V [ G ]!} {M′ = M′} {c = (★ ⇒ ★) `?} injV⊑M′ (⊢? G-⇒) c≤id)
...
... | V₂ , vV₂ , N₁—↠V₂ , V₂⊑N′
    with castInj—↠N₁ ++ᶜ N₁—↠V₂
... | cast (inj V [ G ]!) [ H `? ] ∎ᶜ = ⊥-elim (cast-proj-not-value vV₂)
... | cast (inj V [ G ]!) [ H `? ] —→ᶜ⟨ castInj→N₂ ⟩ N₂—↠V₂
...
sim-back (⊑inj M⊑M′ vM vM′ g) N→N =
  ⊥-elim (value-irreducible (V-! vM) N→N)
sim-back (⊑injL M⊑M′ vM g vM′) N→N =
  ⊥-elim (value-irreducible (V-! vM) N→N)
...
compile-precision {A = A} {A′ = A′} M≤M′ M⦂A M′⦂A′ =
  subst
    ...
    (subst
      (λ T₂ → []⊑[] ⊢ compile (⊑ᵀ-left-typed d) ⦂ A ⊑ᶜᵀ compile T₂ ⦂ A′)
...
cast-right-⊑
  ...
  → []⊑[] ⊢ V ⦂ A ⊑ᶜᵀ V′ ⦂ B
  → []⊑[] ⊢ V ⦂ A ⊑ᶜᵀ V′ ⦂ C
cast-right-⊑ {V = V} {V′ = V′} {A = A} B≡C V⊑V′ =
...
dynamic-gradual-guarantee : ∀{M M′}{A A′}
  ...
  → [] ⊢ M ⦂ A
  → [] ⊢ M′ ⦂ A′
  → (∀ V′ → Valueᶜ V′ → M′ ⇓ V′ → ∃[ V ] Valueᶜ V × M ⇓ V × []⊑[] ⊢ V ⦂ A ⊑ᶜᵀ V′ ⦂ A′) × (Diverges M′ → Diverges M)
...
dynamic-gradual-guarantee {M = M} {M′ = M′} {A = A} {A′ = A′} M≤M′ M⦂A M′⦂A′ =
  ...
  forward V′ vV′ (A″ , M′⦂A″ , M′—↠V′ , r-val .V′ vV′′)
      with gg
        (compile-precision M≤M′ M⦂A M′⦂A″)
        M′—↠V′
        vV′′
  ... | V , vV , M—↠V , V⊑V′ =
      V
      , (vV
      , (A , M⦂A , M—↠V , r-val _ vV)
      , cast-right-⊑ (typing-unique M′⦂A″ M′⦂A′) V⊑V′)

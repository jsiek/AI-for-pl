...
-- conversion at a looked-up rep carries the owner-lookup premise, and
...
-- boundary rules mint at a fresh owner; they are DERIVED FROM THE TYPE,
...
-- an OWNER at slot 0 — the slot the conversion's `` `∀ `` had left
-- ABSTRACT.  Every leaf of the conversion that reads that slot is an
-- identity (`id (` 0)`, because an abstract slot has no owner to seal or
...
data _⊢_-→_ : Ctxᵗ → Term → Term → Set where
  ...
  -- TYPEEL — the ∀-conversion analogue; the new owner is prepended and the
  ...
  -- and is now the OWNER this rule binds, so every leaf of `s` that
  ...
  -- CANCEL — a conceal directly under the owner it names.  The
  ...
  -- at a looked-up rep, so the rule carries the owner lookup; determinism
  -- for it is `∋:=-det`.
  --

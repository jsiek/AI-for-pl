...
**OPEN — THIS IS A PROPOSAL, NOT A DECISION.**

v3's grammar is LAYERED, inside out: simple, conceal, conversions,
positives.
...
EVERY ADMINISTRATIVE RULE IS GENERATED BY ONE OUT-OF-ORDER PAIR of that
layering:
...
so PushIntro is not an independent rule — it is the price of putting
positives outside conceals.

WHY THE QUESTION CHANGES IN v4.  There are no masks, so
`ν new β [ ν -Z [ M ] ]` and `ν -Z [ ν new β [ M ] ]` express the same
thing: Y present, Z absent.  The order is a NORMALIZATION choice, not an
expressiveness one.  (And note `Δ ∖ X` deletes ONE slot — it is not `Γ↓X`,
which drops a suffix.  v4 does not resurrect the prefix design; it removes
the thing the prefix design was competing with.)

WHERE THE OUT-OF-ORDER SHAPES COME FROM.  `crossΛ` and `AppBnd`-at-an-
intro wrap an ARBITRARY value in a conceal; v4's TyConceal now
deliberately produces the same shape.  Under v3's layering each of those
is an administrative redex — which is why, in v3, "substituting a value
into a value yields a value" is FALSE (notes/AppBndExample.agda §5).

PROPOSAL: DROP THE LAYERING.

  V⁺ ::= λx:A. N | ΛX.V | ν b [ V⁺ ] | V⁺⟨cⁱ⟩
  V  ::= k | V⁺

Any value under any boundary; any value under any inert conversion.  ONE
stratification survives — a constant under a boundary must not be a value,
or it overlaps DropConst.

  KILLS   PushConv, PushIntro, Commute, MergeConceal, DropReveal,
          DropConceal — six rules, all pure bookkeeping.
  COSTS   canonical forms stop being UNIQUE.
  BUT     Progress needs COVERAGE, not uniqueness, and coverage holds: at
          a function type a value is a λ, a boundary, or a conversion
          (Beta / AppBnd / AppConv); at a ∀ type a Λ, a boundary, or a
          conversion (TyBeta / TyPos-TyConceal / TyConv).  v3 already
          generalized AppBnd to any value under a positive boundary for
          exactly this reason.
  AND     `crossΛ` then always yields a value, so the v3 caution above
          disappears instead of having to be worked around in Progress.

  STILL TO CHECK  determinism.  v3 used the layering to keep the
  administrative rules from overlapping each other; with them gone the
  overlap cannot arise, but the remaining rules have not been checked
  pairwise.
...
# Reduction Rules
  ...
  UNDER THE §Values PROPOSAL there are no others: PushConv, PushIntro,
  Commute, MergeConceal, DropReveal and DropConceal all exist only to
  normalize a LAYERED value grammar, and the proposal drops the layering.
  If the layering is kept instead, all four Push/Commute/Merge rules come
  back, PushIntro among them — and TyConceal's placement of the intro
  INSIDE the conceal is undone on the very next step.

  (ξ)          as v3, with `ᵇ[M]` stepping at the interior context.

WHAT HAPPENED TO v3'S SET RULES.  `Commute` and `MergeConceal` existed
because a tag carried a SET and two adjacent tags had to be reconciled.
With one slot per tag they are replaced by `CancelBnd` — an adjacent
matched pair annihilates, by ANCHOR IDENTITY — and by nothing else.
`DropReveal`/`DropConceal` (the empty-set tags) have no analogue.

**OPEN — PushIntro.**  In v3 this is the second rule that forces an intro
to sit visible beneath a conceal.  In v4 the intro's rep is anchor-closed,
so the rule MAY be unnecessary: the intro no longer needs to escape the
conceal to keep its representation readable.  If it is unnecessary, then
BOTH of v3's non-prefix shapes are gone and the `Γ↓X` question changes
character entirely.  This is the first thing to check.
...
CHECKS, IN THE ORDER I WOULD DO THEM:
 ...
 1. RULE ON §Values.  Everything else depends on it: PushIntro, Commute,
    MergeConceal and the two Drops live or die with the layering, and if
    the layering is kept then TyConceal's placement is undone on the next
    step and v4 loses its motivating case.
...
`Commute`, `MergeConceal`, `DropReveal`, `DropConceal`, and the
non-empty-set side conditions.

...
... | c′ , c₂⨟c₃↠c′ , c′≤d
  with simᶜ-β-idL-⨟ c₁≤id c′≤d c₁wt (preserve-—↠ (⊢⨟ c₂wt c₃wt) c₂⨟c₃↠c′) dwt
... | c″ , c₁⨟c′↠c″ , c″≤d =
  c″
  , (((((c₁ ⨟ c₂) ⨟ c₃)
    —→⟨ β-assocRᶜ ⟩
...
sim*ᶜ : ∀ {c d d′ A B A′ B′}
  ...
  → d —↠ d′
  → ∃[ c′ ] ((c —↠ c′) × (c′ ⊑ᶜ d′))
sim*ᶜ {c = c} c≤d cwt dwt (d ∎) =
  c , ((c ∎) , c≤d)
sim*ᶜ c≤d cwt dwt (d —→⟨ d→d₁ ⟩ d₁↠d′)
  with simᶜ c≤d cwt dwt d→d₁
... | c₁ , c↠c₁ , c₁≤d₁
  with sim*ᶜ c₁≤d₁ (preserve-—↠ cwt c↠c₁) (preserve-—→ dwt d→d₁) d₁↠d′
... | c′ , c₁↠c′ , c′≤d′ =

...
| [`NuImprecisionWorldCoherentRightTargetPendingSequenceContinuationDef.agda`](NuImprecisionWorldCoherentRightTargetPendingSequenceContinuationDef.agda) | completed strict statement; proof not started | Three reachable smaller-rank continuation families for narrowing, widening, and identity-only widening sequences. Each consumes hereditary subplans and returns the existing complete right-catchup result rooted at the nested casts; reveal and conceal have no sequence constructor. This is a recursion capability, not a result carrier. |
| [`NuImprecisionWorldCoherentRightTargetActiveRootResumeDef.agda`](NuImprecisionWorldCoherentRightTargetActiveRootResumeDef.agda) | completed strict statement and fit audit; five of ten roots proved | Ten provenance-preserving active-root continuations: five `β-id` entries, narrowing untag, general and identity-only instantiation, general unseal, and reveal-unseal. An independent constructor audit confirmed these are exactly the reachable active roots and that the indices/configurations fit the parent terminalization theorem. Every field consumes a completed inner catch-up and returns the same existing complete carrier. Identity-only instantiation remains the hard leaf because the current casted target-allocation entry requires `CastMode`, which `id-onlyᵈ` does not provide. |

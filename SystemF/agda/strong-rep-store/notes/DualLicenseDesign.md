...
New CONTEXT entry form (TyEntry, strong-rep-store/Context.agda):
...
The interior computation  Γ ⇈ Θ  (strong-rep-store/Boundary.agda; used by (env) in the
...
Boundary well-formedness  Γ ∣ Ψ ⊢ Θ  (strong-rep-store/Boundary.agda) — one new
...
The dual construction (entᴳ/cncOfRevs in strong-rep-store/BReduction.agda, feeding the

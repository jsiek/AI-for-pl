...
| `ColorPreservation.agda` | the theorem `color-preservation : ColorPreservation` — at a residual position the scope map is the old one under ρ: `names Δ₂ ≡ map ρ (names Δ₁)`, with a `WfCtx Δ` premise (spent re-typing the run's middle terms by `preservation`) and the premise-free closed form `color-preservation-closed` at `empty`.  Proof in `proof/ColorPreservation.agda`; the concrete instance is `notes/ColorPreservationProbe.agda` |
...
| `ColorPreservation.agda` | **the color-preservation proof**: `⊢C-ren` transports a frame derivation along a representation-only renaming (`interior-ren`/`RepWk` at boundary frames), `⊢C-len` transports it across the `abstR → bindR R` slot refinement, `residual-frame` constructs the target frame per step (minted frames read by `instantiate-interior`/`dual-interior`/`rewind-interior`/`merged-interior`/`addLock0-interior-ren`/`crossΛ-interior`), and `residuals-color` composes along `ρ′ ∘ ρ`, re-typing by `preservation` |

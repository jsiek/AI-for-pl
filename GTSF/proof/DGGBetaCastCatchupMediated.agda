module proof.DGGBetaCastCatchupMediated where

-- File Charter:
--   * Catchup packaging for mediated-store DGG target
--     beta-through-function-cast steps.
--   * Catches source application subterms up to values and delegates the
--     value/wrap core to `DGGBetaCastValueMediated`.
--   * Split out of `DGGBetaCastMediated` so the public dispatcher stays small.
...
------------------------------------------------------------------------
-- Application beta-cast catchup packaging
------------------------------------------------------------------------
...
mediated-dgg-beta-cast-left-first {ΔL = ΔL} {ΔR = ΔR}
  ...
  let
    χsL , WL , ΔL₁ ,
      ...
      catchup-lemmaᵐ okL (vV′ ⟨ c ↦ d ⟩) L⊒V′cd
    ...
    χsR , WR , ΔL₂ ,
      ...
      catchup-lemmaᵐ (applyTerms-preserves-RuntimeOK χsL okR) vW′ R⊒W′L
  ...
  in
  (χsL ++ χsR) ++ χsT , N , ΔL′ , ρ′ , C , D , r ,
  source-steps , ΔL′≡ , ρ′≡ , C≡ , D≡ᵀ , N⊒target
...
mediated-dgg-beta-cast-right-first {ΔL = ΔL} {ΔR = ΔR}
  ...
  in
  χsR ++ χsT , N , ΔL′ , ρ′ , C , D , r , source-steps ,
  ΔL′≡ , ρ′≡ , C≡ , D≡ᵀ , N⊒target

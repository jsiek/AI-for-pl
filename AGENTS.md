...
- Read the world invariants directly from the grid: a right-only store chain
  stays within right-only cells and ends at `★`; a cell with source entry `★`,
  mark `X⊑★`, and a right occupant is forbidden; and same-pivot scope edges
  entering one world must have the same origin.

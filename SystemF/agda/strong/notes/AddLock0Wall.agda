...
--     the spelling with `SameConv`.  `strong.Reduction` carries the
--     repaired rule; `strong.proof.Progress` constructs its premises
--     outright (`addLock0-reading`); and the RESHAPED transport is PROVED
--     (`strong.proof.AddLock0`), which makes `strong.Preservation`
...
--
-- WHAT THE RULE SAID, BEFORE (strong.Reduction, until 2026-09-20).
...
-- of the same one", strong.Reduction).
--
...
-- `AddLock0Typing°` is `strong.proof.Preserve.AddLock0Typing` AS IT STOOD
...
-- `` `∀ (renᶜ (extᵗ suc) s) ``.
--
-- The live `strong.proof.Preserve.AddLock0Typing` was RESHAPED with the
...
-- here — and it is PROVED, `strong.proof.AddLock0.addLock0-⊢`, so nothing
...
-- `strong.Preservation.Preservation` — which is now an UNCONDITIONAL

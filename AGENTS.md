...
- Name all type variables, including type binders, `X`, `Y`, `Z` in order,
  repeating them with subscript groups (`X₁`, ..., `Z₂`, ...) beyond three.
  Prime target names after any subscript (`X′`, `X₁′`). Never use Greek
  letters, `♭`-prefixed names, `A`, or regular-size numeric suffixes for
  generated type-variable names.
...
- Name all type variables, including type binders, `X`, `Y`, `Z` in order,
  repeating them with subscript groups beyond three; prime target names after
  the subscript. Name term variables and binders `x`, `y`, `z` with the same
  subscript convention. Never generate Greek letters, `♯`/`♭` names, `A` as a
  type-variable name, or regular-size numeric suffixes.

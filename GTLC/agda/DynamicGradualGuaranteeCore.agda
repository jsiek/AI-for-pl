...
sim {A = A} {A′ = A′}
    (⊑cast {M = M} {M′ = M′} {c = c} M⊑M′ c≤c′ c⦂ (⊢? gA′))
    (β-proj-inj-ok {V = V′} vV′)
    with catchup (V-cast! vV′) M⊑M′
... | V , vV , M→V , V≤M′
    with cast-left-?-val V≤M′ vV vV′ c≤c′ c⦂
... | N , Vc→N , N≤ =
    let prec : []⊑[] ⊢ cast M [ c ] ⦂ A ⊑ᶜᵀ cast M′ [ A′ `? ] ⦂ A′
        prec = ⊑cast M⊑M′ c≤c′ c⦂ (⊢? gA′) in
...
sim {A = A} {A′ = A′}
    (⊑castR {M = M} {M′ = cast V′ [ A′ ! ]} M⊑M′ (⊢? gA′) id≤c′)
...
... | .(cast V [ idᶜ ★ ]) , (cast V [ idᶜ ★ ] ∎ᶜ) , N≤ =
    let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ cast (cast V′ [ A′ ! ]) [ A′ `? ] ⦂ A′
        prec = ⊑castR M⊑M′ (⊢? gA′) id≤c′ in
    let red = M→V in
    let prec′ : []⊑[] ⊢ V ⦂ A ⊑ᶜᵀ V′ ⦂ A′
...
sim {M = M} {M′ = M′} {A = A} {A′ = A′} M⊑M′ (β-proj-inj-bad x x₁) =
  let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ M′ ⦂ A′
      prec = M⊑M′ in
  let red = M ∎ᶜ in
  let prec′ : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ blame ⦂ A′
      prec′ = ⊑blameR (⊑ᶜᵀ-left-typed M⊑M′) (⊑ᶜᵀ-type-precision M⊑M′) in
  M , red , prec′

-- Case ξξ-blame

sim {M = M} {M′ = M′} {A = A} {A′ = A′} M⊑M′ (ξξ-blame x) =
  let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ M′ ⦂ A′
      prec = M⊑M′ in
  let red = M ∎ᶜ in
  let prec′ : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ blame ⦂ A′

...
| [`NuImprecisionLeftRenameNoBulletDef.agda`](NuImprecisionLeftRenameNoBulletDef.agda) | completed strict statement; GPT-5.5 proof ready to delegate | Generic left-only renaming for ordinary and quotiented QTI when both terms are bullet-free. It retains arbitrary renaming/world targets safely because every runtime-bullet constructor is impossible and returns the renamed relation directly. This is the reusable inert branch of canonical source-allocation runtime transport, not a path or result carrier. |
| [`NuImprecisionLeftSourceAllocationRuntimeTargetBulletDef.agda`](NuImprecisionLeftSourceAllocationRuntimeTargetBulletDef.agda) | completed strict statement; higher-order proof in progress | Focused direct `⊑αᵀ` reconstruction for canonical source allocation. The statement keeps the original and renamed right-lift worlds explicit, so the proof can normalize `νᵢᶜ (⇑ᴿᵢ Φ)` to `⇑ᴿᵢ (νᵢᶜ Φ)`, use the existing right-lift inverse store/context lemmas, transport the bullet-free body through `LeftRenameNoBullet`, and return the renamed QTI without a path or result carrier. |
| [`NuImprecisionLeftSourceAllocationRuntimeTransportDef.agda`](NuImprecisionLeftSourceAllocationRuntimeTransportDef.agda) | completed root statement; implementation plan repaired | Mutually transports ordinary and quotiented QTI derivations through the canonical source-allocation renaming and returns direct relation derivations. A second GPT-5.5 Ginger attempt exposed a precise recursive mismatch: at `⊑αᵀ`, the checked renaming lemma needs the target world visibly indexed as `⇑ᴿᵢ Ψ`, but a generalized runtime worker loses that shape under source polymorphic binders. The worker removed its non-checking draft. The repaired implementation uses `LeftRenameNoBullet` for every inert branch, recurses canonically only through the unique active `RuntimeOK` subterm, and isolates direct `⊑αᵀ` reconstruction in one focused helper using the existing right-lift inverse lemmas and local `νᵢᶜ`/`⇑ᴿᵢ` commutation. No public interface or carrier change is needed. |

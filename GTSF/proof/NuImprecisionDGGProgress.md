...
| [`NuImprecisionWorldCoherentSourceOneStepSourceNuFramesDef.agda`](NuImprecisionWorldCoherentSourceOneStepSourceNuFramesDef.agda) | completed flat statement; proof not started | Four matched/source-only ordinary/casted source-`ν` maps consume and return the existing complete continuing result. The recursive join will lift source blame directly with `ν-blame-tailᵀ`; no source-`ν` outcome carrier is introduced. |

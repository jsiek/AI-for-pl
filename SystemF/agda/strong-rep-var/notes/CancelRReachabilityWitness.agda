module strong-rep-var.notes.CancelRReachabilityWitness where
...
--     and installed in `strong-rep-var.Reduction`.  The first nine steps are
...
-- `strong-rep-var.Examples` §§1–7 passes a `∀Z. … ⇒ X` — an argument whose
-- polymorphic type RETURNS the abstracted variable — so no run there ever
-- produced the leaf; `strong-rep-var.Examples` §8 is this program, merged in.
--
...
-- `[ℕ]`), which is what `strong-rep-var.Examples` §2c already does for
-- `IdPush`.
...
--   and then CancelR.
--
-- Rendered (strong-rep-var.Show; α is a representation variable, X the
-- ordinary

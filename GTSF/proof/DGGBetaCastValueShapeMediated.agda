module proof.DGGBetaCastValueShapeMediated where

-- File Charter:
--   * Mediated-store DGG value/wrap structural recursion for target
...
--   * Split out of `DGGBetaCastValueMediated` so the public wrapper stays
--     small and quick to reload.
...
mediated-dgg-beta-cast-value-shape {ΔL = ΔL} {ΔR = ΔR} {ρ = ρ}
  ...
  let
    ...
    left-ready = ·₁-↠ noR L↠WF
  in
  χsF , WF · applyTerms χsF R , ΔLF , ρF , left-ready ,
  ΔLF≡ , refl , target-result-cast
mediated-dgg-beta-cast-value-shape
    vL noL vR noR vV′ vW′
    (⊒cast-ᵗ {t = id X} pᶜ r⊒ t⊒ʳ M⊒M′) ()
    p-domainᶜ R⊒W′

module strong-rep-var.proof.TypeSubst where
...
--     `singleTyEnv` and `_[_]ᵗ` are strong-rep-var.Types.  The few
-- single-index
--     facts the two-universe layer needs — `substᵗ-cong`,
--     `extsᵗ-renᵗ`, `substᵗ-renᵗ` — are strong-rep-var.proof.Types, kept apart
--     so that strong-rep-var.Ctx and everything above it can stand on a module
--     that imports nothing but strong-rep-var.Types.  Nothing here mentions a
...
--     strong-rep-var.proof.Preserve, so a law added here is not automatically

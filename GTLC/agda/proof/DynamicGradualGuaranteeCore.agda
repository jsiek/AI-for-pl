...
value-—↠-refl : ∀ {V N} → Valueᶜ V → V —↠ N → N ≡ V
value-—↠-refl vV (V ∎) = refl
value-—↠-refl vV (V —→⟨ V→M ⟩ M—↠N) = ⊥-elim (value-irreducible vV V→M)
...
blame-⊑-value V-$ ()
blame-⊑-value V-ƛ ()
blame-⊑-value {ℓ = ℓ} (V-cast! vM′) (⊑castR {M = blame {ℓ = ℓ}} blame⊑M′ _ _) =
  blame-⊑-value vM′ blame⊑M′
blame-⊑-value {ℓ = ℓ} (V-cast↦ vM′) (⊑castR {M = blame {ℓ = ℓ}} blame⊑M′ _ _) =
  blame-⊑-value vM′ blame⊑M′
...
⊑injL M⊑M′ vM g vM′ =
  ⊑castL M⊑M′ (⊢! g)
    (⊑idR atom-! (⊢! g) (⊑ᶜᵀ-type-precision M⊑M′) ⊑-★)
...
cast!-⊑-not-★ : ∀ {V V′ H A}
  ...
  → []⊑[] ⊢ cast V [ H ! ] ⦂ ★ ⊑ᶜᵀ V′ ⦂ A
  → Ground H × []⊑[] ⊢ V ⦂ H ⊑ᶜᵀ V′ ⦂ A
cast!-⊑-not-★ A≢★ (V-cast! vV′) (⊑cast V⊑V′ c≤ (⊢! h) (⊢! g)) = ⊥-elim (A≢★ refl)
cast!-⊑-not-★ A≢★ (V-cast↦ {c = c} {d = d} vV′) (⊑cast V⊑V′ c≤ (⊢! h) (⊢↦ c⦂ d⦂))
  with c≤
... | ()
cast!-⊑-not-★ A≢★ vV′ (⊑castL V⊑V′ c⦂ c⊑id)
  with c⦂
... | ⊢! h = h , V⊑V′
cast!-⊑-not-★ A≢★ (V-cast! vV′) (⊑castR V⊑V′ c⦂ id⊑c)
  with c⦂
... | ⊢! g = ⊥-elim (A≢★ refl)
cast!-⊑-not-★ A≢★ (V-cast↦ vV′) (⊑castR V⊑V′ c⦂ id⊑c)
  with c⦂
... | ⊢↦ c₁⦂ c₂⦂
  with cast!-⊑-not-★ (λ ()) vV′ V⊑V′
... | g , V⊑M′
  with g
... | G-⇒ =
  g , ⊑castR V⊑M′ (⊢↦ c₁⦂ c₂⦂)
...
inj-⊑-inj : ∀{V V′}{G G′}
   → Valueᶜ V
   → Valueᶜ V′
   → []⊑[] ⊢ cast V [ G ! ] ⦂ ★ ⊑ᶜᵀ cast V′ [ G′ ! ] ⦂ ★
   → []⊑[] ⊢ V ⦂ G ⊑ᶜᵀ V′ ⦂ G′
inj-⊑-inj vV vV′ (⊑cast V⊑V′ c≤ (⊢! g) (⊢! g′)) = V⊑V′
inj-⊑-inj vV vV′ (⊑castL V⊑V′ (⊢! g) c⊑id)
  with c⊑id
... | ⊑idR atom-! (⊢! g′) G⊑★ ★⊑★ = ⊥-elim (ground-not-⊑★ g G⊑★)
inj-⊑-inj vV vV′ (⊑castR V⊑V′ (⊢! g′) id⊑c′)
  with cast!-⊑-not-★ (less-ground-not-★ g′ ⊑-refl) vV′ V⊑V′
... | g , V⊑V′ = V⊑V′
...
proj-?-less-ground
  ...
  → Valueᶜ V′
  → G ⊑ A
  → ∃[ W ] Valueᶜ W × []⊑[] ⊢ W ⦂ G ⊑ᶜᵀ V′ ⦂ A × cast V [ (_`? {ℓ = ℓ}) G ] —↠ W
proj-?-less-ground {ℓ = ℓ} gG V⊑V′ vV vV′ G⊑A
    with canonical-★-inj vV (⊑ᶜᵀ-left-typed V⊑V′)
... | H , W , vW , refl
    with cast!-⊑-not-★ (less-ground-not-★ gG G⊑A) vV′ V⊑V′
... | g , W⊑V′
...
... | refl =
    ...
    , vW
    , W⊑V′
    , (cast (cast W [ _ ! ]) [ (_`? {ℓ = ℓ}) _ ]
         —→⟨ β-proj-inj-ok {ℓ = ℓ} vW ⟩
       W
       ∎)
...
Substᶜ-⊑ : Ctx → Ctx → ∀ {Δ Δ′} → (ρ′ : Δ ⊑ᵉ Δ′) → Substᶜ → Substᶜ → Set
Substᶜ-⊑ Γ Γ′ ρ′ σ σ′ = ∀ {x A A′} → Γ ∋ x ⦂ A → Γ′ ∋ x ⦂ A′
                                   → ρ′ ⊢ σ x ⦂ A ⊑ᶜᵀ σ′ x ⦂ A′

renameᶜ-⊑ : ∀ {Γ₁ Γ₂ Δ₁ Δ₂ A A′ M M′}
  → (r : Renameᶜ)
  → r ⦂ Γ₁ ⇒ʳ Δ₁
  → r ⦂ Γ₂ ⇒ʳ Δ₂
  → (ρ′ : Δ₁ ⊑ᵉ Δ₂)
...
cast-left-id-val : ∀ {V V′}{A A′ B}{c}
  ...
  → ⊢ c ⦂ A ⇨ B
  → c ⊑ᶜ idᶜ A′
  → ∃[ V₂ ] cast V [ c ] —↠ V₂ × []⊑[] ⊢ V₂ ⦂ B ⊑ᶜᵀ V′ ⦂ A′ × Valueᶜ V₂
cast-left-id-val {V} {c = c} V≤V′ vV vV′ ⊢idᶜ (⊑idᶜ A⊑A′) =
  V , (cast V [ c ] —→⟨ β-id vV ⟩ V ∎) , V≤V′ , vV
...
cast-left-id-val {V} {V′}{G}{A′} {c = G !} V≤V′ vV vV′ (⊢!{G = G} g)
  ...
  cast V [ G ! ]
  , (cast V [ G ! ] ∎)
  , ⊑castL V≤V′ (⊢! g) (⊑idR nseq (⊢! g′) A⊑A′ B⊑A′)
  , V-cast! vV
...
... | V₂ , V₁-d—↠V₂ , V₂⊑V′ , vV₂ =
  ...
  , V₂⊑V′
  , vV₂
cast-left-id-val {V} {V′} {A′ = A′} {c = ((_`? {ℓ = ℓ}) (★ ⇒ ★)) ⨟ c}
  V≤V′ vV vV′ (⊢⨟ (⊢? G-⇒) c⦂) (⊑drop? {ℓ = ℓ} c≤id)
  with ⊑ᶜ→⊑ ⊢idᶜ c⦂ c≤id
... | A⊑A′ , _
  with A⊑A′
... | ⊑-⇒ A₁⊑A₂ B₁⊑B₂
...
... | V₁ , V-c—↠V₁ , V₁⊑V′ , vV₁ =
  ...
  , (cast V [ c ⨟ (★ ⇒ ★) ! ]
     ...
     ∎)
  , ⊑castL V₁⊑V′ (⊢! G-⇒)
      (⊑idR atom-! (⊢! G-⇒) (⊑ᶜᵀ-type-precision V₁⊑V′) ⊑-★)
  , V-cast! vV₁
--------------------------------------------------------------------------------
-- Cast Left of Arrow
--
-- If V ⊑ V′ and c ⊑ c₁↦c₂
-- then V[c] —↠ V₂ and V₂ ⊑ V′[c₁↦c₂] for some V₂.
---
--------------------------------------------------------------------------------
...
cast-left-⨟-val : ∀{V V′}{A A′ B B′ C′}{d c′ d′}
   ...
   → ⊢ c′ ⦂ A′ ⇨ B′
   → ⊢ d′ ⦂ B′ ⇨ C′
   → ∃[ N₂ ] cast V [ d ] —↠ N₂ × []⊑[] ⊢ N₂ ⦂ B ⊑ᶜᵀ cast cast V′ [ c′ ] [ d′ ] ⦂ C′
cast-left-⨟-val{V} V≤V′ vV vV′ (⊑idL⨟ d≤c′⨟d′ d≤c′⨟d′₁) ⊢idᶜ c′⦂ d′⦂ =
  cast V [ idᶜ _ ] , (_ ∎) , ⊑castR (⊑cast V≤V′ d≤c′⨟d′ ⊢idᶜ c′⦂) d′⦂ d≤c′⨟d′₁
cast-left-⨟-val {V} {V′} {cd} {c′} {d′} V≤V′ vV vV′ (⊑⨟ {c′ = c} {d′ = d} d≤c′⨟d′ d≤c′⨟d′₁) (⊢⨟ d⦂ d⦂₁) c′⦂ d′⦂ =
  cast cast V [ c ] [ d ]
  , (cast V [ c ⨟ d ] —→⟨ β-seq vV ⟩ cast (cast V [ c ]) [ d ] ∎ )
  , ⊑cast (⊑cast V≤V′ d≤c′⨟d′ d⦂ c′⦂) d≤c′⨟d′₁ d⦂₁ d′⦂
cast-left-⨟-val {V} {d = ((_`? {ℓ = ℓ}) (★ ⇒ ★)) ⨟ d}
  V≤V′ vV vV′ (⊑drop? {c′ = d} d≤c′⨟d′) (⊢⨟ (⊢? {ℓ = ℓ} x) d⦂₁) c′⦂ d′⦂
    with ⊑ᶜ→⊑ (⊢⨟ c′⦂ d′⦂) d⦂₁ d≤c′⨟d′
... | aa , bb
...
... | N₂ , →N₂ , N₂≤ =
     let red = cast V [ ((_`? {ℓ = ℓ}) (★ ⇒ ★)) ⨟ d ] —↠⟨ cast-seq-lift vV →V₂ ⟩
               cast V₂ [ d ] —↠⟨ →N₂ ⟩
               N₂ ∎ in
     N₂ , red , N₂≤
...
... | N₂ , →N₂ , N₂≤ =
    let red = cast V [ d ⨟ (★ ⇒ ★) ! ] —↠⟨ cast-seq-lift vV →N₂ ⟩
              cast N₂ [ (★ ⇒ ★) ! ]
              ∎ in
    cast N₂ [ (★ ⇒ ★) ! ]
    , red
...
--------------------------------------------------------------------------------
cast-left-?-val : ∀ {V V′ A B A′ c ℓ}
  → []⊑[] ⊢ V ⦂ A ⊑ᶜᵀ cast V′ [ A′ ! ] ⦂ ★
  → Valueᶜ V
  → Valueᶜ V′
  → c ⊑ᶜ ((_`? {ℓ = ℓ}) A′)
  → ⊢ c ⦂ A ⇨ B
  → ∃[ N ] cast V [ c ] —↠ N × []⊑[] ⊢ N ⦂ B ⊑ᶜᵀ V′ ⦂ A′
cast-left-?-val {V} {V′} {A′ = A′} {c = (_`? {ℓ = ℓ}) A₁}
  V≤V′! vV vV′ (⊑? {ℓ = ℓ} A₁⊑A′) (⊢? {ℓ = ℓ} gA₁)
    with canonical-★-inj vV (⊑ᶜᵀ-left-typed V≤V′!)
... | G , W , vW , refl
...
cast-left-!-val : ∀ {V V′ A B G}{c}
  ...
  → c ⊑ᶜ (G !)
  → ⊢ c ⦂ A ⇨ B
  → ∃[ N ] Valueᶜ N × cast V [ c ] —↠ N × []⊑[] ⊢ N ⦂ B ⊑ᶜᵀ cast V′ [ G ! ] ⦂ ★
...
catchup-cast-idL↦ : ∀ {M M′ V A A′ B B′ A₀}{c c′}
  ...
  → ⊢ c ⦂ A ⇨ B → ⊢ c′ ⦂ A′ ⇨ B′
  → c ≡ idᶜ A₀ → c ⊑ᶜ c′
  → ∃[ V₂ ] Valueᶜ V₂ × (cast M [ c ] —↠ V₂) × []⊑[] ⊢ V₂ ⦂ B ⊑ᶜᵀ cast M′ [ c′ ] ⦂ B′
...
catchup : ∀ {V′ N A A′}
  → Valueᶜ V′
  → []⊑[] ⊢ N ⦂ A ⊑ᶜᵀ V′ ⦂ A′
  → ∃[ V ] ((Valueᶜ V) × (N —↠ V) × ([]⊑[] ⊢ V ⦂ A ⊑ᶜᵀ V′ ⦂ A′))
catchup vV′ (⊑` _ ())
catchup vV′ ⊑$ = $ _ , V-$ , ($ _ ∎) , ⊑$
catchup vV′ (⊑ƛ A⊑A′ N⊑M) = ƛ _ ⇒ _ , V-ƛ , (ƛ _ ⇒ _ ∎) , (⊑ƛ A⊑A′ N⊑M)
catchup () (⊑· L⊑L′ M⊑M′)
catchup vV′ (⊑cast {B′ = ℕ} M⊑M′ c⊑c′ cwt c′wt) = ⊥-elim (cast-value-not-ℕ vV′ c′wt)
catchup (V-cast! vV′) (⊑cast {M = M} {c = c} {c′ = c′} M⊑M′ c⊑c′ cwt (⊢! g))
    with catchup vV′ M⊑M′
... | V , vV , M—↠V , V⊑M′
    with cast-left-!-val V⊑M′ vV vV′ g c⊑c′ cwt
... | N , vN , V-c—↠N , N⊑castV′ =
      N
      , vN
      , (cast M [ c ]
...
catchup (V-cast↦ vV′) (⊑cast {M = M} {c = c} {c′ = c′} M⊑M′ c⊑c′ cwt c′wt)
    ...
    | ⊑idL↦ c≤id d≤id
    = catchup-cast-idL↦ M—↠V vV V⊑M′ cwt c′wt refl (⊑idL↦ c≤id d≤id)
catchup (V-cast↦ vV′) (⊑cast {M = M} {c = (((_`? {ℓ = ℓ}) (★ ⇒ ★)) ⨟ d)} {c′ = c′} M⊑M′ c⊑c′ cwt c′wt)
    | V , vV , M—↠V , V⊑M′
    | ⊑drop? {c = c₀} {c′ = d} d⊑c₀
    with cwt
... | ⊢⨟ (⊢? {ℓ = ℓ} G-⇒) dwt
...
... | V≤injV′ =
  ...
  , (cast V [ idᶜ ★ ]
     ...
     ∎)
  , V≤injV′
cast-left-!-val {V} {V′} {G = ℕ} {c = ((_`? {ℓ = ℓ}) (★ ⇒ ★)) ⨟ d}
  V≤V′ vV vV′ G-ℕ (⊑drop? {c′ = d} d≤!) (⊢⨟ (⊢? G-⇒) d⦂)
    with ⊑ᶜ→⊑ (⊢! G-ℕ) d⦂ d≤!
... | A⊑ℕ , B⊑★
    with A⊑ℕ
... | ()
cast-left-!-val {V} {V′} {G = ★ ⇒ ★} {c = ((_`? {ℓ = ℓ}) (★ ⇒ ★)) ⨟ d}
  V≤V′ vV vV′ G-⇒ (⊑drop? {c′ = d} d≤!) (⊢⨟ (⊢? G-⇒) d⦂)
    with star-to-inj V≤V′ vV vV′ G-⇒
... | V≤injV′
    with cast-left-?-val V≤injV′ vV vV′ (⊑? {ℓ = ℓ} ⊑-refl) (⊢? G-⇒)
... | W , V-?—↠W , W≤V′
    with catchup vV′ W≤V′
... | W₁ , vW₁ , W—↠W₁ , W₁≤V′
...
--
-- If V ⊑ λ N′ and W ⊑ W′
-- then V · W —↠ N and N ⊑ N′ [ W′] for some N.
---
--------------------------------------------------------------------------------
sim-beta : ∀{V W N′ W′}{A B A′ A′₂ B′}
  ...
  → Valueᶜ W
  → Valueᶜ W′
  → Σ[ N ∈ Termᶜ ] (V · W —↠ N) × ([]⊑[] ⊢ N ⦂ B ⊑ᶜᵀ N′ [ W′ ]ᶜ ⦂ B′)
sim-beta {V = V}{W = W}{N′ = N′}{W′ = W′}{A = A}{B = B}{A′ = A′}{A′₂ = A′₂}{B′ = B′}
  (⊑ƛ A⊑A′ N⊑N′) (V-ƛ {N = N}) W⊑W′ vW vW′ =
  let prec : []⊑[] ⊢ V · W ⦂ B ⊑ᶜᵀ (ƛ A′₂ ⇒ N′) · W′ ⦂ B′
      prec = ⊑· (⊑ƛ A⊑A′ N⊑N′) W⊑W′ in
  let red = V · W —→⟨ β-ƛ vW ⟩ N [ W ]ᶜ ∎ in
  let prec′ : []⊑[] ⊢ N [ W ]ᶜ ⦂ B ⊑ᶜᵀ N′ [ W′ ]ᶜ ⦂ B′
      prec′ = []ᶜ-⊑ A⊑A′ N⊑N′ W⊑W′ in
  N [ W ]ᶜ , red , prec′
sim-beta
  {V = cast U [ c₁ ↦ d₁ ]}{W = W}{N′ = N′}{W′ = W′}{A = A}{B = B}{A′ = A′}{A′₂ = A′₂}{B′ = B′}
...
sim-beta-cast
  {W = W}{V′ = V′}{W′ = W′}{A = A}{B = B}{A′ = A′}{B′ = B′}{c′ = c′}{d′ = d′}
  (⊑cast {M = V₁} V₁≤V′ c≤c′↦d′ c⦂ (⊢↦ c′⦂ d′⦂))
  (V-cast↦ {c = c} {d} vV₁) W≤W′ vW vW′
    with c≤c′↦d′ | c⦂
... | ⊑↦ c≤c′ d≤d′ | ⊢↦ c⦂₁ d⦂ =
  let prec : []⊑[] ⊢ cast V₁ [ c ↦ d ] · W ⦂ B ⊑ᶜᵀ cast V′ [ c′ ↦ d′ ] · W′ ⦂ B′
      prec = ⊑· (⊑cast V₁≤V′ c≤c′↦d′ (⊢↦ c⦂₁ d⦂) (⊢↦ c′⦂ d′⦂)) W≤W′ in
  let red = cast V₁ [ c ↦ d ] · W
            —→⟨ β-↦ vV₁ vW ⟩
          cast (V₁ · cast W [ c ]) [ d ]
          ∎ in
  let prec′ : []⊑[] ⊢ cast (V₁ · cast W [ c ]) [ d ] ⦂ B ⊑ᶜᵀ cast (V′ · cast W′ [ c′ ]) [ d′ ] ⦂ B′
      prec′ = ⊑cast (⊑· V₁≤V′ (⊑cast W≤W′ c≤c′ c⦂₁ c′⦂)) d≤d′ d⦂ d′⦂ in
  cast (V₁ · cast W [ c ]) [ d ] , red , prec′
sim-beta-cast
  {W = W}{V′ = V′}{W′ = W′}{A = A}{B = B}{A′ = A′}{B′ = B′}{c′ = c′}{d′ = d′}
  (⊑castL {M = U} V⊑V′cd (⊢↦ cwt₁ dwt₁) c⊑id)
  (V-cast↦ {c = c₁} {d = d₁} vU) W⊑W′ vW vW′
    with ⊑ᶜ→⊑ ⊢idᶜ (⊢↦ cwt₁ dwt₁) c⊑id
... | A₁⇒B₁⊑A⇒B , A⇒B⊑A′⇒B′
    with A₁⇒B₁⊑A⇒B | A⇒B⊑A′⇒B′
... | ⊑-⇒ A₁⊑A B₁⊑B | ⊑-⇒ A⊑A′ B⊑B′
...
... | N , UW₁—↠N , N⊑ =
      let prec : []⊑[] ⊢ cast U [ c₁ ↦ d₁ ] · W ⦂ B ⊑ᶜᵀ cast V′ [ c′ ↦ d′ ] · W′ ⦂ B′
          prec = ⊑· (⊑castL V⊑V′cd (⊢↦ cwt₁ dwt₁) c⊑id) W⊑W′ in
      let red = cast U [ c₁ ↦ d₁ ] · W
                —→⟨ β-↦ vU vW ⟩
              cast (U · cast W [ c₁ ]) [ d₁ ]
                —↠⟨ ξ* (cast□[ d₁ ]) (ξ* (U ·□ vU) W-c₁—↠W₁) ⟩
              ...
              cast N [ d₁ ]
              ∎ in
      let prec′ : []⊑[] ⊢ cast N [ d₁ ] ⦂ B ⊑ᶜᵀ cast (V′ · cast W′ [ c′ ]) [ d′ ] ⦂ B′
          prec′ = ⊑castL N⊑ dwt₁ d₁⊑id in
      cast N [ d₁ ] , red , prec′
sim-beta-cast
  {W = W}{V′ = V′}{W′ = W′}{A = A}{B = B}{A′ = A′}{B′ = B′}{c′ = c′}{d′ = d′}
  ...
  vV W⊑W′ vW vW′
    with id⊑c′↦d′
... | ⊑idL↦ c≤c′ d≤d′ =
  let prec : []⊑[] ⊢ V₁ · W ⦂ B ⊑ᶜᵀ cast V′ [ c′ ↦ d′ ] · W′ ⦂ B′
      prec = ⊑· (⊑castR V₁⊑V′ (⊢↦ c′⦂ d′⦂) id⊑c′↦d′) W⊑W′ in
  let red = V₁ · W ∎ in
  let prec′ : []⊑[] ⊢ V₁ · W ⦂ B ⊑ᶜᵀ cast (V′ · cast W′ [ c′ ]) [ d′ ] ⦂ B′
      prec′ = ⊑castR (⊑· V₁⊑V′ (⊑castR W⊑W′ c′⦂ c≤c′)) d′⦂ d≤d′ in
  V₁ · W , red , prec′
...
sim {A = A} {A′ = A′} (⊑· {L = L} {L′ = L′} {M = M} {M′ = M′} L⊑L′ M⊑M′)
    (ξξ {F = □· .M′}{N = N′} refl refl L′→N′)
    with sim L⊑L′ L′→N′
... | N , L→N , N⊑N′ =
    let prec : []⊑[] ⊢ L · M ⦂ A ⊑ᶜᵀ L′ · M′ ⦂ A′
        prec = ⊑· L⊑L′ M⊑M′ in
    let red = ξ* (□· M) L→N in
    let prec′ : []⊑[] ⊢ N · M ⦂ A ⊑ᶜᵀ N′ · M′ ⦂ A′
        prec′ = ⊑· N⊑N′ M⊑M′ in
    N · M , red , prec′
sim {A = A} {A′ = A′} (⊑· {L = L} {L′ = V′} {M = M} {M′ = M′} L⊑V′ M⊑M′)
    (ξξ {F = .V′ ·□ vV′}{N = N′} refl refl M′→N′)
    with catchup vV′ L⊑V′
... | V , vV , L→V , V⊑V′
    with sim M⊑M′ M′→N′
... | N , M→N , N⊑N′ =
    let prec : []⊑[] ⊢ L · M ⦂ A ⊑ᶜᵀ V′ · M′ ⦂ A′
        prec = ⊑· L⊑V′ M⊑M′ in
    let red = L · M
              —↠⟨ ξ* (□· M) L→V ⟩
              V · M
              ...
              V · N
              ∎ in
    let prec′ : []⊑[] ⊢ V · N ⦂ A ⊑ᶜᵀ V′ · N′ ⦂ A′
        prec′ = ⊑· V⊑V′ N⊑N′ in
    V · N , red , prec′
sim {A = A} {A′ = A′}
    (⊑cast {M = M} {M′ = M′} {c = c} {c′ = c′} M⊑M′ c≤c′ c⦂ c′⦂)
    (ξξ {F = cast□[ .c′ ]}{N = N′} refl refl M′→N′)
    with sim M⊑M′ M′→N′
... | N , M→N , N⊑N′ =
    let prec : []⊑[] ⊢ cast M [ c ] ⦂ A ⊑ᶜᵀ cast M′ [ c′ ] ⦂ A′
        prec = ⊑cast M⊑M′ c≤c′ c⦂ c′⦂ in
    let red = cast M [ c ]
              —↠⟨ ξ* (cast□[ c ]) M→N ⟩
              cast N [ c ]
              ∎ in
    let prec′ : []⊑[] ⊢ cast N [ c ] ⦂ A ⊑ᶜᵀ cast N′ [ c′ ] ⦂ A′
        prec′ = ⊑cast N⊑N′ c≤c′ c⦂ c′⦂ in
    cast N [ c ] , red , prec′
sim {A = A} {A′ = A′} (⊑castR {M = M} {M′ = M′} {c′ = c′} M⊑M′ c′⦂ id≤c′)
    (ξξ {F = cast□[ .c′ ]}{N = N′} refl refl M′→N′)
    with sim M⊑M′ M′→N′
... | N , M→N , N⊑N′ =
    let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ cast M′ [ c′ ] ⦂ A′
        prec = ⊑castR M⊑M′ c′⦂ id≤c′ in
    let red = M→N in
    let prec′ : []⊑[] ⊢ N ⦂ A ⊑ᶜᵀ cast N′ [ c′ ] ⦂ A′
        prec′ = ⊑castR N⊑N′ c′⦂ id≤c′ in
    N , red , prec′
...
sim (⊑·{A}{A′}{B}{B′}{L}{L′}{M}{M′} L⊑L′ M⊑M′) (β-ƛ{A = A₂}{N = N′} vM′)
...
... | N , VW→N , N⊑N′W′ =
      let prec : []⊑[] ⊢ L · M ⦂ B ⊑ᶜᵀ (ƛ A₂ ⇒ N′) · M′ ⦂ B′
          prec = ⊑· L⊑L′ M⊑M′ in
      let red = L · M    —↠⟨ ξ* (□· M) L→V ⟩
                V · M    —↠⟨ ξ* (V ·□ vV) M→W ⟩
                V · W    —↠⟨ VW→N ⟩
                N        ∎ in
      let prec′ : []⊑[] ⊢ N ⦂ B ⊑ᶜᵀ N′ [ M′ ]ᶜ ⦂ B′
          prec′ = N⊑N′W′ in
      N , red , prec′
...
sim {A = A} {A′ = A₀}
  (⊑cast{M = M}{M′ = M′}{c = c}{c′ = idᶜ A₀} M⊑M′ c≤ c⦂ ⊢idᶜ)
  (β-id {A = A₀} vN′) =
  let prec : []⊑[] ⊢ cast M [ c ] ⦂ A ⊑ᶜᵀ cast M′ [ idᶜ A₀ ] ⦂ A₀
      prec = ⊑cast M⊑M′ c≤ c⦂ ⊢idᶜ in
  let red = cast M [ c ] ∎ in
  let prec′ : []⊑[] ⊢ cast M [ c ] ⦂ A ⊑ᶜᵀ M′ ⦂ A₀
      prec′ = ⊑castL M⊑M′ c⦂ c≤ in
  cast M [ c ] , red , prec′
sim {A = A} (⊑castL{M = M}{c = c} M⊑M′ x x₁) (β-id {A = A₀} vN′)
    with ⊑ᶜᵀ-right-typed M⊑M′
... | ⊢cast {M = M′} M′⦂ ⊢idᶜ = 
    let prec : []⊑[] ⊢ cast M [ c ] ⦂ A ⊑ᶜᵀ cast M′ [ idᶜ A₀ ] ⦂ A₀
        prec = ⊑castL M⊑M′ x x₁ in
    let red = cast M [ c ] ∎ in
    let prec′ : []⊑[] ⊢ cast M [ c ] ⦂ A ⊑ᶜᵀ M′ ⦂ A₀
        prec′ = ⊑castL (⊑castR-id-inversion M⊑M′) x x₁ in
    cast M [ c ] , red , prec′
sim {A = A} (⊑castR{M = M}{M′ = M′} M⊑M′ ⊢idᶜ x₁) (β-id {A = A₀} vN′) =
  let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ cast M′ [ idᶜ A₀ ] ⦂ A₀
      prec = ⊑castR M⊑M′ ⊢idᶜ x₁ in
  let red = M ∎ in
  let prec′ : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ M′ ⦂ A₀
      prec′ = M⊑M′ in
  M , red , prec′
...
sim {A = A} {A′ = A′}
  (⊑cast {M = M} {V′} {c = c} {c′ ⨟ d′} M⊑V′ c≤c′⨟d′ c⦂ (⊢⨟ c′⦂ d′⦂))
  (β-seq vV′)
    with catchup vV′ M⊑V′
... | V , vV , M→V , V⊑V′ 
    with ⊑ᶜ→⊑ (⊢⨟ c′⦂ d′⦂) c⦂ c≤c′⨟d′
...
... | N₂ , →N₂ , N₂≤ =
    let prec : []⊑[] ⊢ cast M [ c ] ⦂ A ⊑ᶜᵀ cast V′ [ c′ ⨟ d′ ] ⦂ A′
        prec = ⊑cast M⊑V′ c≤c′⨟d′ c⦂ (⊢⨟ c′⦂ d′⦂) in
    let red = cast M [ c ] —↠⟨ ξ* _ M→V ⟩
              cast V [ c ] —↠⟨ →N₂ ⟩
              N₂ ∎ in
    let prec′ : []⊑[] ⊢ N₂ ⦂ A ⊑ᶜᵀ cast (cast V′ [ c′ ]) [ d′ ] ⦂ A′
        prec′ = N₂≤ in
    N₂ , red , prec′
...
sim {A = A} {A′ = A′}
  (⊑castR{M = M}{M′ = V′} M⊑V′ (⊢⨟ {c = c} {d = d} c⦂ d⦂) (⊑idL⨟ id≤c id≤d))
  (β-seq vV′) =
    let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ cast V′ [ c ⨟ d ] ⦂ A′
        prec = ⊑castR M⊑V′ (⊢⨟ c⦂ d⦂) (⊑idL⨟ id≤c id≤d) in
    let red = M ∎ in
    let prec′ : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ cast (cast V′ [ c ]) [ d ] ⦂ A′
        prec′ = ⊑castR (⊑castR M⊑V′ c⦂ id≤c) d⦂ id≤d in
    M , red , prec′

-- Case β-↦

sim {M = L · M} {A = A} {A′ = A′} (⊑· L⊑L′ M⊑W′) (β-↦ {V = V′}{W′}{c = c′} {d′} vV′ vW′)
...
... | N , VW→N , N≤ =
    let prec : []⊑[] ⊢ L · M ⦂ A ⊑ᶜᵀ cast V′ [ c′ ↦ d′ ] · W′ ⦂ A′
        prec = ⊑· L⊑L′ M⊑W′ in
    let red = L · M    —↠⟨ ξ* (□· M) →V ⟩
              V · M    —↠⟨ ξ* (V ·□ vV) →W ⟩
              V · W    —↠⟨ VW→N ⟩
              N        ∎ in
    let prec′ : []⊑[] ⊢ N ⦂ A ⊑ᶜᵀ cast (V′ · cast W′ [ c′ ]) [ d′ ] ⦂ A′
        prec′ = N≤ in
    N , red , prec′
...
sim {A = A} {A′ = A′}
    (⊑cast {M = M} {M′ = M′} {c = c} M⊑M′ c≤c′ c⦂ (⊢? {ℓ = ℓ} gA′))
    (β-proj-inj-ok {V = V′} {ℓ = ℓ} vV′)
    with catchup (V-cast! vV′) M⊑M′
... | V , vV , M→V , V≤M′
    with cast-left-?-val V≤M′ vV vV′ c≤c′ c⦂
... | N , Vc→N , N≤ =
    let prec : []⊑[] ⊢ cast M [ c ] ⦂ A ⊑ᶜᵀ cast M′ [ (_`? {ℓ = ℓ}) A′ ] ⦂ A′
        prec = ⊑cast M⊑M′ c≤c′ c⦂ (⊢? {ℓ = ℓ} gA′) in
    let red = cast M [ c ] —↠⟨ ξ* (cast□[ c ]) M→V ⟩
              cast V [ c ] —↠⟨ Vc→N ⟩
              N ∎ in
    let prec′ : []⊑[] ⊢ N ⦂ A ⊑ᶜᵀ V′ ⦂ A′
        prec′ = N≤ in
...
sim {A = A} {A′ = A′}
    (⊑castR {M = M} {M′ = cast V′ [ A′ ! ]} M⊑M′ (⊢? {ℓ = ℓ} gA′) id≤c′)
    (β-proj-inj-ok {G = A′} {ℓ = ℓ} vV′)
    with id≤c′
... | (⊑idL atom-? (⊢? {ℓ = ℓ} gA′) A⊑★ A⊑A′)
    with catchup (V-cast! vV′) M⊑M′
... | V , vV , M→V , V≤M′
    with A⊑★
... | ⊑-★
    with cast-left-?-val V≤M′ vV vV′ (⊑idL atom-? (⊢? {ℓ = ℓ} gA′) ⊑-★ A⊑A′) ⊢idᶜ
... | .(cast V [ idᶜ ★ ]) , (cast V [ idᶜ ★ ] ∎) , N≤ =
    let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ cast (cast V′ [ A′ ! ]) [ (_`? {ℓ = ℓ}) A′ ] ⦂ A′
        prec = ⊑castR M⊑M′ (⊢? {ℓ = ℓ} gA′) id≤c′ in
    let red = M→V in
    let prec′ : []⊑[] ⊢ V ⦂ A ⊑ᶜᵀ V′ ⦂ A′
        prec′ = ⊑castL-id-inversion N≤ in
    V , red , prec′
... | N , (cast V [ idᶜ ★ ] —→⟨ β-id vV ⟩ V→N) , N≤ =
    let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ cast (cast V′ [ A′ ! ]) [ (_`? {ℓ = ℓ}) A′ ] ⦂ A′
        prec = ⊑castR M⊑M′ (⊢? {ℓ = ℓ} gA′) id≤c′ in
    let red = M→V ++ᶜ V→N in
    let prec′ : []⊑[] ⊢ N ⦂ A ⊑ᶜᵀ V′ ⦂ A′
        prec′ = N≤ in
    N , red , prec′
... | _ , (cast V [ idᶜ ★ ] —→⟨ ξ (cast□[ idᶜ ★ ]) V→W ⟩ W→N) , _ =
    ⊥-elim (value-irreducible vV V→W)
... | _ , (cast V [ idᶜ ★ ] —→⟨ ξ-blame (cast□[ idᶜ ★ ]) ⟩ W→N) , _ =
    ⊥-elim (value-not-blame vV)

-- Case β-proj-inj-bad

sim {M = M} {M′ = M′} {A = A} {A′ = A′} M⊑M′ (β-proj-inj-bad {ℓ = ℓ} x x₁) =
  let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ M′ ⦂ A′
      prec = M⊑M′ in
  let red = M ∎ in
  let prec′ : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ blame {ℓ = ℓ} ⦂ A′
...
sim {M = M} {M′ = M′} {A = A} {A′ = A′} M⊑M′ (ξξ-blame {ℓ = ℓ} x) =
  let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ M′ ⦂ A′
      prec = M⊑M′ in
  let red = M ∎ in
  let prec′ : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ blame {ℓ = ℓ} ⦂ A′
...
sim* : ∀ {M M′ N′ A A′}
  ...
  → M′ —↠ N′
  → ∃[ N ] ((M —↠ N) × ([]⊑[] ⊢ N ⦂ A ⊑ᶜᵀ N′ ⦂ A′))
sim* {M = M} M≤M′ (M′ ∎) = M , (M ∎) , M≤M′
sim* {M = M} M≤M′ (M′ —→⟨ M′→N₁ ⟩ N₁→N′)
    with sim M≤M′ M′→N₁
... | N₁ , M→N₁ , N₁≤N₁′
    with sim* N₁≤N₁′ N₁→N′
... | N , N₁→N , N≤N′ =

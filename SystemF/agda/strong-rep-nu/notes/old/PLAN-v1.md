...
## 1. Current state (files under `SystemF/agda/strong-rep-nu/`)
...
(`DualDef`, `ProgressDef`) — no holes, no postulates anywhere under `strong-rep-nu/`.
...
4. **The public/proof split refactoring** (Jeremy, 2026-09-04; the AGENTS.md
   ...
   `strong-rep-nu/` holds ONLY the definitions the main theorem statements depend on (Types,
   ...
   `strong-rep-nu/proof/`. Audit contract: reading `strong-rep-nu/` alone gives 100% confidence, because
...
- **No `cd`** in Bash — use `agda -v0 strong-rep-nu/File.agda` from `SystemF/agda`, or `make -C`.
...
From `SystemF/agda/`: `agda --safe -v0 strong-rep-nu/All.agda` (or `make -C strong agda`)
checks the whole NEW design; `make -C strong check` adds `postulate-check`
(no `postulate`/holes/unsafe pragmas anywhere under `strong-rep-nu/`, `notes/old/`

...
| [`NuImprecisionLeftSourceAllocationRuntimeTargetBulletDef.agda`](NuImprecisionLeftSourceAllocationRuntimeTargetBulletDef.agda) | completed repaired strict statement; higher-order proof in progress | Focused direct `⊑αᵀ` reconstruction for canonical source allocation. The first statement used the equal but proof-relevantly different left-after-right world; equality of the world expressions did not identify their renaming witnesses or indices. The repaired statement uses the constructor-natural `⇑ᴿᵢ (νᵢᶜ Φ)` world and `rename-assm²-⇑ᴿᵢ rename-assm²-source-νᵢ`, so `left-rename-⊑αᵀ` applies exactly with the bullet-free body supplied by `LeftRenameNoBullet`. It removes the redundant source-typing premise and adds no coherence or result carrier. |

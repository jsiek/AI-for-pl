...
- [x] Port `SystemF/agda/curry/Eval.agda` into Lean (intrinsic-style development).
- [x] Port `SystemF/agda/curry/Examples.agda` into Lean (intrinsic-style development). Completed seed/TAPL/Church/list-pair suites with Agda/extrinsic-style quoted aliases (`«..._↠»`) and canonical boolean/nat endpoints, including the cast-sensitive `nilIsNil` endpoint proof to `true`.

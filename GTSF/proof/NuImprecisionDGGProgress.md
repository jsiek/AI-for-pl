...
| [`NuImprecisionWorldCoherentRightTargetActiveRootResumeDef.agda`](NuImprecisionWorldCoherentRightTargetActiveRootResumeDef.agda) | completed strict statement; proofs not started | Ten provenance-preserving active-root continuations: five `β-id` entries, narrowing untag, general and identity-only instantiation, general unseal, and reveal-unseal. Every field consumes a completed inner catch-up and returns the same existing complete carrier rooted at the original cast. Identity-only instantiation remains an explicit hard leaf because the current casted target-allocation entry requires `CastMode`, which `id-onlyᵈ` does not provide. |
| [`NuImprecisionWorldCoherentRightTargetCastTerminalizationDef.agda`](NuImprecisionWorldCoherentRightTargetCastTerminalizationDef.agda) | completed statement; proof blocked on one explicit boundary | Five-field terminalization boundary for target narrowing, widening, identity-only widening, reveal, and conceal. Sequence and inert plans now have exact continuations, but active plans still need constructor-specific root resumption for `β-id`, narrowing untag, widening/id-only instantiation and unseal, and reveal-unseal. Existing weak one-step root modules do not retain transport, coherence, lineage, source-bullet, or final-world evidence and therefore cannot safely fill this boundary. |
...
| [`NuImprecisionLeftSourceAllocationRuntimeTransportDef.agda`](NuImprecisionLeftSourceAllocationRuntimeTransportDef.agda) | completed root statement; recursive interface repair required | Mutually transports ordinary and quotiented QTI derivations through the canonical source-allocation renaming and returns direct relation derivations. A second GPT-5.5 Ginger attempt exposed a precise recursive mismatch: at `⊑αᵀ`, the checked renaming lemma needs the target world visibly indexed as `⇑ᴿᵢ Ψ`, but descent through source polymorphic binders leaves only an arbitrary renamed world `Ψ`. The worker removed its non-checking draft. The next repair must retain this binder-lift lineage through a specialized theorem or higher-order bullet handler without adding a path/result/outcome carrier. |

...
- DualRep≈: FALSE as stated, REPAIRED AND FULLY PROVEN
  (strong-rep-var/DualRepProof.agda): BlkRepWf (the index relation
...
- DualInt≈: FALSE (two corners, xrvld and double-refusal — both the
  ...
  above; strongest-true version delivered (strong-rep-var/DualIntProof.agda):
...
FACE half of a split boundary — scope skeleton (strong-rep-var/, rep-free) +
...
RESULT: strong-rep-var.Preservation has top-level `preservation : Preservation`
...
retired from identifiers, comments and notes of strong-rep-var/ (v1 memos,
...
INSTALLED in strong-rep-var.Boundary §3.  The whole development still checks, and
...
THE TOOLING THIS FORCED (strong-rep-var.TypeCheck).  Because `_⋉_` and `rewind`
...
map with `Unique` names by `same-target-unique` (strong-rep-var.Ctx) — already
...
and `SameConv` in `strong-rep-var.Conversion` §2b, which is `_⊢_~_` one universe
...
`notes/PeelPremise.agda` to `strong-rep-var.Boundary` §3a, where it is stated from
...
THE RULING, INSTALLED in `strong-rep-var.Boundary` §3a:
...
`strong-rep-var.proof.Preserve` keeps `CancelRCase` as the parameter of
`Impl` it always was, and `strong-rep-var.Preservation` and `strong-rep-var.TypeSafety`
...
THE INTERFACE SHRANK. `strong-rep-var.Preservation.Stage1` now takes `crossΛ`,
`addLock0`, `repWeaken` and `cancel`; `peel` and `idpush` are discharged
and gone. `strong-rep-var.TypeSafety.Stage1` and `proof/TypeSafety.agda` follow.
...
* `proof/MaskFacts.agda` — its whole subject is masking: `mask`/`unmask`
  ...
  `conv-lock` itself (`strong-rep-var.Boundary` §3), which skips a lock outright,
...
* `proof/PreserveObstruct.agda` — four concrete redexes, each written in
  ...
  into `strong-rep-var.Examples`), which exercise them from plain source rather
...
* `proof/Canonicity.agda`, with ONE NEW SECTION. The family and its mint,
  ...
  which validated the invariant on `strong-rep-var.Examples`, is dropped while that
...
* `proof/ShiftAudit.agda`. The criterion survives verbatim; what changes is
  ...
  live facts about the live wrapper clause, and `strong-rep-var.Reduction`'s own
  comment cites them.
...
`strong-rep-var.proof.PreserveObstruct` twice (§§ around its IdPush and TyPeelR
...
comments in `strong-rep-var.Reduction` still name the four deleted modules; they
...
* old §12b, and the witnesses old §13a/§13b imported — they came from
  `strong-rep-var.proof.PreserveObstruct`, deleted in the module sweep.
* old §15 (TIGHTNESS, RULE BY RULE) — its seven frame identities were
  ...
  (`strong-rep-var.Boundary` §3a), and `proof/ShiftAudit.agda` is what consumes
  them.
...
`showRun` renders a whole `strong-rep-var.Eval` trace with the name of the rule
...
THE RULING: STILL NOTHING IS CHANGED.  `strong-rep-var.Reduction` is untouched,
...
approved and installed.  `strong-rep-var.Reduction` is no longer untouched.
...
`strong-rep-var.proof.MoveScope.preserve-CancelR`, beside `preserve-IdPush` and by
...
CONSEQUENTLY THE INTERFACE SHRANK AGAIN.  `strong-rep-var.Preservation.Stage1`
takes `crossΛ`, `addLock0` and `repWeaken` — `cancel` is gone, as `peel`
and `idpush` went before it.  `strong-rep-var.TypeSafety.Stage1` and
...
`strong-rep-var.proof.RepWeaken.rep-weaken-⊢`, and `PeelCase` is therefore
UNCONDITIONAL: `strong-rep-var.Preservation.Stage1` now takes only `crossΛ` and
...
`strong-rep-var.proof.PeelDual` §3 — the premise is `bw-binds mwΘ` of the very
...
The cut is then entirely in the closure lemmas, `strong-rep-var.Boundary` §3d:
...
per-relation lemma, in the style §3a already used:
  ...
  * the conversion's TYPING by `conv-ren` (strong-rep-var.Conversion §2d).  A
...
`TermSubst.agda` to `strong-rep-var.Ctx` §8, and `renᶠᴿ`/`renᴮᴿ` from
`TermSubst.agda` to `strong-rep-var.Boundary` §2/§3, because the renaming
...
`strong-rep-var.Ctx` is already opened there.  `strong-rep-var.Ctx` §8 also gains the
...
The proof is `strong-rep-var.proof.RepWeaken.cross-Λ-⊢`, beside
...
THE INTERFACE SHRANK.  `strong-rep-var.Preservation.Stage1` now takes ONLY
...
`CancelR` and `IdPush`.  `strong-rep-var.TypeSafety.Stage1` and
`strong-rep-var.proof.TypeSafety.Stage1` now take `merged-reading` plus
...
WHAT THE PUBLIC SURFACE SAYS NOW.  `strong-rep-var.Preservation.Stage1` and
`strong-rep-var.TypeSafety.Stage1` keep their `addLock0` parameter, but that
...
experiment reported.  `strong-rep-var.Progress.Stage1` therefore still takes
...
CONSEQUENTLY `strong-rep-var.Preservation.Stage1` IS GONE.  `preservation` and
`preservation*` are stated outright, unconditionally.  `strong-rep-var.TypeSafety`
exports them unconditionally too, and its `Stage1` — like
`strong-rep-var.Progress.Stage1` and `strong-rep-var.proof.TypeSafety.Stage1` — now takes
...
The resulting module map, bottom up:
  ...
  * `proof/Types.agda` (NEW) — `substᵗ-cong`, `extsᵗ-renᵗ`,
    `substᵗ-renᵗ`.  Imports `strong-rep-var.Types` and the standard library, and
...
at all.  Downstream, twelve files gained `open import strong-rep-var.proof.Ctx`;
...
`strong-rep-var.Boundary.merged-conversion-exists`; no premise was added.

THE MILESTONE. `strong-rep-var.proof.Progress.Impl` now consumes the proved
`merged-reading`, and `strong-rep-var.Progress.progress` is stated outright.
`Stage1` is deleted from `strong-rep-var.Progress`, `strong-rep-var.proof.TypeSafety` and
`strong-rep-var.TypeSafety`; `type-safety` is stated outright. Preservation,
...
`strong-rep-var.All` already reached `strong-rep-var.Examples`, so the checked set loses
...
| old | new |
|---|---|
| module `strong-rep-var.CtxMorph`, file `CtxMorph.agda` | `strong-rep-var.Boundary`, `Boundary.agda` |

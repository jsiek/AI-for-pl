...
proj-?-less-ground
  ...
  → Valueᶜ V′
  → G ⊑ A
  → ∃[ W ] Valueᶜ W × []⊑[] ⊢ W ⦂ G ⊑ᶜᵀ V′ ⦂ A × cast V [ (_`? {ℓ = ℓ}) G ] —↠ᶜ W
...
... | N₂ , →N₂ , N₂≤ =
     let red = cast V [ ((_`? {ℓ = ℓ}) (★ ⇒ ★)) ⨟ d ] —↠ᶜ⟨ cast-seq-lift vV →V₂ ⟩
               cast V₂ [ d ] —↠ᶜ⟨ →N₂ ⟩
               N₂ ∎ᶜ in
     N₂ , red , N₂≤
...
cast-left-!-val : ∀ {V V′ A B G}{c}
  ...
  → c ⊑ᶜ (G !)
  → ⊢ c ⦂ A ⇨ B
  → ∃[ N ] Valueᶜ N × cast V [ c ] —↠ᶜ N × []⊑[] ⊢ N ⦂ B ⊑ᶜᵀ cast V′ [ G ! ] ⦂ ★
...
sim-beta {V = V}{W = W}{N′ = N′}{W′ = W′}{A = A}{B = B}{A′ = A′}{A′₂ = A′₂}{B′ = B′}
  ...
  let red = V · W —→ᶜ⟨ β-ƛ vW ⟩ N [ W ]ᶜ ∎ᶜ in

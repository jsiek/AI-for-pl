...
| [`NuImprecisionWorldCoherentRightValueCatchupRuntimeNoBulletTransportDef.agda`](NuImprecisionWorldCoherentRightValueCatchupRuntimeNoBulletTransportDef.agda) | completed flat statement; proof in progress | Transports an active source sibling and no-bullet target sibling directly through a completed right-value catch-up. Its output is the final QTI derivation itself; the hard `α⊑ᵀ` crossing is isolated without a runtime-transport result carrier. |

...
`proof.Compilation.DynamicGradualGuarantee`:
...
`extendReplaceRel-term (replace-here qᶜ)` from `proof.Catchup.Core.Catchup` after the
recursive call:
...
the existing split warning in `proof.Catchup.Core.Catchup`, where `catchup-split-catchup`
...
mechanized in `proof.Core.Properties.RightSealInversionCounterexample`:

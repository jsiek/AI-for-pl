...
`strong-rep-store/Examples.agda`, so the de Bruijn indices are the machine's, not
...
* Because lookup is by slot identity, moving a term across a renaming
  ...
  `ren-kn r d = ren∋ r d` in `strong-rep-store.Ctx`.
...
1. **Mask, don't drop.**  An `unbind X` masks `X` *in place* (`mask`,
   `strong-rep-store.Ctx`) and retains every other entry, so a variable bound
...
### Boundary scopes (`strong-rep-store.Boundary`)
...
One derived number, in `strong-rep-store.Boundary`:
...
### Conversions (`strong-rep-store.Conversion`)
...
## 3. Type contexts and the mask discipline (`strong-rep-store.Ctx`)
...
for `showBinding`/`showEntry` in `strong-rep-store.Show`, whose rendered strings
...
## 4. Typing (`strong-rep-store.Boundary`, `strong-rep-store.Terms`, `strong-rep-store.Conversion`)
...
## 5. Values, and the active/inert split (`strong-rep-store.Terms`)
...
## 6. Reduction (`strong-rep-store.Reduction`; the minted conversions `reveal`/`conceal`/`instReveal`/`instConceal` are in `strong-rep-store.Conversion` §4)
...
**The appended unbind** (§6.4, `strong-rep-store.Boundary` §5):
...
case *is* the substitution lemma (`strong-rep-store.TermSubst.⊢subst`).  Term
...
(`strong-rep-store.TermSubst` §6) is proved by the same two moves: `⊢rename` at
...
Both live in the substitution's IMAGES (`strong-rep-store.TermSubst` §5b):
...
Stated in `strong-rep-store.Progress` (and again in `strong-rep-store.TypeSafety`), proven in
...
* **`TyBeta`** (`proof/Preserve.preserve-TyBeta`) — the mint.  The new
  ...
  definitional.
* **`Beta`** — `⊢subst` (`strong-rep-store.TermSubst`).
...
* **`TyPeelR-⟪⟫`** (`proof/Preserve.preserve-TyPeelR-⟪⟫`) — the same
  ...
  (`strong-rep-store.TermSubst`): its reps by `⊢ʳ-ren`, its changes by `⊢ˢ-++`
...
`applyChanges-++` plus `applyChanges-shiftScope1` (`strong-rep-store.Boundary` §5,
`strong-rep-store.TermSubst`), and the `Peel` and scope-move rows are the theorems
...
### The evaluator (`strong-rep-store.Eval`)
...
* **The shift audit, and the `TyPeelR` split** (Jeremy, 2026-09-08; this
  ...
  moved boundary's **own** change list (`addUnbind0`, `strong-rep-store.Boundary`

module strong-rep-var.proof.Preserve where
...
-- repair and proved the same day in `strong-rep-var.proof.AddLock0` — so
-- `strong-rep-var.Preservation` exposes no parameter at all.
...
open import strong-rep-var.proof.TypeSubst
...
-- (2026-09-20, `strong-rep-var.proof.RepWeaken.cross-Λ-⊢`), using one
-- zero-bind
...
-- (2026-09-20, `strong-rep-var.proof.RepWeaken.rep-weaken-⊢`).
...
-- `strong-rep-var.notes.AddLock0Wall.no-addLock0°` refutes — it keeps the old
...
-- strong-rep-var.Examples at step 8 (measured).  No fixed
...
-- `conv-ren` (strong-rep-var.Conversion §2d) followed by
-- `strong-rep-var.proof.PeelDual.respell-⊢` — whose `reps Γ′ ≡ reps Γ` premise
-- is
...
-- because their proofs import this module.  `strong-rep-var.Preservation`
-- plugs in
...
--                `strong-rep-var.proof.PeelDual.preserve-Peel` applied to
--                `strong-rep-var.proof.RepWeaken.rep-weaken-⊢`, which proves
-- the
--                representation-only weakening `RepWeakenTyping` above.
--                `preserve-Peel` keeps its `module _ (repWeaken : …)`
--                shape; `strong-rep-var.Preservation` plugs the theorem in.
--   IdPushCase   PROVED outright —
--                `strong-rep-var.proof.MoveScope.preserve-IdPush`.
--   CancelRCase  PROVED outright, on the rule REPAIRED 2026-09-19 —
--                `strong-rep-var.proof.MoveScope.preserve-CancelR`.  The old

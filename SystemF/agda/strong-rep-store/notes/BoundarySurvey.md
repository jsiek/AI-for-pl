...
Three new files, all `--safe` clean, none in `strong-rep-store/All.agda`:

| file | what it is |
|---|---|
| `strong-rep-store/EvalLog.agda` | `event : Δ ⊢ M -→ M′ → String` — pattern-matches THE DERIVATION `stepΣ` returns, recurses through the ξ-frames, and reports the base rule with its boundary action.  `traceLog : ℕ → TCtx → Term → String` interleaves states and events.  Also `repClass`, `demoteE`/`isDemote`/`demoteCount`. |
| `strong-rep-store/Oblig.agda` | the REQUIREMENTS instrument.  `synthTy`, `obligs`, `obligLog` — bottom-up type synthesis from the TERM ALONE plus a top-down DEMAND.  **It computes no `γᵇ`, no `ρᵇ`, no `intOf`, no `⟦·⟧ᴴ`, no `dualᴳ`, and reads no context entry.** |
| `strong-rep-store/notes/probes/SurveyCorpus.agda` | the corpus: the eleven existing critical examples imported from the gauntlet/probes, six new typed configurations built and typed here, and the cross-corpus facts of §4, all refl-checked or ⊢-derived. |

Regenerate any render from the repo root:

```
scripts/render_term.sh 'c10Run' \
  'open import strong-rep-store.notes.probes.SurveyCorpus' | sed 's/\\n/\n/g'
```
...
hand-copied (`strong-rep-store/Show.agda` exists because hand-copying cost a
transcription error once already).

**Reading the names.**  `strong-rep-store.Show` names type variables in the order
...
configuration is plantable.  The crossing value is `strong-rep-store.Boundary`'s
...
This section reads `strong-rep-store/Oblig.agda`'s tables.  Nothing in them

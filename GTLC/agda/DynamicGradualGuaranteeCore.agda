...
... | N , UW₁—↠N , N⊑N′[W′] =
      ...
      let red : cast U [ c₁ ↦ d₁ ] · W —↠ᶜ cast N [ d₁ ]
          red = cast U [ c₁ ↦ d₁ ] · W
                  —→ᶜ⟨ β-↦ vU vW ⟩
                cast (U · cast W [ c₁ ]) [ d₁ ]
...
... | ⊑↦ c≤c′ d≤d′ | ⊢↦ c⦂₁ d⦂ =
  ...
  let red : cast V₁ [ c ↦ d ] · W —↠ᶜ cast (V₁ · cast W [ c ]) [ d ]
      red = cast V₁ [ c ↦ d ] · W
              —→ᶜ⟨ β-↦ vV₁ vW ⟩
            cast (V₁ · cast W [ c ]) [ d ]
            ∎ᶜ in
...
... | .(cast V [ idᶜ ★ ]) , (cast V [ idᶜ ★ ] ∎ᶜ) , N≤ =
    let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ cast (inj V′ [ A′ ]!) [ A′ `? ] ⦂ A′
...
sim {A = A} {A′ = A′}
    (⊑cast {M = M} {M′ = V′} {c = c} {c′ = G !} M⊑V′ c≤! c⦂ (⊢! g))
    (β-inj {G = G} vV′)
    with catchup vV′ M⊑V′
... | V , vV , M→V , V≤V′
    with cast-left-!-val V≤V′ vV vV′ g c≤! c⦂
... | N , V-c—↠N , N≤ =
    let prec : []⊑[] ⊢ cast M [ c ] ⦂ A ⊑ᶜᵀ cast V′ [ G ! ] ⦂ A′
        prec = ⊑cast M⊑V′ c≤! c⦂ (⊢! g) in
    let red = cast M [ c ]
              —↠ᶜ⟨ ξ* (cast□[ c ]) M→V ⟩
              cast V [ c ]
              —↠ᶜ⟨ V-c—↠N ⟩
...
... | V , vV , M→V , V≤V′
    with A⊑★
... | ⊑-★ =
    let prec : []⊑[] ⊢ M ⦂ A ⊑ᶜᵀ cast V′ [ G ! ] ⦂ A′
        prec = ⊑castR M⊑V′ (⊢! g) id≤! in
    let red = M→V in
    let prec′ : []⊑[] ⊢ V ⦂ A ⊑ᶜᵀ inj V′ [ G ]! ⦂ A′
        prec′ = star-to-inj V≤V′ vV vV′ g in
    V , red , prec′

...
| [`NuImprecisionPairedLambdaTargetClosingFrameClosingHandlersProof.agda`](NuImprecisionPairedLambdaTargetClosingFrameClosingHandlersProof.agda) | completed top-level higher-order assembly; canonical `Lemma` absent | Constructs all thirteen handler fields from thirteen exact semantic dependencies, composing matched-`Λ` and source-gen structural reductions, the normalized paired-widening source-inert core, and quotient constructor dispatchers; this is the fit-checking skeleton that prevents independently proved leaves from drifting away from the interpreter |
| [`NuImprecisionPairedLambdaTargetClosingFrameClosingAssemblyProof.agda`](NuImprecisionPairedLambdaTargetClosingFrameClosingAssemblyProof.agda) | completed final higher-order frame-closing assembly; canonical `Lemma` absent | Connects thirteen semantic-handler capabilities and five target-frame branch capabilities through the checked dispatchers and interpreter to the exact final `SourceNuPairedAllConversionPostBetaAllRevealClosingRelationFrameClosingᵀ` consumer |
...
| [`NuImprecisionPairedUniversalConversionFreshPathSquareDef.agda`](NuImprecisionPairedUniversalConversionFreshPathSquareDef.agda) | completed exact statement; proof not yet started | Names the joint path-square theorem required by the cycle proof; the remaining mathematical core is the prefix-indexed target/imprecision half-square under structural binder descent |
...
| [`NuImprecisionPairedLambdaTargetClosingPairedConversionFrameClosingProof.agda`](NuImprecisionPairedLambdaTargetClosingPairedConversionFrameClosingProof.agda) | completed handler adapter; assembly partial | Verifies exact fit of the fused theorem to `handle-frame-paired-conversion`; no recursive invocation or one-sided conversion theorem is assumed |
...
| [`NuImprecisionPairedLambdaTargetClosingUpGenAllFrameClosingProof.agda`](NuImprecisionPairedLambdaTargetClosingUpGenAllFrameClosingProof.agda) | completed GPT 5.5 handler adapter; assembly partial | Verifies direct fit to `handle-frame-up-gen-all` without using recursive frame closing |

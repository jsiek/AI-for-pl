module strong-rep-nu.notes.CancelRReachabilityWitness where
...
--     and installed in `strong-rep-nu.Reduction`.  The first nine steps are
...
-- `strong-rep-nu.Examples` §§1–7 passes a `∀Z. … ⇒ X` — an argument whose
-- polymorphic type RETURNS the abstracted variable — so no run there ever
-- produced the leaf; `strong-rep-nu.Examples` §8 is this program, merged in.
--
...
-- `[ℕ]`), which is what `strong-rep-nu.Examples` §2c already does for
...
open import strong-rep-nu.TypeCheck using (tc; int!; conv!; sq!; tr)
...
--   and then CancelR.
--
-- Rendered (strong-rep-nu.Show; α is a representation variable, X the

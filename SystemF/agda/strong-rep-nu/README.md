...
* **Consequences.**  `canon-step`/`canon-steps`
  ...
  because nothing mints `instReveal` any more; `instReveal`, its
  lemmas and the refuted record `¬CanonTyPeelR` are deleted.  The
  stacked layer costs steps:
...
| `Conversion.agda` | conversions `id` / `seal` / `unseal` / `_↦_` / `` `∀ ``, the judgment `Δ ⊢ c ∶ A ⇝ B` with NO polarity index, `mkId`, the canonical mints at a slot (`reveal`/`conceal`), the re-spelling relation `SameConv` with its uniqueness and `respell` lemmas, `conv-ren`, the inversions and `conv-types-unique` |

...
Nothing is installed: `strong-rep-store.Reduction` is untouched, and
...
`strong-rep-store.Reduction`'s `CancelR` now carries the three readings the old rule
...
`strong-rep-store.proof.MoveScope.preserve-CancelR` proves `CancelRCase` outright,
by `preserve-IdPush`'s argument. `strong-rep-store.Preservation.Stage1` and
`strong-rep-store.TypeSafety.Stage1` no longer take a `cancel` parameter. The

...
the rule performs (`strong-rep-var.TermSubst.interior-addLock0-cross`):
...
| `preservation-TyPeelR-Λ`, `preservation-TyPeelR-⟪⟫` | `strong-rep-var.Preservation` | the public per-rule statements |
| `det` | `strong-rep-var.Reduction` | **determinism** over the whole rule set.  The two `TyPeelR` patterns are disjoint (a `Λ` is not a boundary); the wrapper case needs `conv-src-unique`, exactly as the old rule did, and the Λ case needs nothing |

...
strong-rep-nu.notes.CancelRReachabilityWitness'` (α, β, γ are representation
...
Nothing is installed: `strong-rep-nu.Reduction` is untouched, and
...
`strong-rep-nu.proof.MoveScope.preserve-CancelR` proves `CancelRCase` outright,
by `preserve-IdPush`'s argument. `strong-rep-nu.Preservation.Stage1` and
`strong-rep-nu.TypeSafety.Stage1` no longer take a `cancel` parameter. The

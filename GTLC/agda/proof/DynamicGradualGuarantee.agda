...
cast-value-catchup : ∀ {V A B c}
  ...
  → [] ⊢ᶜ V ⦂ A
  → ⊢ c ⦂ A ⇨ B
  → ∃[ N ] ((cast V [ c ] —↠ᶜ N) × ((Valueᶜ N ⊎ Blameᶜ N) × ([] ⊢ᶜ N ⦂ B)))
...
value-right-catchup vV (⊑blameR {ℓ = ℓ} M⦂A A⊑A′) =
  blame {ℓ = ℓ} , (blame {ℓ = ℓ} ∎ᶜ) , inj₂ (ℓ , refl)
...
blame-—↠ᶜ-value-impossible : ∀ {V ℓ} → Valueᶜ V → blame {ℓ = ℓ} —↠ᶜ V → ⊥
blame-—↠ᶜ-value-impossible vV (blame {ℓ = _} ∎ᶜ) = value-not-blame vV
blame-—↠ᶜ-value-impossible vV (blame {ℓ = _} —→ᶜ⟨ blame→N ⟩ N—↠V) =
  ⊥-elim (blame-irreducible blame→N)

blame-—↠ᶜ-refl : ∀ {N ℓ} → blame {ℓ = ℓ} —↠ᶜ N → N ≡ blame {ℓ = ℓ}
blame-—↠ᶜ-refl (blame {ℓ = _} ∎ᶜ) = refl
blame-—↠ᶜ-refl (blame {ℓ = _} —→ᶜ⟨ blame→N ⟩ N—↠N′) =
  ⊥-elim (blame-irreducible blame→N)
...
... | W′ , M′—↠W′ , inj₂ (ℓ″ , refl) =
  blame {ℓ = ℓ″}
  , (cast M′ [ ⊥ᶜ A₁ ⇨ B₁ at ℓ′ ]
       —↠ᶜ⟨ ξ* (cast□[ ⊥ᶜ A₁ ⇨ B₁ at ℓ′ ]) M′—↠W′ ⟩
     cast (blame {ℓ = ℓ″}) [ ⊥ᶜ A₁ ⇨ B₁ at ℓ′ ]
       —→ᶜ⟨ ξ-blame (cast□[ ⊥ᶜ A₁ ⇨ B₁ at ℓ′ ]) ⟩
     blame {ℓ = ℓ″}
     ∎ᶜ)
  , ⊑blameR {ℓ = ℓ″}
      ⊢blame

...
| [`NuImprecisionLeftRuntimeRenameDef.agda`](NuImprecisionLeftRuntimeRenameDef.agda) | completed flat statement; proof in progress | Mutually transports ordinary and quotiented QTI derivations across a structural left insertion when the source is no-bullet and the target is runtime-safe. Both fields return relation derivations directly; source-allocation index normalization remains an assembly boundary rather than a result wrapper. |

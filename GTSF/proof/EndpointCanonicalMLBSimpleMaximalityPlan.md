...
`enumMLB-νν-complete-elim` for the paired-`∀` route and five source-`ν`/`ν`
clauses for one-sided `∀` and arrow endpoint shapes.  The variable and base
clauses are impossible, while the `★`/`★` clause follows from
`StarMeetCtxᵢ`.  The module has no remaining postulates; these six obligations
are explicit holes.
...
The corrected `ννRouteCover` carries the whole-source comparison

```agda
idᵢ Δᶜ ∣ Δᶜ ⊢ `∀ D ⊑ `∀ R ⊣ Δᶜ
```

instead.  This comparison may end in `ν`, as it does for the counterexample;
forcing it to end in `∀ⁱ` loses valid lower bounds.  The conditional theorem
`νν-route-cover-complete` now consumes this corrected evidence and produces
real `enumMLB` membership.

The remaining proof should normalize the pair of lower-bound derivations while
carrying this whole-source comparison and any binder-swap or unused-binder
equation.  This keeps binder normalization out of the list and deduplication

module strong-rep-store.All where
...
open import strong-rep-store.Progress
open import strong-rep-store.TypeSafety

-- THE LIVING REGRESSION AND THE RENDERER (ported 2026-09-19, closing the
-- frontier).  `strong-rep-store.Examples` is closed programs, their runs, their
-- typings and the refutations that still hold, stated through
-- `TypeCheck.agda`/`Eval.agda` rather than by hand-written boundary
-- derivations; `strong-rep-store.Show` renders de Bruijn terms with names,
-- printing
...
open import strong-rep-store.proof.Residual
open import strong-rep-store.Show
...
open import strong-rep-store.proof.Adversary
...
open import strong-rep-store.proof.IdLayer
...
open import strong-rep-store.proof.Canonicity
...
-- transport lemmas of strong-rep-store.Boundary §3a plus the observation that
-- every
...
open import strong-rep-store.proof.ShiftAudit

-- The checked notes come last because CancelRShiftWall reaches MoveScope.
open import strong-rep-store.notes.All

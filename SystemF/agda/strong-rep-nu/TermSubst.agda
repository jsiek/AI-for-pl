module strong-rep-nu.TermSubst where
...
--     private definition is strong-rep-nu.proof.TermSubst, which
--     KEEPS the section numbers its material had here.  Do not
--     renumber either file.  `extN` (strong-rep-nu.Ctx §8) and
--     `renᶠᴿ`/`renᴮᴿ` (strong-rep-nu.Boundary §2/§3) are one layer
...
-- lambda, so that strong-rep-nu.Residual can cite the same one.

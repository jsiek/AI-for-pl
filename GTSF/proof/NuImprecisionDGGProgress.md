...
| [`NuImprecisionWorldCoherentRightValueCatchupRuntimeNoBulletTransportDef.agda`](NuImprecisionWorldCoherentRightValueCatchupRuntimeNoBulletTransportDef.agda) | completed repaired flat statement; proof partial | Transports an active source sibling and no-bullet target sibling directly through a completed right-value catch-up. The proof now splits through `AtMostOne•`: the zero branch uses ordinary weakening, while the active branch carries only the completed source-store equality and derives every recursive typing from QTI. Bullet, allocation-prefix, application, and primitive cases are checked locally; `ν`, cast, quotient-up, and explicit impossible cases remain. No runtime-transport result carrier is introduced. |

TODO:
  ...
  Resolve the paradox of ⊒⟨ν⟩.
  (1) We need the ⊒⟨ν⟩ rule to show that Left ν Widening holds.
      To use it in this context, we require V ⟨να.s[α]⟩ to be
      derivable from *any* s[α].
  (2) Once we have the ⊒⟨ν⟩ rule, it is a possible way to derive
      the relation in Catchup.  To use it in this context, we
      require V ⟨να.s[α]⟩ to be derivable only when V ⟨s[α]⟩
      is a value.
  Currently (2) fails, because V ⟨να.(★→★)?;(!α→?α)⟩ : ★ ⊒ ∀X.X→X
  is legitimate, but V ⟨(★→★)?;(!α→?α)⟩ is not a value.

  Look at reduction sequences for

     I^★ ⟨να.α!→α?⟩ ⟨ν̅α.α♯→α⟩ ⊒ I

  and variants to see whether structural rules can be eliminated or
  simplified.  In particular, try to give a more refined theory
  of term narrowing for casts over σ.

  Work out how to prove that λν^⊒ is simulated by PolyC^ν.
...
I was delegated to think about how to make the (split) and (extend) rules
easier to use in formal proofs.

The key point is that a type name on the left that matches with a type
name on the right might higher up in the derivation need to match with a
different type name on the right. On reflection, it's not clear to me that
(extend) is an appropriate rule, even if it weren't for issues in reasoning.
In our type derivations, type names are introduced by ν bindings and never
disappear as one moves up the tree, so γ bindings should behave similarly.

I thought that one way to deal with the issue might be that γ bindings
could associate a type name on the left to a different type name on the
right, but that doesn't fit well with out use of id_α in narrowings to
relate associated variables. Nor does it fit well with our de Bruijn
approach to the formalisation.

Instead, I suggest that we allow an arbitrary permutation of contexts:

   π : Γ ↝ Δ

means that Γ permutes to Δ (it must be the case that both Γ and Δ are
well-formed, so one cannot permute the introduction of a type name
past it's use).  Permutations act on types, terms, and coercions to
map the type variables, type names, and term variables they contain.

   If π : Γ ↝ πΓ  and  Γ ⊢ M : A  then  πΓ ⊢ πM : πA
   If π : Γ ↝ πΓ  and  s : A ⇒ B  then  πs : πA ⇒ πB

We also define a permutation of the relation between two contexts,
writing

   π : γ ↝ δ  iff  γ : Γ ⊒ Γ′, δ : Δ ⊒ Δ′, π₁Γ = Δ, π₂Γ′ = Δ′.

And then we add a permutation rule:

   γ ⊢ M ⊒ M′ : p
   ------------------ π : γ ↝ πγ  (permute)
   πγ ⊢ πM ⊒ πM′ : πp

Note we can reverse the hypothesis and conclusion if that's easier.


This rule should play well with all the other rules, in that one
can always push a permutation through.  For instance,
...
And similarly for every other rule.

What do you think? I'm about to leave to give a talk at IMPA,
but tomorrow I'll go through the examples and see how this
plays out. Go well, -- P
...
I’ve got the p,t,r in the following contexts (simplified a bit):

Γ ⊢ p : A ⊒ A′
Γ′ ⊢ t : A′ ⊒_Σ′ B
Γ ⊢ r : A ⊒_Σ′ B

So it seems like p need to be transported to Γ′ before it can be composed with t,
Because compose has this type:

(p ⨟ q) : ∀{Γ A B}(Γ ⊢ A ⊒ B) → (Γ ⊢ B ⊒ C) → (Γ ⊢ A ⊒ C)

---

Can I construct a problematic example? If I can't that will give me a
hint as to how to solve the problem.
...
The key point is p and q. What guarantees that if the source is
well-formed in Γ that the target is well-formed in Γ′?

Claim. If γ : Γ ⊒ Γ′ and Γ ⊢ p : A ⊒ B and Γ′ ⊢ B then Γ′ ⊢ p : A ⊒ B.

This would be falsified given α ∈ dom(Γ), α ∉ dom(Γ′)
such that Γ ⊢ p : A ⊒ B and α ∈ fv(A) and α ∉ fv(B).
However, by the nature of narrowing, the candidate arrows
with α are id_α : α→α and α? : ★→α.


------------------------------------------------------------------------
One important property of Atsushi's system is that gradual polymorphism
is "pay as you go". If you only use parametric polymorphism and no graduality,
then type substitution suffices and one never needs to created fresh seals.
That's harder to achieve in our system, because the rules for ν and ν̅
in narrowing and widening need to assume that forall types are always
instantiated at α with a widening to take B[α] to B[A].

I conjecture we can restore Atsushi's benefit at the cost of having
two forms of forall corresponding to the two forms of Λ in Atsushi's
system.

  type    A,B,C  ::=  ι | α | X | ★ | A→B | ∀X.B[X] | ∀α.B[α]
...
Type rules
  ...
  Γ,X ⊢ V[X] : B[X]        Γ ⊢ L : ∀X.B[X]    Γ ⊢ A
  ---------------------    ------------------------
  Γ ⊢ ΛX.V[X] : ∀X.B[X]    Γ ⊢ L∙A : B[A]

  Γ,α ⊢ V[α] : B[α]
  ---------------------
  Γ ⊢ Λα.V[α] : ∀α.B[α]

  Γ ⊢ L : ∀α.B[α]  s̅[α] : B[α] ⊑_{Σ,α:=A} C
  -----------------------------------------
  Γ ⊢ να:=A. L·α ⟨s̅[α]⟩ : C
...
Reductions
  ...
  (Λα.V[α])·α      ⊢→  V[α]
  V (ν̅X.s̅)         ⊢→  V ★ ⟨s̅⟩
  V (ν̅α.s̅[α])      ⊢→  να:=★. V α ⟨s̅[α]⟩
  (V ⟨να.s[α]⟩)·α  ⊢→  V ⟨s[α]⟩


Converting between ∀'s.

  if L : ∀α.1→B[α] then (ΛX.λ().να:=X. L·α ⟨s̅[α]⟩) : ∀X.1→B[X].
     where  s̅[α] : B[α] ⊑_{α:=X} B[X]

  if L : ∀X.1→B[X] then (Λα.λ(). L∙α) : ∀α.1→B[α]


------------------------------------------------------------------------
Write s ≈ t if s : A ⊒_Σ B and t : A ⊒_Σ′ B, that is, if s and t
have the same endpoints regardless of store. Unlike before, we don't
require that Σ and Σ′ are related by some σ.

Now the rules are as follows, where (crucially!) we change our convention.
Narrowings s and t may be over arbitrary stores, but p, q, r are over
the empty store. (Previously, r was over an arbitrary store.)
...
Does this still permit the same derivations as previously?  It does so
long as for every r_old : A ⊒_Σ B we can find an r_new : A ⊒_∅ B.
This is true if every entry in Σ is of the form α:=C where C only has
free type names, but false if C may contain free type variables.  For
instance, before the following relation held, but now it does not.
   ...
   q  =  id_α
   r_old  =  α♯
   r_new  =  (there is no narrowing X ⊒_∅ α)

Does it matter if we lose these? If it does matter, I suppose we
could bring back the compound rules,
...
------------------------------------------------------------------------
Good point, looks like we still have uniqueness. So that's one less problem.

There remains the issue that we cannot claim that (V ⟨να.s[α]⟩) α —→ (V ⟨s[α]⟩)
always yields a value, which is important in the proof of Left ν Widening.
One way forward might be to return to the rule that generalises ⊒Λ and ⊒⟨ν⟩,
namely
...
Advantage: we no longer need a corner case ruling out (να.α?).
Disadvantage: induction over derivations may become trickier.
...
By sanity, fv(A) = fv(B) \ dom(Σ′) and fv(A′) = fv(B) \ dom(Σ′).
Hence, p : A ⊒_∅ A′ implies p : A ⊒_Σ′ A′.
------------------------------------------------------------------------
Jeremy and Peter,

Actually, I think Peter has stepped into a more serious problem;
I encountered a similar problem earlier today.  The problem is this:
we have rules for type application on both sides and type application
on the right:

    γ ⊢ L ⊒ L′ : ∀X.p[X]           γ ⊢ L ⊒ L′ : να.p[α]
    ---------------------------    -------------------------
    γ, α:=q ⊢ L α ⊒ L′ α : p[α]    γ, α:=A ⊢ L ⊒ L′ α : p[α]

But there is no rule for a type application on the left. Indeed,
it's hard to think of what such a rule would look like. A sketch
of it would be:
...
But then we need p : ∀X.A[X] ⊒ B and q : A[α] ⊒ B and there are
no narrowings of either of these types.

The lack of such a rule makes it difficult to see how we could
ever prove

  ∅ ⊢ να:=★.(ΛX.λx:X.x) α ⟨ α♯→α♭ ⟩ ⊒ (ΛX.λx:X.x) : (να.α!→α?)

Here is a more complete example where the same problem arises
(see Example 1 in cambridge25):
     ...
     Here +⊒ holds by (i) since \dual{ν̅α.α♯→α♭} = (να.α!→α?)
...
One possible solution is to allow steps on both sides in the simulation.

  If γ ⊢ L ⊒ L′ : p and L —→_Π M then there exist reductions
  M —↠_Σ N and L′ —↠ N′ such that N ⊒ N′ : p.

I'm wondering if another way forward is to combine multiple reduction
steps into a single reduction, along lines that we considered earlier
when we were discussing alternative formulations for ν∀^⊒:

  V α ⊢↠ W
  -----------------------------------
  E[να:=A.V α ⟨s̅⟩] —→_{α:=A} E[W ⟨s̅⟩]

This rule is more complex, but it does have a key advantage:
we no longer need runtime type applications at all! I was wondering
about adopting such a rule anyway, because I've encountered serious
issues in trying to establish that New's system simulates ours,
and I think adopting the rule above might help.

We might also need a second evaluation rule.

  V α ⊢↠ W
  ------------------------------------
  E[V ⟨ν̅α.s̅[α]⟩] —→_{α:=★} E[W ⟨s̅[α]⟩]

I'm not sure if this approach fixes the problem or if it's the best
way forward. Let's ponder, and discuss tomorrow!
...
I think it can be dealt with as follows. In the discussion below I will
just discuss narrowing, widening is handled via duality.

To make the problem tractable, let's add one more assumption, that when
a narrowing is typed over a store that the store has at most one
assignment. That's a reasonable restriction. If we need more than one
assignment, we can use more than one cast.

Semicolon appears in only two narrowings: untag and seal. If it's in
(G?;g) then we know G? is over the empty store. If it's in (p₀;α#),
then we know p₀ is over the empty store (which is why I write p₀
instead of s).

Let's consider the first, as the argument for the second is similar.
If we have a derivation, it must be of the form
...
and after the breakup it will be of the form
...
I think all the other cases work out similarly.

So I suspect it can be coped with, although admitedly it makes
some of the proofs quite finicky. Some of the cases can
perhaps be merged if one talks of c and d in general,
assuming that one of the two is over the empty store
and the other is over a store with at most one assignment.
...
Uh oh. In the draft paper I had a mapping of ν∀^⊒ into itself that
compiled out all coercions except for G!, G?, α♯, α♭, which it is easy
to show is a simulation. I claimed that it would be easy to adapt this
to a map of ν∀^⊒ into PolyC^ν that we could show is a simulation.

That claim turns out to be false. The problem is that PolyC^ν has
no construct analogous to the ``runtime'' type application (V α).
We cannot use the (let x = M[X≅A] in N) construct because that needs
to have a type that doesn't mention X, whereas (V α) has a type that
does mention α.

I think there's a fix. We change the runtime type application
to also include a list of coercions.

    Γ ⊢ V : ∀X.B[X]    c₁ : B[α] =⇒_Σ₁ C₁[α]  ...  cₙ : Cₙ₋₁[α] =⇒_Σₙ Cₙ
    -------------------------------------------------------------------- α ∉ fv(Cₙ)
    Γ, α:=A ⊢ V α ⟨c₁⟩ ... ⟨cₙ⟩ : Cₙ

and then we map

    ⟦V α ⟨c₁⟩ ... ⟨cₙ⟩⟧ =  let x = ⟦V⟧[X≅α] in ⟦cₙ⟧[X/α] ( ... (⟦c₁⟧[X/α] x)...)

Let me confirm this works. Here is the update to the
reduction rules for ν∀^⊒.

    (ΛX.V[X]) α ⟨c₁⟩ ... ⟨cₙ⟩      ⊢→  V[α] ⟨c₁⟩ ... ⟨cₙ⟩
    (V ⟨∀X.c[X]⟩) α ⟨c₁⟩ ... ⟨cₙ⟩  ⊢→  V α ⟨ c[α] ⟩ ⟨c₁⟩ ... ⟨cₙ⟩
    (V ⟨να.c[α]⟩) α ⟨c₁⟩ ... ⟨cₙ⟩  ⊢→  V ⟨ c[α] ⟩ ⟨c₁⟩ ... ⟨cₙ⟩

    -----------------------------------------------
    E[να:=A.V α ⟨s̅[α]⟩] —→_{α:=A} E[V α ⟨s̅[α]⟩]

Here are the releavant translation rules (in addition to the new one above).

    ⟦∀X.c[X]⟧  =  (λf.ΛX. let x = f{Y≅X}; ⟦c[X]⟧[Y/X] x)
    ⟦να.s[α]⟧  =  (λf.ΛX. ⟦s[α]⟧[X/α] f)
    ⟦ν̅α.s̅[α]⟧  =  (λf. let x = f{X≅?}; ⟦s̅[α]⟧[X/α] x)

    ⟦να:=A. L α ⟨s̅[α]⟩⟧  =  (let x = L{X≅A}; ⟦s̅[α]⟧[X/α] x)

And here is the update to the proof of simulation.


  (ΛX.V[X]) α ⟨c₁⟩ ... ⟨cₙ⟩  ⊢→  V[α] ⟨c₁⟩ ... ⟨cₙ⟩
        Σ ▷ let x = (ΛX.⟦V[X]⟧)[X≅α] in ⟦cₙ⟧[X/α] ( ... (⟦c₁⟧[X/α] x)...)
    —→  Σ, σ:α ▷ ⟦cₙ⟧[X/α] ( ... (⟦c₁⟧[X/α] ⟦V[α]⟧[σ/α])...)

  (V ⟨∀X.c[X]⟩) α ⟨c₁⟩ ... ⟨cₙ⟩  ⊢→  V α ⟨c[α]⟩ ⟨c₁⟩ ... ⟨cₙ⟩
        Σ ▷ let x = (λf.ΛY. let y = f{Z≅Y}; ⟦c[Z]⟧ y)⟦V⟧{X≅α} ; ⟦cₙ⟧[X/α] ( ... (⟦c₁⟧[X/α] x)...)
    —→  Σ ▷ let x = (ΛY. let y = ⟦V⟧{Z≅Y}; ⟦c[Z]⟧ y){X≅α} ; ⟦cₙ⟧[X/α] ( ... (⟦c₁⟧[X/α] x)...)
    —→  Σ, σ:α ▷ ⟦cₙ⟧[σ/α] ( ... (⟦c₁⟧[σ/α] (let y = ⟦V⟧{Z≅σ}; ⟦c[Z]⟧ y))...)
    ≃   Σ, σ:α ▷ let y = ⟦V⟧{Z≅σ}; ⟦cₙ⟧[σ/α] ( ... (⟦c₁⟧[σ/α] (⟦c[Z]⟧ y))...)

  V ⟨να.s[α]⟩ α ⟨c₁⟩ ... ⟨cₙ⟩  ⊢→  V ⟨s[α]⟩ ⟨c₁⟩ ... ⟨cₙ⟩

So it doesn't go through immediately, we need one observational equivalence rule

But all of this is making me wonder if we actually need ⟨c₁⟩ ... ⟨cₙ⟩ since they
appear to move to the outside of the binding anyway! NEED BETTER UNDERSTANDING.


ANOTHER WAY TO REFORMULATE THE RULE
...
Then we need to show that V α ⊢↠ W in ν∀^⊒ implies

  Σ ▷ E[let x = ⟦V⟧{X≅A} in ⟦s̅⟧ x] —→^+ Σ,σ:A,Σ′ ▷ E[⟦s̅⟧ ⟦W⟧]

in PolyC^ν. (Possibly Σ′ is always empty?)

Let's try to show this.

An example first, to gain insight.

       (νβ:=ι. ((λx:★.x) ⟨να.α!→α?⟩) β ⟨β♯→β♭⟩
    —→_{β:=ι}
       (λx:★.x) ⟨β!→β?⟩ ⟨β♯→β♭⟩

    =  ∅ ▷ let x₁ = ((λf₁.ΛX.(λf₂.λx₂.(λf₃.⟨tag_X⟩↓f₃)(f₂((λf₄.⟨tag_X⟩↑f₄)x₂)))f₁)(λx:★.x)){Y≅ι};
             (λf₅.λx₃.(λf₆.unseal_Y f₆)(f₅((λf₆.seal_Y f₆)x₃)))x₁
    —→
       ∅ ▷ let x₁ = (ΛX.(λf₂.λx₂.(λf₃.⟨tag_X⟩↓f₃)(f₂((λf₄.⟨tag_X⟩↑f₄)x₂)))(λx:★.x)){Y≅ι}
             (λf₅.λx₃.(λf₆.unseal_Y f₆)(f₅((λf₆.seal_Y f₆)x₃)))x₁
    —→
       σ:ι ▷ let x₁ = ((λf₂.λx₂.(λf₃.⟨tag_σ⟩↓f₃)(f₂((λf₄.⟨tag_σ⟩↑f₄)x₂)))(λx:★.x))
               (λf₅.λx₃.(λf₆.unseal_σ f₆)(f₅((λf₆.seal_σ f₆)x₃)))x₁
    —→
       σ:ι ▷ let x₁ = (λx₂.(λf₃.⟨tag_σ⟩↓f₃)((λx:★.x)((λf₄.⟨tag_σ⟩↑f₄)x₂)))
               (λf₅.λx₃.(λf₆.unseal_σ f₆)(f₅((λf₆.seal_σ f₆)x₃)))x₁
    —→
       σ:ι ▷ (λf₅.λx₃.(λf₆.unseal_σ f₆)(f₅((λf₆.seal_σ f₆)x₃)))
               (λx₂.(λf₃.⟨tag_σ⟩↓f₃)((λx:★.x)((λf₄.⟨tag_σ⟩↑f₄)x₂)))
    ←—
       σ:ι ▷ (λf₅.λx₃.(λf₆.unseal_σ f₆)(f₅((λf₆.seal_σ f₆)x₃)))
               ((λf₇.λx₂.(λf₃.⟨tag_σ⟩↓f₃)(f₇((λf₄.⟨tag_σ⟩↑f₄)x₂)))(λx:★.x))

So it is not quite a simulation. Damn!

If we treat all of the f's as meta-level bindings then it may work out.

       ⟦id_A⟧f     =  f
       ⟦c;d⟧f      =  ⟦d⟧(⟦c⟧f)
       ⟦c→d⟧f      =  ⟦d⟧(f(⟦c⟧x))
       ⟦∀X.c[X]⟧f  =  ΛX.(let x = f{Y≅X}; ⟦c[Y]⟧x)
       ⟦να.c[α]⟧f  =  ΛX.⟦c[X]⟧f
       ⟦G!⟧f       =  ⟨tag_G(G)⟩↑f
       ⟦G?⟧f       =  ⟨tag_G(G)⟩↓f
    ...
    —→_{β:=ι}
       ...
       ∅ ▷ let x₁ = (ΛX₁.λx₂.⟨tag_X₁(X₁)⟩↓((λx:★.x)(⟨tag_X₁(X₁)⟩↑x₂))){X₂≅ι};
           (λx₃.unseal_X₂(x₁(seal_X₂(x₃))))
    —→
       β:ι ▷ let x₁ = (λx₂.⟨tag_β(β)↓((λx:★.x)(⟨tag_β(β)⟩↑x₂))))
             (λx₃.unseal_β(x₁(seal_β(x₃))))
    —→
       (λx₃.unseal_β((λx₂.⟨tag_β(β)↓((λx:★.x)(⟨tag_β(β)⟩↑x₂))))(seal_β(x₃))))

So at least this one example works.

       (νβ:=ι. ((λx:★.x) ⟨να.α!→α?⟩ ⟨∀X.id_X→id_X⟩) β ⟨β♯→β♭⟩
    —→_{β:=ι}
       (λx:★.x) ⟨β!→β?⟩ ⟨id_β→id_β⟩ ⟨β♯→β♭⟩

       ∅ ▷ let x₁ = (ΛX₃.let x₄ = (ΛX₁.λx₂.⟨tag_X₁(X₁)⟩↓((λx:★.x)(⟨tag_X₁(X₁)⟩↑x₂))){X₄≅X₃}; x₄){X₂≅ι};
           (λx₃.unseal_X₂(x₁(seal_X₂(x₃))))
    —→
       β:ι ▷ let x₁ = (let x₄ = (ΛX₁.λx₂.⟨tag_X₁(X₁)⟩↓((λx:★.x)(⟨tag_X₁(X₁)⟩↑x₂))){X₄≅β}; x₄);
             (λx₃.unseal_β(x₁(seal_β(x₃))))
    —→
       β:ι,α:β ▷ let x₁ = (let x₄ = (λx₂.⟨tag_α(α)⟩↓((λx:★.x)(⟨tag_α(α)⟩↑x₂))); x₄);
             (λx₃.unseal_β(x₁(seal_β(x₃))))
    —→
       β:ι,α:β ▷ (λx₃.unseal_β((λx₂.⟨tag_α(α)↓((λx:★.x)(⟨tag_α(α)⟩↑x₂))))(seal_β(x₃))))

So it doesn't quite work, because the extra α:β binding gets in the way of a complete simulation.
(Actually, looking back I see the exactly same problem occurs when interpeting my translation of
ν∀ into ν∀ as a simulation. A possible fix is to mark and then collapse dummy α:=α (equivalently,
σ:σ) assignments that occur during the simulation.)



CAN WE ALSO REDUCE TO Ahmed et al. 2017? That would require showing

  Σ ▷ ⟦s̅⟧ (⟦V⟧ A) —→^+ Σ,α:=A ▷ ⟦t̅⟧ (⟦W⟧ : B[α] =⇒^{+α} B[A])

where s̅ = φ̅ ⨾ t̅ and φ̅ : B[α] ⊑ B[A], which may be tricky.

------------------------------------------------------------------------
Relating ν∀=⇒ to ν∀⊒


Conjecture. If ∅ ⊢ M : A and M —↠_Σ V in ν∀⊒ then
∅ ⊢ M : A and M —↠_Σ′ V$ in ν∀=⇒, where Σ′ is a permutation of Σ.

(a) How to prove the above?
(b) How to adjust it to deal with unbounded reductions sequences?

Let E[M] be a redex in ν∀⊒.  The corresponding redex of ν∀=⇒ is computed
as follows.
...
We also compute a store from the evaluation context.
...
Then M ⊢→ N in ν∀⊒ implies |E|[M] —→_∅ |E|[N] in ν∀=⇒.


Conjecture 2.
(i) If L —↠_Σ V in ν∀⊒ then L —↠_Σ′ V in ν∀=⇒, where Σ and Σ′ are permutations.
(ii) If L —↠_Σ E[M] and M ⊢→ N in ν∀⊒ then L —↠_Σ′ |E|[M′] in ν∀=⇒,
where Σ, ∥E∥ and Σ′ are permutations. (Not clear how M and M′ relate.)


Here's a possible way to model simulation of ν∀⊒ in ν∀=⇒ step-by-step.

A configuration consists of a pair of a reduction L —↠_Σ M in ν∀⊒
and a corresponding reduction L —↠_Σ′ M′ in ν∀=⇒.

...



I can think of two ways to structure the proof. Not clear which is better.

(a) M —↠_Σ V in ν∀⊒ implies M —↠_Σ V in ν∀=⇒.
A sequence of reductions in ν∀⊒ maps to a sequence in ν∀=⇒.


(b)  Look at the possible steps in ν∀⊒.
     Each implies a corresponding (optional) step in ν∀=⇒.

(i)    M ⊢→ N                     M ⊢→ N
       --------------   becomes   ------------------
       E[M] —→_∅ E[N]             |E|[M] —→_∅ |E|[N]

(ii)   -----------------------------------------  becomes no step on  E[(Vα)⟨s̅[α]⟩]
       E[να:=A.Vα⟨s̅[α]⟩] —→_{α:=A} E[(Vα)⟨s̅[α]⟩]

(iii)  -----------------------   becomes   -------------------------
       E[blame ℓ] —→_∅ blame ℓ             |E|[blame ℓ] —→_∅ blame ℓ


M ⊢→ N in ν∀⊒ implies |E|[M] —→_∅ |E|[N] in .
    E[να:=A.Vα⟨s̅[α]⟩] —→_{α:=A} E[(Vα)⟨s̅[α]⟩ in ν∀⊒
      implies |E|[

------------------------------------------------------------------------
Checking what happens with fewer restrictions

  I ⟨ ν̅α.α♯→α♭ ⟩ κ✯  —↠_{α:=✯}  κ✯ ⟨ α♯ ⟩ ⟨ α♭ ⟩  —→  κ⋆
  I ⟨ ν̅α.α?→α♭ ⟩ κ✯  —↠_{α:=✯}  κ✯ ⟨ α? ⟩ ⟨ α♭ ⟩  —→  blame
  I ⟨ ν̅α.α♯→α! ⟩ κ✯  —↠_{α:=✯}  κ✯ ⟨ α♯ ⟩ ⟨ α! ⟩  unusable, since there is no way to apply ⟨ α? ⟩ or ⟨ α♭ ⟩
  I ⟨ ν̅α.α?→α! ⟩ κ✯  —↠_{α:=✯}  κ✯ ⟨ α? ⟩ ⟨ α! ⟩  —→  blame

Minor curiousity: Is there any way we can guarantee unusable values like the above don't arise?

  (να:=ι. (I✯ ⟨ να.α!→α? ⟩) α ⟨ α♯→α♭ ⟩)  κ  —↠_{α:=ι}  I✯ (κ ⟨ α♯ ⟩ ⟨ α! ⟩) ⟨ α? ⟩ ⟨ α♭ ⟩  ⊢↠  κ
  the other four combinations are illegal

So we don't need to restrict ν̅ but must restrict ν.

             (να:=✯. (K✯ ⟨ νβ.β!→α♭→β? ⟩) α ⟨ α♯→α♯→α♭ ⟩) κ✯ κ′✯
  —↠_{α:=✯}  (K✯ ⟨ α!→α♭→α? ⟩ ⟨ α♯→α♯→α♭ ⟩) κ✯ κ′✯

This isn't a problem. It violates unique coercions, but we don't care.
...
d          (α♯;p) in the same imprecision judgement.
...
να:=ℕ. (((ΛX. (λx:X. 0) ⟨ id_X → α♯ ⟩) α) ⟨ α♯ → α♯ ⟩)
-->
να:=ℕ. (((λx:α. 0) ⟨ id_α → α♯ ⟩) ⟨ α♯ → α♯ ⟩)
...
to a polymorphic type:
    ...
    id★  =  λx:★.x
    id   =  ΛX.λx:X.x
    id′  =  ΛX.λx:X.(id★ ⟨ να.α!→α?} ⟩) X x
         =  ΛX.λx:X.να:=X. ((id★ ⟨ να.α!→α?} ⟩) α ⟨ α♯→α♭ ⟩) x

But it can also be done with explicit tagging and sealing:

    id″ = ΛX.λx:X.να:=X. (id★ ⟨ α!→α! ⟩ ⟨ α♯→α♯ ⟩)
...
Example (f).
       ...
       ∅ ⊢ I★ ⟨να.α!→α?⟩ ⊒ I : (∀X.id_X→id_X)

Example (g).

       ∅ ⊢ I★ ⟨να.α!→α?⟩ ⟨ν̅α.α♯→α⟩ ⊒ I : (∀X.id_X→id_X)
...
Example 4.
    ...
    —→
      ...
      A different derivation avoiding split and extend:
...
Example 5. Example where the term on the right fails.
...
Example 12. Up and then down.
  ...
  —↠
   ...
   This example makes clear why we need αᵢ:=☆ bindings.
    ...
    α:=ι? ⊢ (ƛx:α.x) ⟨ α♯→α♭ ⟩ ⊒ (ƛx:α.x) : α!→α?
    -------------------------------------------------- (split)
    α:=☆, β:=ι ⊢ (ƛx:α.x) ⟨ α♯→α♭ ⟩ ⊒ (ƛx:β.x) : β!→β?
    --------------------------------------------------------- (extend)
    α:=☆, β:=id_ι ⊢ (ƛx:α.x) ⟨ α♯→α♭ ⟩ ⊒ (ƛx:β.x) : β!→β?
    ------------------------------------------------------------------- (-⊒) (ii)
    α:=☆, β:=id_ι ⊢ (ƛx:α.x) ⟨ α♯→α♭ ⟩ ⟨ β!→β? ⟩ ⊒ (ƛx:β.x) : id_β→id_β
    --------------------------------------------------------------------------------------- (+⊒+)
    α:=☆, β:=id_ι ⊢ (ƛx:α.x) ⟨ α♯→α♭ ⟩ ⟨ β!→β? ⟩ ⟨ β♯→β♭ ⟩ ⊒ (ƛx:β.β) ⟨ β♯→β♭ ⟩ : id_ι→id_ι
...
Example 18. Constant function, up on the right.
    ...
    ∅ ⊢ (K ⟨ ν̅α.ν̅β.α♯→β♯→α♭ ⟩) 42★ 69✯ ⊒ K ι ι 42 69 : ι?
  ~>
...
Example 18b. Constant function, down on the left.

K✯ = λx:✯.λy:✯.x

    ∅ ⊢ K ι ι κ κ′
      ⊒ (K✯ ⟨ να.νβ.α!→β!→α? ⟩) ι ι κ κ′ : ι?
...
This is not quite a simulation! The problem is the redundant (α:=α) assignments.
I need some way of recognising and treating these as redundant.
...
## Narrowing and Widening
  ...
  narrowing         s,t  ::=  g | id_★ | να.s[α] | G?;g | G? | s;α♯ | α♯
  cross widening    g̅,h̅  ::=  id_α | id_X | id_ι | s→t̅ | ∀X.s̅[X]
  widening          s̅,t̅  ::=  g̅ | id_★ | ν̅α.s̅[α] | g̅;G! | G! | α♭;s̅ | α♭
...
## Composition for narrowing.
     ...
     f,g,h  ::=  id_α | id_X | id_ι | s̅→t | ∀X.s[X]
     i,j    ::=  g | να.t
     r,s,t  ::=  i | G?;i | id_★ | s;α♯
...
Observe that (V ⟨ να.s[α] ⟩) is a value. The redex,
V ⟨ s[α] ⟩, is very nearly a value, with one exceptional
corner case.

Consider the possibilities for s[α]. It will be one of
...
It cannot be id_a or α♯, because s[α] : B ⊒ A[α] and with α ∈ A[α].

For all of these, V ⟨ s[α] ⟩ is itself a value, with the sole
exception being the case α?. This can arise only from:

    (V ⟨να.α?⟩) α ⊢→ V ⟨α?⟩

Here V : ★ and (να.α?) : ★ ⊒ (∀X.X). The right-hand side V ⟨ α? ⟩ must
(by parametricity) reduce to blame. (The other possibility, that it
loops forever, cannot occur becase V is a value.)
...
TODO: CLAUDE NOTES THAT THE ABOVE IS NOT CORRECT,
BECAUSE WE HAVE
...
Note also that the existence of the above means that narrowing
(and widening) is not unique, as we have both

    (να.(★→★)? ; (α!→α?)) : ★ ⊒ (∀X.X→X)
    ((★→★)? ; (να.α!→α?)) : ★ ⊒ (∀X.X→X)

A possible fix is to change the grammar and rules as follows:

  cross narrowing
    g,h  ::=  id_α | id_X | id_ι | s̅→t | ∀X.s[X] | να.s[α]
  narrowing
    s,t  ::=  g | id_★ | G?;g | s;α♯
...
This means that cross-narrowing no longer preserves the underlying
type, but that's not a serious problem. (It does preserve underlying
ground type, if we define ∥∀X.A[X]∥ = ∥A[X]∥.) The change also
syntactically rules out the problematic corner case, (να.α?).

If we make this fix, can we update the definition of composition?
At minimum, it get's messier.
...
Some examples:
  ...
  (να.νβ.α!→β!→α?) : ★→★→★ ⊒ ∀X.∀Y.X→Y→X
  (νβ.να.α!→β!→α?) : ★→★→★ ⊒ ∀Y.∀X.X→Y→X
  (να.α!→id_★→α?) ⨾ (νβ.∀X.id_X→β!→id_X)  =  (νβ.να.α!→β!→α?)
...
## Relating imprecisions: (γ ⊢ p ≈_σ q)
...
## Relating imprecisions [alternate definition, unused] (γ ⊢ p ≈ q)
...
Proof. By case analysis on the derivation of p.
  ...
  Case ⊒-
    ...
    --------------------------------- ⊒- p ⨾ t ≈ id_α
    σ ⊢ V ⟨ α♯ ⟩ ⊒ V′ ⟨ t ⟩ : id_α
    Can only happen if p = t = id_α, in which case
    V′ ⟨ t ⟩ is not a value.
  ...
  Case ⊒+
    ...
    ------------------------------ ⊒+  r ≈ id_α ⨾ t
    σ ⊢ V ⟨ α♯ ⟩ ⊒ V′ ⟨ t̅ ⟩ : id_α
    Can only happen if t = r₀ = id_α, in which case
    V′ ⟨ t̅ ⟩ is not a value.
...
on the derivation of ⟨ ν̅α.t[α♯] ⟩ and the derivation of V ⊒ V′.
  ...
  Case ⊒+
      ...
      σ ⊢ V ⊒ V′ ⟨ ∀X.s[X] ⟩ : ∀X.p₁[X]
      ----------------------------------------------- +⊒ (ii)
      σ ⊢ V ⟨ ν̅α.t[α] ⟩ ⊒ V′ ⟨ ∀X.s[X] ⟩ : (να.p₂[α])

      (i)   (∀X.p₁[X]) ⨾ (∀X.s̄[X]) ≈ (∀X.p₀[X])
      (ii)  (να.p₂[α]) ≈ (να.t[α]) ⨾ (∀X.p₁[X])
    ...
    =/=
      ...
      (iii)  (να.p₃[α]) ≈ (να.t[α]) ⨾ (∀X.p₀[X])
      (iv)   (να.p₂[α]) ⨾ (∀X.s̄[X]) ≈ (να.p₃[α])
  ...
  Case +⊒
    ...
    =/=
      ...
      σ ⊢ V ⊒ V′ : ∀X.p₀[X]
      ----------------------------------- +⊒ (iii)
      σ ⊢ V ⟨ ν̅α.t̅[α] ⟩ ⊒ V′ : (να.p₃[α])
      ---------------------------------------------------- ⊒+ (iv)
      σ, Π^☆ ⊢ V ⟨ ν̅α.t̅[α] ⟩ ⊒ V′ ⟨ ∀X.s̅[X] ⟩ : (να.p₂[α])
      (iii)  (να.p₃[α]) ≈ (να.t[α]) ⨾ (∀X.p₀[X])
      (iv)   (να.p₃[α]) ⨾ (∀X.s[X]) ≈ (να.p₂[α])

    —↠_{Π^★}/=  (by induction, V ⟨ ν̅α.t[α] ⟩ —↠_{Π^★} W)

      σ, Π^☆ ⊢ W ⊒ V′ : (να.p₃[α])
      ----------------------------------------- ⊒+ (iv)
      σ, Π^☆ ⊢ W ⊒ V′ ⟨ ∀X.s̅[X]} ⟩ : (να.p₂[α])

      We define p₃ by (iii), and (iv) follows because
        (να.p₃[α]) ⨾ (∀X.s[X]) ≈(iii)
        (να.t[α]) ⨾ (∀X.p₀[X]) ⨾ (∀X.s[id_X]) ≈(i)
        (να.t[α!]) ⨾ (∀X.p₁[id_X]) ≈(ii)
        (να.p₂[α!])
  ...
  Case -⊒-
      ...
      σ ⊢ V ⊒ V′ : p₀
      ---------------------------------------------- -⊒- (i)
      σ ⊢ V ⟨ να.s₀[α] ⟩ ⊒ V ⟨ να.t₀[α] ⟩ : ∀X.p₁[X]
      ---------------------------------------------------------- +⊒ (ii)
      σ ⊢ V ⟨ να.s₀[α] ⟩ ⟨ ν̅α.t̅[α] ⟩ ⊒ V ⟨ να.t₀[α] ⟩ : να.p₂[α]

      (i)   p₀ ⨾ (να.s₀[α]) ≈ (να.t₀[α]) ⨾ (∀X.p₁[X])
      (ii)  (να.p₂[α]) ≈ (να.t[α♯,α♭->α!,α?]) ⨾ (∀X.p₁[X])

    ⊢→/=
      σ ⊢ (να:=★. (V ⟨ να.s₀[α] ⟩) α) ⟨ t̅[α] ⟩ ⊒ V ⟨ να.t₀[α] ⟩ : να.p₂[α]
    —→_{α:=★}/=
      σ, α:=☆ ⊢ (V ⟨ να.s₀[α] ⟩) α ⟨ t̅[α] ⟩ ⊒ V ⟨ να.t₀[α]} ⟩ : να.p₂[α]
    ⊢→/=

      σ ⊢ V ⊒ V′ : p₀
      ------------------------------------------------- -⊒- (iii)
      σ, α:=id_★ ⊢ V ⟨ s₀[α] ⟩ ⊒ V ⟨ t₀[α] ⟩ : p₁[id_α]
      ------------------------------------------------------- +⊒ (iv)
      σ, α:=id_★ ⊢ V ⟨ s₀[α] ⟩ ⟨ t̅[α] ⟩ ⊒ V ⟨ t₀[α] ⟩ : p₂[α]
      ---------------------------------------------------------- ⊒⟨ν⟩
      σ, α:=☆ ⊢ V ⟨ s₀[α] ⟩ ⟨ t̅[α] ⟩ ⊒ V ⟨ να.t₀[α] ⟩ : να.p₂[α]
...
on the derivations of t̅ and V ⊒ V′.
  ...
  Case (s;t)
   ...
   Case α♭
      ...
      ----------------------------- +⊒  α♯ ⨾ id_α ≈ α♯
      σ ⊢ V ⟨ α♭ ⟩ ⊒ V′ : α♯
      by canonical values, V = V″ ⟨ α♯ ⟩ and by Left Seal Narrowing Inversion
    =/=
      σ ⊢ V″ ⊒ V′ : α♯
      ------------------------ -⊒  α♯ ⨾ id_α ≈ α♯
      σ ⊢ V″ ⟨ α♯ ⟩ ⊒ V′ : id_α
...
Proof. By induction on the proof of the hypothesis.
  ...
  —↠_{Π^★}/= (induction)
    ...
    Note that V ⟨ s[α] ⟩ is itself a value, so induction applies.
...
## Wrap Narrowing Lemma
    ...
    σ ⊢ V′ ⊒ V ⟨ s̅→t ⟩ : p̅→q    σ ⊢ W′ ⊒ W : p
    -------------------------------------------
    σ ⊢ V′ W′ ⊒ (V ⟨ s̅→t ⟩) W : q
  —↠_{Π^★}/⊢→
    σ, Π^☆ ⊢ N′ ⊒ V (W ⟨ s̅ ⟩) ⟨ t ⟩ : p

Proof. By induction on the derivation of σ ⊢ V′ ⊒ V ⟨ s̅→t ⟩ : p̅→q.
...
(Jeremy: Could the p here be an r?)
...
Proof: By induction on the derivations of σ ⊢ M ⊒ M′ : p and M′ —→_Π′ N′.
    ...
    V′ ⟨ ν̅α.p̅[α] ⟩  ⊢→  να:=★.(V′ α ⟨ p̅[α] ⟩)

                                       p̅[α♯,α♭] : A[α] ⊑ B
                                       ----------------
         σ ⊢ L ⊒ V′ : (να.r[α!,α?])    ν̅α.p̅[α♯,α♭] : ∀X.A[X] ⊑ B
         ------------------------------------------------------- ⊒+ (i)
         σ ⊢ L ⊒ V′ ⟨ ν̅α.p̅[α♯,α♭] ⟩ : q
         (i)  q⨾(να.p[α!,α?]) ≈ (να.r[α!,α?])
       ⊢→
         σ, α:=★ ⊢ L ⊒ V′ : (να.r[α!,α?])
         -------------------------------- ⊒α
         σ, α:=★ ⊢ L ⊒ V′ α : r[α!,α?]          p̅[α♯,α♭] : A[α] ⊑ B
         ---------------------------------------------------------- ⊒+ (ii)
         σ, α:=★ ⊢ L ⊒ (V′ α ⟨ p̅[α♯,α♭] ⟩) : q
         ------------------------------------- ⊒ν
         σ ⊢ L ⊒ να:=★.(V′ α ⟨ p̅[α♯,α♭] ⟩) : q

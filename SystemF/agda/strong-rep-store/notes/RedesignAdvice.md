...
owner lookup.  Two realizations:
  (i) STORE-PASSING (GTSF's Σ): reps in a store, `(α , A) ∈ Σ`;
      ...
      absence killed strong-rep-store/'s ambient-dependent readings.
...
- Jeremy's caveat is correct and load-bearing: Conversion does NOT do
  ...
  exist inside — strong-rep-store/'s tight-interior discipline, kept, rep-free)
  + the FACE CONVERSION (GTSF-style, store/owner-backed).  strong-rep-store/

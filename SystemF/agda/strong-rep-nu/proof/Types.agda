module strong-rep-nu.proof.Types where

-- File Charter:
--   * THE LEMMAS ABOUT `renameᵗ` AND `substᵗ` the two-universe layer
--     needs: `substᵗ-cong`, `extsᵗ-renᵗ`, `substᵗ-renᵗ`.
--   * NOT THE DEFINITIONS (strong-rep-nu.Types) and not the full
--     algebraic theory (strong-rep-nu.proof.TypeSubst).
--   * IT IS THE BOTTOM OF THE HIERARCHY: it imports
--     strong-rep-nu.Types and the standard library and NOTHING

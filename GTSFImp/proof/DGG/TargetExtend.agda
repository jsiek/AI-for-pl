...
mutual
  data TargetExtendPlan : ∀ {Cᴸ Cᴿ : Ctx}
    ...
    target-extend-lift-left :
      ...
      → (Γᴸ≡ : Γᴸ¹ ≡ TC.⇑ᶜ Γᴸ)
      → TargetExtendPlan
          (W ▻ᶜ lift-left-changeᶜ Γᴸ≡) Cᴿ⁺ rho (keep pi)
    ...
    target-extend-bind-left :
      ...
      → (Γᴸ≡ : Γᴸ¹ ≡ TC.⇑ᶜ Γᴸ)
      → TargetExtendPlan
          (W ▻ᶜ bind-left-changeᶜ A Γᴸ≡) Cᴿ⁺ rho (keep pi)
    ...
    target-extend-bind-right :
      ...
      → (Γᴿ⁺≡ : Γᴿ⁺¹ ≡ TC.⇑ᶜ Γᴿ⁺)
      → TargetExtendPlan
          (W ▻ᶜ bind-right-changeᶜ B fresh Γᴿ≡)
          ⟨ suc Δᴿ⁺ ,
    ...
    target-extend-bind-both-raw :
      ...
      → (Γᴿ⁺≡ : Γᴿ⁺¹ ≡ TC.⇑ᶜ Γᴿ⁺)
      → TargetExtendPlan
          (W ▻ᶜ bind-both-changeᶜ represented Γᴸ≡ Γᴿ≡)
          ⟨ suc Δᴿ⁺ ,
            store-bind Σᴿ⁺ (renameᵗ (toRenameᵗ rho) B) ,
            Γᴿ⁺¹ ⟩
          (keep rho) (keep pi)
...
extendTarget-sourceRebaseCount : ∀ {Cᴸ Cᴿ} {W : Cᴸ ⊑ᶜ Cᴿ}
    {Cᴿ⁺ rho Δ⁺} {pi : centerᶜ W ↪ᵗ Δ⁺}
  → (plan : TargetExtendPlan W Cᴿ⁺ rho pi)
  → sourceRebaseCountᶜ (extendTarget plan) ≡ sourceRebaseCountᶜ W
extendTarget-sourceRebaseCount
    (target-extend-star fresh eqᴿ refl refl) = refl
extendTarget-sourceRebaseCount
    (target-extend-alias fresh eqᴿ refl refl) = refl
extendTarget-sourceRebaseCount (target-extend-skip plan) =
  extendTarget-sourceRebaseCount plan
extendTarget-sourceRebaseCount
    (target-extend-lift-both plan Γᴸ≡ Γᴿ≡ Γᴿ⁺≡) =
  extendTarget-sourceRebaseCount plan
extendTarget-sourceRebaseCount (target-extend-lift-left plan Γᴸ≡) =
  extendTarget-sourceRebaseCount plan
extendTarget-sourceRebaseCount (target-extend-bind-left plan Γᴸ≡) =
  extendTarget-sourceRebaseCount plan
extendTarget-sourceRebaseCount
    (target-extend-bind-right plan fresh⁺ Γᴿ≡ Γᴿ⁺≡) =
  extendTarget-sourceRebaseCount plan
extendTarget-sourceRebaseCount
    (target-extend-bind-both-raw plan represented⁺ Γᴸ≡ Γᴿ≡ Γᴿ⁺≡) =
  extendTarget-sourceRebaseCount plan
extendTarget-sourceRebaseCount
    (target-extend-bind-both-star-raw
      plan represented⁺ Γᴸ≡ Γᴿ≡ Γᴿ⁺≡) =
...
extendTarget-invariants plan no-rebase =
  directInvariantsᶜ (extendTarget plan)
    (trans (extendTarget-sourceRebaseCount plan) no-rebase)

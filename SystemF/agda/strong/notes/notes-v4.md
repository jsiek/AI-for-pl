...
SET.  v4 tags carry one slot each, so the conditions have no analogue —
but v3 used them to keep `DropReveal`/`DropConceal` from overlapping the
value grammar.  What plays that role here needs deciding; see §Reduction.

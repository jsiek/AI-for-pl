...
term-weaken Δ≤Δ′ incl (⊢blame ℓ) = ⊢blame ℓ
...
typing-renameᵀ {Δ′ = Δ′} {Σ = Σ} {Γ = Γ} {ρ = ρ}
    hρ (⊢Λ {M = M} {A = A} vM hM) =
  ⊢Λ (renameᵗᵐ-preserves-Value (extᵗ ρ) vM)
    (subst
...
typing-renameᵀ hρ (⊢blame ℓ) = ⊢blame ℓ
...
typing-renameˣ {Γ = Γ} {Γ′ = Γ′} {ρ = ρ} hρ (⊢Λ vM hM) =
...
typing-renameˣ hρ (⊢blame ℓ) = ⊢blame ℓ
...
typing-substˣ hσ (⊢blame ℓ) = ⊢blame ℓ

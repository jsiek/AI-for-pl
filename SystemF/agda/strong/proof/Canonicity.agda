...
-- went with the index (Jeremy's ruling, strong.Conversion): a mixed tree
...
-- spelling `s′` with a `SameConv` relating the two (strong.Reduction).
...
-- dropped while `strong.Examples` is unported on this branch; the
...
-- (strong.proof.Preserve, `preserve-TyPeelR-Λ`; the tree was untypeable

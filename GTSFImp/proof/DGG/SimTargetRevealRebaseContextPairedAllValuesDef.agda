...
module proof.DGG.SimTargetRevealRebaseContextPairedAllValuesDef where

-- File Charter:
--   * States paired type-application value simulation beneath one enclosing
--     target reveal/source-rebase boundary and an arbitrary CTI zipper.
--   * Begins after the source and target heads are values; its Proof owns the
--     five universal root analyses and replays the target trace through the
...
SimTargetRevealRebaseContextPairedAllValuesᵀ : Set₁

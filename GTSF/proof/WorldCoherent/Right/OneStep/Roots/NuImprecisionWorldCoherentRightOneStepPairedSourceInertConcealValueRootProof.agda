...
-- File Charter:
--   * Proves the exact paired-conceal outer-cast value root when the source
--     cast is inert.
--   * Catches the source inner term to a bullet-free value, transports exact
--     conceal evidence, and applies the generic source indexed value root.
--   * Contains no reveal or widening case, active-source synchronization,

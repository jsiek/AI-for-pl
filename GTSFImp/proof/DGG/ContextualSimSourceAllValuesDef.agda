...
module proof.DGG.ContextualSimSourceAllValuesDef where

-- File Charter:
--   * States source-only type-application value simulation beneath an
--     arbitrary caller CTI context after the target head has caught up.
--   * Keeps the silent target and rebuilt whole source in the inline
--     conclusion because target evolution may change the caller path.
...
ContextualSimSourceAllValuesᵀ : Set₁

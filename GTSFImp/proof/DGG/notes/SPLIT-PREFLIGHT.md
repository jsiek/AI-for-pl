...
| --- | --- | --- |
| 1. Four-mode model | PASS | `Var∼₅` has `★∼X∼★`, `X∼X`, `X∼★`, `★∼X`; `idᶜ₅` is crossable, `extᵐ₅` opens strict, `instᵐ₅`/`genᵐ₅` are unchanged, and star gates include dynamic plus crossable only. |
...
| 6. `SubstEnv∼` | PASS | `SubstEnv∼₅` re-keys the landed rigid fields to `cross-to-★`/`cross-from-★` for `★∼X∼★` only.  Strict fresh slots are exact-mode impossible again: `open-to-★-strict-slot-impossible₅`, `open-from-★-strict-slot-impossible₅`, and `open-cross-to-★-strict-slot-impossible₅`. |

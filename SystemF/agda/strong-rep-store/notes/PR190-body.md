...
| theorem (`strong-rep-store.TypeSafety`) | statement |
|---|---|
...
* **Public/proof split for progress.**  `Progress.agda` held the whole
  ...
  public `strong-rep-store.Progress` now holds only the statement `Progress` and
...
* **`Eval` for v2 — DONE.**  New public `strong-rep-store/Eval.agda`; no
...
* **Stranded v1 Agda deleted.**  Every `.agda` under `notes/old/` (the
  ...
  files) imported v1 modules — `strong-rep-store.Context`, `strong-rep-store.Boundary`,
  `strong-rep-store.BReduction`, `strong-rep-store.Weakening`, `strong-rep-store.Unfold` — deleted at
  ...
  v1 tree, commit `c5db9f59`, `SystemF/agda/strong-rep-store/notes/old/`), where
...
* **Deletion of four record files — RULED, DELETED.**  `proof/WallReach`,
  ...
  *delete* (2026-09-06, closed-world repo); the record lives in
  ...
  with them; later section numbers are unchanged.  `strong-rep-store/PLAN.md` moved
...
* **`Eval` — DONE**, review round 1 above.  v1's step/trace evaluator
  with `step-sound` (`strong-rep-store/Eval` + `EvalDec`, `8530bb7f`) went the way
  of the rest of v1; the v2 successor is `strong-rep-store/Eval.agda`, where the

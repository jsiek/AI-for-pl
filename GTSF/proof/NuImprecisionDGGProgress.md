...
| [`NuImprecisionLeftSourceAllocationRuntimeTargetBulletDef.agda`](NuImprecisionLeftSourceAllocationRuntimeTargetBulletDef.agda) | completed strict statement; proof fit under audit | Focused direct `⊑αᵀ` reconstruction for canonical source allocation. A first proof attempt established that equality of the two world expressions does not identify their proof-relevant renaming witnesses. The current audit instead factors the canonical `suc` renaming into `LiftLeftStoreⁱ`/`LiftLeftCtxⁱ` evidence and targets the existing `left-source-lift-⊑αᵀ` commuting square, avoiding equality between membership-proof functions and adding no carrier. |
| [`NuImprecisionLeftSourceAllocationRuntimeTransportDef.agda`](NuImprecisionLeftSourceAllocationRuntimeTransportDef.agda) | completed root statement; implementation plan partial | Mutually transports ordinary and quotiented QTI derivations through the canonical source-allocation renaming and returns direct relation derivations. Ginger exposed two integration constraints before implementation: generalized recursion loses the visibly right-lifted target world required at `⊑αᵀ`, and bullet-free cast/allocation branches require `LeftInsertion`. The plan now uses the repaired `LeftRenameNoBullet` for inert branches, recurses only through the unique active `RuntimeOK` subterm, and isolates the target-bullet commuting square in its focused helper. The helper's exact fit remains under audit; no public result carrier has been added. |

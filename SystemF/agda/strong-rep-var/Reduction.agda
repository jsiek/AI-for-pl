module strong-rep-var.Reduction where
...
--     strong-rep-var.TypeCheck, and the redex search that assembles them is
--     strong-rep-var.Eval.  Preservation, progress and canonical forms live
--     under strong-rep-var.proof with their public statements in
--     strong-rep-var.Preservation, strong-rep-var.Progress and
-- strong-rep-var.TypeSafety.  The
...
--     (strong-rep-var.Conversion) for a conversion, `_⊢_≈_⊣_`
-- (strong-rep-var.Ctx §5)
...
--
--   (1) V-Λ carries `Value N` (in strong-rep-var.Terms) — reduction goes under
-- Λ.
...
data _⊢_-→_ : Ctxᵗ → Term → Term → Set where
  ...
  -- always exists (`peel-premises-env`, strong-rep-var.Conversion §2c).  `t`
  -- needs no premise: it
  ...
  -- (strong-rep-var.Conversion) for the wrapper clause, exactly as it is
  -- `∋:=-det`
  ...
  -- of strong-rep-var.Examples loses its type at step 8 because

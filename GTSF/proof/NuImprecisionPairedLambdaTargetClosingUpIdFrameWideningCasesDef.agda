...
-- File Charter:
--   * Defines the two fused quotient up-id frame closing contracts split by
--     the constructors of QuotientWideningPair.
...
--     outer id-only narrowing typings, the forall-permutation proof, the
--     constructor-specific widening fields, the outer precision index,
--     external paired conversion, source runtime-bullet allocation, both
--     store lifts, and final structural universal reveal in one theorem.
...
PairedLambdaTargetClosingUpIdFrameQuotientIdWideningClosingᵀ : Set₁
...
PairedLambdaTargetClosingUpIdFrameQuotientCastWideningClosingᵀ : Set₁
PairedLambdaTargetClosingUpIdFrameQuotientCastWideningClosingᵀ =
  ...
  (∀ {ρ : StoreImp Φ Δᴸ Δᴿ}
      ...
      {ρ∀ : StoreImp ((zero ˣ⊑ˣ zero) ∷ ⇑ᵢ Φ)
        (suc Δᴸ) (suc Δᴿ)}
      {A C₀′ F E : Ty} {c c′ t : Coercion} {ν : ModeEnv}
      {p : Φ ∣ Δᴸ ⊢ `∀ F ⊑ `∀ C₀′ ⊣ Δᴿ}
      {q : ((zero ˣ⊑ˣ zero) ∷ ⇑ᵢ Φ)
        ∣ suc Δᴸ ⊢ `∀ E ⊑ C₀′ ⊣ suc Δᴿ} →
    StoreImpPrefix ρ₀ ρ →

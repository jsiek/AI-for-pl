...
| [`NuImprecisionWorldCoherentRightValueCatchupRuntimeNoBulletTransportDef.agda`](NuImprecisionWorldCoherentRightValueCatchupRuntimeNoBulletTransportDef.agda) | completed repaired flat statement; higher-order proof partial | Transports an active source sibling and no-bullet target sibling directly through a completed right-value catch-up. The proof now splits through `AtMostOne•`: the zero branch uses ordinary weakening, while the active branch carries only the completed source-store equality and derives every recursive typing from QTI. Bullet, allocation-prefix, application, primitive, and initial cast cases are checked locally; `ν`, the paired-cast and quotient-widening dependencies above, remaining casts, and explicit impossible cases remain. The Proof is parameterized on the two genuine right-silent leaves rather than hiding them in a new runtime result. |
| [`NuImprecisionLeftSourceAllocationRuntimeTransportDef.agda`](NuImprecisionLeftSourceAllocationRuntimeTransportDef.agda) | completed canonical statement; GPT-5.5 proof retry in progress | Mutually transports ordinary and quotiented QTI derivations through the one canonical source-allocation renaming. A first Ginger audit rejected arbitrary `LiftLeftStoreⁱ`/`LiftLeftCtxⁱ` inputs because they do not retain the canonical transported precision witnesses required by variable and target-bullet cases. The repaired boundary uses existing `LeftStoreRenameⁱ`/`LeftCtxRenameⁱ` evidence and returns direct relation derivations, with no parallel path or result carrier. |
